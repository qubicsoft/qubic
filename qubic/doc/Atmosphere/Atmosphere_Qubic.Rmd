...
with open('absorption coefficient/h2o_lines_550hPa_280K_1mm.out', 'r') as file:
    for line in file:
...
for name, list in name_dict.items():
    with open('absorption coefficient/h2o_lines'+name+'.out', 'r') as file:
        for line in file:
            parts = line.split() # Split the line into two parts based on whitespace
...
with open('absorption coefficient/h2o_self_continuum.out', 'r') as file:
    for line in file:
        parts = line.split() # Split the line into two parts based on whitespace
        h2o_self.append(float(parts[1]))

with open('absorption coefficient/h2o_air_continuum.out', 'r') as file:
    for line in file:
        parts = line.split() # Split the line into two parts based on whitespace
...
plt.plot(frequencies, name_dict['_550hPa_280K_1mm']+np.array(h2o_air)*(1e-10*n_air))
...
plt.vlines(247.5, 0, y_max, 'black', 'dashed')
plt.xlabel('Frequency [GHz]')
plt.ylabel(r'Molecular absorption coefficient $k_{H_2O}$ [$m^2$]')
