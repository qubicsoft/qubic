...
def gaussian_beam_width(z, freq):
    ...
    gaussian_beam_waist = wavelenght / (np.pi * beam_FWHM) * np.sqrt(2 * np.log(2)) # Not certain about the coefficient
    return gaussian_beam_waist * np.sqrt(1 + (wavelenght * z / (np.pi * gaussian_beam_waist**2))**2)
...
plt.plot(distances, [correlation_function(r) for r in distances])
...
```{python}
time_list = np.linspace(0, 25, 150)
...
for r0 in r0_list:
    ...
    for t in range(np.shape(time_list)[0]):
        ...
        corr.append(atm_correlations_gaussian_wind(h, h+3000, r_i=(0,0,1), ti=0, az_ti=0, el_ti=np.radians(45), r_j=(0,0,1), 
                               tj=time, az_tj=azimuth, el_tj=np.radians(45), W=W, phi_W=phi_W, freq=150e9, n=8, r0=r0)[1])
    plt.plot(time_list, corr/corr[0], label='r0 = '+str(r0)+' m')

plt.xlabel('Time (s)')
plt.ylabel('Correlations '+r'$(K^2)$')
plt.title('Varying r0. '+r'$ss = 1°,\, \Delta az = 5°,\, W = 0 m.s^{-1},\, \phi_W = 0°$')
plt.legend()
plt.show()
```
...
for Wind in W_list:
    ...
    for t in range(np.shape(time_list)[0]):
        ...
        azimuth = azimuth_list[t]
        corr.append(atm_correlations_gaussian_wind(h, h+3000, r_i=(0,0,1), ti=0, az_ti=0, el_ti=np.radians(45), r_j=(0,0,1), 
                               tj=time, az_tj=azimuth, el_tj=np.radians(45), W=Wind, phi_W=phi_W, freq=150e9, n=8, r0=300)[1])
    plt.plot(time_list, corr/corr[0], label='W = '+str(Wind)+r' $m.s^{-1}$')

plt.xlabel('Time (s)')
plt.ylabel('Correlations '+r'$(K^2)$')
plt.title('Varying wind speed W. '+r'$r0 = 300 m,\, ss = 1°,\, \Delta az = 5°,\, \phi_W = 0°$')
plt.legend()
plt.show()
```
...
for phi_Wind in phi_W_list:
    ...
    for t in range(np.shape(time_list)[0]):
        ...
        azimuth = azimuth_list[t]
        corr.append(atm_correlations_gaussian_wind(h, h+3000, r_i=(0,0,1), ti=0, az_ti=0, el_ti=np.radians(45), r_j=(0,0,1), 
                               tj=time, az_tj=azimuth, el_tj=np.radians(45), W=10, phi_W=np.radians(phi_Wind), freq=150e9, n=8, r0=300)[1])
    plt.plot(time_list, corr/corr[0], label=r'$\phi_W =$'+str(phi_Wind)+'°')

plt.xlabel('Time (s)')
plt.ylabel('Correlations '+r'$(K^2)$')
plt.title('Varying wind azimuth '+r'$\phi_W.\. r0 = 300 m,\, ss = 1°,\, \Delta az = 5°,\, W = 10 m.s^{-1}$')
plt.legend()
plt.show()
```
