...
for i in range(len(all_nfsub)):
    ...
    for isub in range(nfsub):
        xx, yyfs, fitcov = qss.get_noise_invcov_profile(maps[isub,:,:], cov, 
...
We can see here that we should probably use a different fitting for I on one heand and Q,U on the other hand, this is not necessary for low nsub  but becomes quite clear from 3 and above. This will be treated later...
...
for isub in range(nfsub):
    xx, yyfs, fitcov = qss.get_noise_invcov_profile(new_sub_maps[isub,:,:], cov, 
                                                 label='FastSim sub={}/{}'.format(isub+1,nfsub), fit=False, norm=False,
                                                 allstokes=True, doplot=True)
    plot(myfitcovs[isub][0,:], np.sqrt(myfitcovs[isub][1,:]), label='End-To-End Fit sub={}/{}'.format(isub+1,nfsub))
    legend(fontsize=9)

cInew, cQnew, cUnew, fitcov, noise_norm = qss.get_cov_nunu(new_sub_maps, cov)
...
Now th real case with correlation from End-To-End simulations.
