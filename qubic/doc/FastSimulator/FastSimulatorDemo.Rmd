...
```{python}
# If you want to save maps somewhere, change the following path:
#save_dir = '/home/lmousset/QUBIC/Qubic_work/Component_sep/QSSmaps/'
...
### First Example: CMB Only, a single noise realization
In this case the code returns the number of sub-bands from the input dictionary (here 1). 

The maps are centered on the Galactic Center. You can provide your own coverage map centered elsewhere (optional argument) for another target...
...
maps_qubic, maps_noiseless, maps_noise, coverage = Qubic_sky.get_partial_sky_maps_withnoise(spatial_noise=True,
...
#### Save the maps
# Reduce the full map to a patch
seenmap = coverage > np.max(coverage) * 0.1
patch_noise = maps_noise[:, seenmap, :]

name = f'NoisePatch_{nbands}bands_{config}_spatialCovTrue_nunuCovTrue.npy'
#np.save(save_dir + name, patch_noise)
```
...
for isub in range(newd['nf_recon']):
    ...
    plt.legend()
    plt.xlim(0.9, 10.1)
    plt.xlabel('1/(Normalized coverage)')
#     ylim(0, np.max(yyfs)*1.1)
plt.tight_layout()
plt.savefig(save_dir + f'FastSim_RMSscaling_{config}_{nbands}bands.pdf', bbox_inches='tight')
```
...
for s in range(3):
    ...
    plt.title('End-To-End \n Cov {} - {} bands'.format(stn[s], newd['nf_recon']))
    ...
    plt.title('FastSim \n Cov {} - {} bands'.format(stn[s], newd['nf_recon'] ))
plt.tight_layout()
plt.savefig(save_dir + f'FastSim_Corr_{config}_{nbands}bands.pdf', bbox_inches='tight')
```
...
```{python}
cell = DataFastSim['clnoise']
theta_deg, ctheta_end2end =  qc.cell_2_ctheta(cell, theta_deg=np.linspace(0, 20, 100))
```

```{python}
plt.figure(figsize=(8, 6))
for i in range(newd['nf_recon']):
    plt.plot(th, allcthI[i]/allcthI[i][0], 'o', label=f'Sub-band {i+1}')
#    plot(th, allcthQ[i]/allcthQ[i][0], 'o', label=f'Sub - band {i+1}')
 #   plot(th, allcthU[i]/allcthU[i][0], 'o', label=f'Sub - band {i+1}')
#plt.plot(theta_deg, ctheta_end2end/ctheta_end2end[0], 'k')
plt.title(f'FastSim - I - {nbands} bands')    
plt.legend()
plt.xlabel(r'$\theta$ [deg]', fontsize=16)
plt.ylabel(r'$C(\theta$)', fontsize=16)

plt.tight_layout()
plt.savefig(save_dir + f'FastSim_Ctheta_{config}_{nbands}bands.pdf', bbox_inches='tight')
