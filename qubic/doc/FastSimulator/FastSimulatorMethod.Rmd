---
jupyter:
  jupytext:
    text_representation:
      extension: .Rmd
      format_name: rmarkdown
      format_version: '1.2'
      jupytext_version: 1.16.1
  kernelspec:
    display_name: Python 3 (ipykernel)
    language: python
    name: python3
---

```{python}
import sys
sys.path.insert(0, '/home/lkardum/qubic/qubic')
import os
```

```{python}
# %matplotlib inline
from matplotlib import pyplot as plt
import healpy as hp
import glob
from scipy.optimize import curve_fit, minimize
import pickle
from importlib import reload
import time
import scipy
import matplotlib as mpl
import numpy as np
from numpy import exp
import pandas as pd
import gplearn as gp

# Specific qubic modules
from qubicpack.utilities import Qubic_DataDir
from pysimulators import FitsArray
import pysm3
import qubic
from qubic import QubicSkySim as qss
from qubic import fibtools as ft
from qubic import camb_interface as qc
from qubic import SpectroImLib as si
from qubic import NamasterLib as nam
from qubic import mcmc

reload(qss)
reload(ft)

plt.rc('figure', figsize=(16, 10))
plt.rc('font', size=15)
mpl.rcParams['image.cmap'] = 'jet'
```

```{python}
import importlib.util
module_path = "/home/lkardum/qubic/qubic/QubicSkySim.py"
spec = importlib.util.spec_from_file_location("qss", module_path)
qss = importlib.util.module_from_spec(spec)
spec.loader.exec_module(qss)
```

```{python}
import importlib.util
module_path = "/home/lkardum/qubic/qubic/fibtools.py"
spec = importlib.util.spec_from_file_location("ft", module_path)
ft = importlib.util.module_from_spec(spec)
spec.loader.exec_module(ft)
```

This notebook shows how the QUBIC FastSimulator is made. It is a shortcut to produce QUBIC noise maps without having to go through the long and CPU greedy process of QUBIC Map-Making on supercomputers.

The FastSimulator reproduces the noise structure that has been measured so far in simulations:
- Noise RMS profile as a function of coverage
- Noise spatial correlation that is responsible for lower $\Delta C_\ell$ at low $\ell$ and is a key aspect of the QUBIC nosie structure, originating from noise anticorrelation at the scale of the distance between two synthesized beam peaks.
- Noise correlation (mostly anticorrelation) sub-bands reconstructed when performing spectro-imaging.




```{python}
directory
```

```{python}
### Path to the files used for the FastSimulator
version_FastSim = '01'
dirFast = os.path.join('..', '..', 'data', f'FastSimulator_version{version_FastSim}') + os.sep
```

```{python}
### Some initializations
### Load a dictionnary
dictfilename = 'BmodesNoDustNoSystPaper0_2020.dict'

# Read dictionary chosen
d = qubic.qubicdict.qubicDict()
d.read_from_file(dictfilename)
d['nside']=256
center = qubic.equ2gal(d['RA_center'], d['DEC_center'])
print(center)
```

# Noise RMS as a function of coverage
Here it is only shown in the case of a single sub-band.

We first need to read maps constructed using the full End-To-End QUBIC Simulation (Map-Making from TOD) and fit its noise profile (as a function of inverse coverage) with an appropriate function:

```{python}
#reload(qss)
#### This function reads the maps
def read_files_mc(dirmap,file_rnd_string, verbose=False):
    m = np.array(FitsArray(glob.glob(dirmap+'*_maps_recon_*'+file_rnd_string+'.fits')[0]))
    c = np.array(FitsArray(glob.glob(dirmap+'*_maps_convolved_*'+file_rnd_string+'.fits')[0]))
    cov = np.array(FitsArray(glob.glob(dirmap+'*_maps_coverage_*'+file_rnd_string+'.fits')[0]))
    with open(glob.glob(dirmap+'*_dictionary_'+file_rnd_string+'.pickle')[0], 'rb') as handle: d = pickle.load(handle)
    with open(glob.glob(dirmap+'*_input_cell_'+file_rnd_string+'.pickle')[0], 'rb') as handle: icl = pickle.load(handle)
    filetype = glob.glob(dirmap+'*_maps_recon_*'+file_rnd_string+'.fits')[0].split('/')[-1]
    if verbose: print('read {}'.format(filetype))
    return m, c, cov, d, icl, filetype

### Get reconstructed maps from a simulation made on NERSC with 200000 pointings and tol=1e-5
### First Instrument
### detector_nep=4.7e-17
### effecgtive_duration = 3 years
nptg = 200000

file_rnd_string='H990hwzaGn'
tol=1e-5

#maps, convolved maps, coverage, dictionary, input_cell
m, c, cov, d, icl, ftype = read_files_mc(dirFast, file_rnd_string, verbose=True)
```

```{python}
plt.figure()
stn = ['I','Q','U']
for istokes in [0,1,2]:
    hp.mollview(m[0,:,istokes], title=stn[istokes], sub = (1,3,istokes+1))

plt.figure()
for istokes in [0,1,2]:
    hp.mollview(c[0,:,istokes], title=stn[istokes], sub = (2,3, istokes+1))

plt.figure()
for istokes in [0,1,2]:
    hp.mollview(m[0,:,istokes]-c[0,:,istokes], title=stn[istokes], sub = (3,3, istokes+1))
```

## sta radi fit u get_noise_invcov_profile? u iducoj celiji je True a onda je posli false u Qubic law
## jel norm radi normalizaciju na 1? zasto je u iducoj celiji sve pomnozeno sa yysim[0] ? kad je pomnozeno sa yysim onda se slaze sa norm = false a inace sa obicnim 
## zasto je signoise 75? kad je 90 onda se odlicno slaze sa simuliranim, vidi se cak i na gnomview kartama
## zasto je allstokes True pa nije

## zasto se na kraju kreirana mapa uopce ne slaze? izgleda ka da ne ovisi puno o inv_cov funkciji nego o samom izboru signoise

```{python}
seenpix = cov > (0.1 * np.max(cov))
covnorm = cov / np.max(cov)
```

```{python}
nbins = 100   
QUsep = True
#It is a law for effective RMS as a function of inverse coverage and is 2D array
#with the first one being (nx samples) inverse coverage and the second being the corresponding effective variance
xx, rms_tot, rms_I, rms_QU, effective_variance_invcov, fitted_params, fitted_params_I, fitted_params_QU = qss.get_noise_invcov_profile(m[0,:,:]-c[0,:,:], 
                                                                  cov, 0.1, nbins=nbins, QUsep=True,
                                                                  fit=True, norm=True, allstokes=True)

label = 'QUBIC Average'
plt.plot(xx ** 2, rms_tot, 'o', label=label + ' IQU')
plt.plot(xx ** 2, rms_I, 'o', label=label + ' I')
plt.plot(xx ** 2, rms_QU, 'o', label=label + ' QU / sqrt(2)')
    
#plt.plot(effective_variance_invcov[0,:], np.sqrt(effective_variance_invcov[1,:]),'--', label='Qubic Law I', color = 'orange')
#plt.plot(effective_variance_invcov[0,:], np.sqrt(effective_variance_invcov[2,:]),'--', label='Qubic Law QU', color = 'green')

poly_model = lambda x, a, b, c, d, e, f, g: (
                    a + b * x + f * x ** 2 + g * x ** 3 + c * np.exp(-d * (x - e)))
plt.plot(xx ** 2, poly_model(xx ** 2, *fitted_params_I[0]), label=' Fit I', color = 'orange')
plt.plot(xx ** 2, poly_model(xx ** 2, *fitted_params_QU[0]), label=' Fit QU / sqrt(2)', color = 'green')
#plt.xlim(0.9,10.1)
plt.ylim(np.min(rms_tot)/1.2, np.max(rms_tot)*1.2)
plt.legend()
```

Calculate the signoise of the simulated maps. This signoise is reapplied when simulating new maps from simulated properties. 

```{python}
sky_config = {'cmb': None}
signoise = qss.optimize_sigma_sec(m[0,:,:]-c[0,:,:], cov, sky_config, d)
```

Create a new map with the same level, coverage, and noise from the inverse coverage profile. 

```{python}
seed = None
sky_config = {'cmb': seed}
Qubic_sky = qss.Qubic_sky(sky_config, d)
qubicnoise = Qubic_sky.create_noise_maps(signoise, cov, effective_variance_invcov=effective_variance_invcov)
```

```{python}
plt.figure()
stn = ['I','Q','U']
for istokes in [0,1,2]:
    hp.mollview(m[0,:,istokes]-c[0,:,istokes], title='Simulated noise '+stn[istokes], sub = (1,3,istokes+1))
for istokes in [0,1,2]:
    hp.mollview(qubicnoise[:,istokes], title='Reconstructed noise '+stn[istokes], sub = (2,3,istokes+1))
```

```{python}
xx, yyfs, _, _, eff_var_invcov_fs, _, _, _ = qss.get_noise_invcov_profile(qubicnoise, cov, fit=True, allstokes=False, QUsep=True)
xx, yysim, _, _, eff_var_invcov_sim, _, _, _ = qss.get_noise_invcov_profile(m[0,:,:]-c[0,:,:], cov, QUsep=True, fit=True, norm = False)
#plt.plot(effective_variance_invcov[0,:], np.sqrt(effective_variance_invcov[1,:])*yysim[0],'--',label='Qubic Law I scaled', color = 'green')
plt.plot(effective_variance_invcov[0,:], np.sqrt(effective_variance_invcov[1,:]),'--',label='Qubic Law I', color = 'green')

#plt.plot(effective_variance_invcov[0,:], np.sqrt(effective_variance_invcov[2,:])*yysim[0],'--',label='Qubic Law QU scaled', color = 'red')
plt.plot(effective_variance_invcov[0,:], np.sqrt(effective_variance_invcov[2,:]),'--',label='Qubic Law QU', color = 'red')


xx, yysim, _, _, _, _, _, _ = qss.get_noise_invcov_profile(m[0,:,:]-c[0,:,:], cov, QUsep=True, fit=False, norm = True)

plt.plot(xx ** 2, yyfs, 'o', label=label + ' Reconstructed map')
plt.plot(xx ** 2, yysim, 'o', label=label + ' Simulated map')
plt.ylabel('Effective Noise Variance')
plt.xlabel(r'$ \sqrt{normalized coverage^{-1}} $')
plt.xlim(0,12)
plt.ylim(0,6)
plt.legend()

plt.rc('figure', figsize=(16, 10))
res = 15
plt.figure()
rng = [5, 5, 5]
for s in range(3):
    hp.gnomview(qubicnoise[:,s], rot=center, reso=res, sub=(3,2,2*s+1), title='FastSim '+stn[s],
               min=-rng[s], max=rng[s])
    hp.gnomview(m[0,:,s]-c[0,:,s], rot=center, reso=res, sub=(3,2,2*s+2), title='NERSC 200000 '+stn[s],
               min=-rng[s], max=rng[s])
```

# Noise spatial correlation
If one flattens the noise in the maps by renormalizing by the above fitted noise profile, one can calculate the C(theta) of the new map and explore possible spatial correlations:

```{python}
######### Flattening maps
#qubicnoise = Qubic_sky.create_noise_maps(40, cov, effective_variance_invcov=effective_variance_invcov)
#qubicnoise = Qubic_sky.create_noise_maps(signoise, cov, effective_variance_invcov=effective_variance_invcov)

new_qubicnoise = qss.correct_maps_rms(qubicnoise, cov, eff_var_invcov_sim)
orig_qubicnoise = qss.correct_maps_rms(m[0,:,:]-c[0,:,:], cov, eff_var_invcov_sim)

######### Displaying new profiles
plt.subplot(1,2,1)
xx_new, avg_new, dyI_new, dyQ_new, dyU_new = qss.get_angular_profile(new_qubicnoise, nbins=100,thmax=22)

plt.plot(xx_new, avg_new, 'o', label=label)
plt.plot(xx_new, dyI_new, label=label + ' I', alpha=0.3)
plt.plot(xx_new, dyQ_new / np.sqrt(2), label=label + ' Q/sqrt(2)', alpha=0.3)
plt.plot(xx_new, dyU_new / np.sqrt(2), label=label + ' U/sqrt(2)', alpha=0.3)
plt.xlabel('Angle [deg.]')
plt.ylabel('RMS')
plt.legend()
plt.ylim(0,1.2)
plt.xlim(0,22)
plt.title('FastSim')

plt.subplot(1,2,2)
xx_orig, avg_orig, dyI_orig, dyQ_orig, dyU_orig = qss.get_angular_profile(orig_qubicnoise, nbins=100,thmax=22)
plt.plot(xx_orig, avg_orig, 'o', label=label)
plt.plot(xx_orig, dyI_orig, label=label + ' I', alpha=0.3)
plt.plot(xx_orig, dyQ_orig / np.sqrt(2), label=label + ' Q/sqrt(2)', alpha=0.3)
plt.plot(xx_orig, dyU_orig / np.sqrt(2), label=label + ' U/sqrt(2)', alpha=0.3)
plt.xlabel('Angle [deg.]')
plt.ylabel('RMS')
plt.legend()
plt.ylim(0,1.2)
plt.xlim(0,22)
plt.title('Original')
```

```{python}
############ Calculating C(theta)
ns = d['nside']
okpix = cov > (np.max(cov)*0.1)
ip = np.arange(12*ns**2).astype(int)
ipok = ip[okpix]

th, cth_qn, err_qn = qss.ctheta_parts(new_qubicnoise[:,0], ipok, 0, 20, 20, nsplit=5, degrade_init=128, verbose=False)
th, cth_200k, err_200k = qss.ctheta_parts(orig_qubicnoise[:,0], ipok, 0, 20, 20, nsplit=5, degrade_init=128, verbose=False)

########### Fitting C(theta) with a relevant function
from scipy.optimize import curve_fit
fct = lambda x, a, b, c: a * np.sin(x/b) * exp(-x/c)
results_qn = curve_fit(fct, th[1:], (cth_qn[1:]/cth_qn[0]),maxfev=100000, ftol=1e-7, p0=[0,1,1])
results_200k = curve_fit(fct, th[1:], (cth_200k[1:]/cth_200k[0]),maxfev=100000, ftol=1e-7, p0=[0,1,1])

plt.figure()
thth = np.linspace(0,20,100)
p=plt.plot(th,(cth_qn/cth_qn[0]),'o', label='FastSim')
plt.plot(thth, fct(thth, *results_qn[0]), color=p[0].get_color(),
     label=r'$a\times\sin(\theta/b)\times\exp(\theta/c)$'+'\n a={0:4.2f} b={1:4.2f} c={2:4.2f}'.format(*results_qn[0]))

p=plt.plot(th,(cth_200k/cth_200k[0]),'o', label='200k')
plt.plot(thth, fct(thth, *results_200k[0]), color=p[0].get_color(),
     label=r'$a\times\sin(\theta/b)\times\exp(\theta/c)$'+'\n a={0:4.2f} b={1:4.2f} c={2:4.2f}'.format(*results_200k[0]))

plt.legend()
plt.xlabel(r'$\theta$ [deg]')
plt.ylabel(r'$C(\theta)$ [Normalized]')
plt.ylim(-0.1,1)
```

We see that the actual maps coming from End-To-End simulations exhibit a non-zero C(theta) (angular correlation function) that we need to account for when simulating. This is explored in details in the Notebook FastSimulation-CMB and referenced notebooks therein. It uses the Legendre transform of the C(theta) fitted above (therefore in harmonic space) to generate noise according to this spatial correlation.

The Cl file obtained from the aboce C(theta) is found in the Doc/FastSimulator/Data directory as cl_corr_noise_nersc200k.pk 

For reasons that are yet to be understood, this Cl needs to be renormalized by a fadctor 4.5 so that simulated maps have the correct C(theta).

An other important point here is that due to aliasing when simualting the nosie in harmonic space (ellmax cutoff) the RMS of the maps obtained this way has nothing to do with the intended RMS. THis is however not a problem as the CL of this noise are correct within the [0, ellmax] useful badwidth. As a consequence, when trying to adjust the noise level (signoise) in the FastSimulator, one needs to adjust the RMS at first without spatial correlations and then turn on spatial correlation, and not use the resulting maps RMS anywhere in the analysis as it is not correct.

NB: if the simualtions you intend to perform are not be tansformed into Cl but rather for working in map-space, it is probably very sufficient to neglect the spatial correlation of the noise and use clnoise=None qss.create_noise_maps()... This will have the important advantage of producing maps with a meaningful RMS.


This was produced by https://github.com/qubicsoft/qubic/blob/master/qubic/scripts/QubicGeneralPaper2020/FastSimulation-CMB.Rmd

```{python}
clth = pickle.load(open(dirFast + 'cl_corr_noise_nersc200k.pk', "rb" ) )
plt.plot(clth,label='initial')
alpha = 4.5 ### See notebook called "2pt-Correlation Function" for an empirical explanation of alpha
clth = (clth -1 ) * alpha + 1

plt.plot(clth, label='renorm')
plt.legend()

```

#### Let's simulate noise with the above spatial correlation:

```{python}
qubicnoise = Qubic_sky.create_noise_maps(signoise, cov, 
                                         effective_variance_invcov=effective_variance_invcov,
                                        clnoise=clth)

### Get the C(theta)
corrected_qubicnoise = qss.correct_maps_rms(qubicnoise, cov, eff_var_invcov_sim)
th, cth, err = qss.ctheta_parts(corrected_qubicnoise[:,0], ipok, 0, 20, 20, nsplit=5, degrade_init=128, verbose=False)

plt.figure()
thth = np.linspace(0,20,100)
p=plt.plot(th,(cth_qn/cth_qn[0]),'o', label='FastSim')
plt.plot(thth, fct(thth, *results_qn[0]), color=p[0].get_color(),
     label=r'$a\times\sin(\theta/b)\times\exp(\theta/c)$'+'\n a={0:4.2f} b={1:4.2f} c={2:4.2f}'.format(*results_qn[0]))

p=plt.plot(th,(cth_200k/cth_200k[0]),'o', label='200k')
plt.plot(thth, fct(thth, *results_200k[0]), color=p[0].get_color(),
     label=r'$a\times\sin(\theta/b)\times\exp(\theta/c)$'+'\n a={0:4.2f} b={1:4.2f} c={2:4.2f}'.format(*results_200k[0]))

plt.plot(th, cth/cth[0], 'ro', label='Simulated')
plt.legend()
plt.xlabel(r'$\theta$ [deg]')
plt.ylabel(r'$C(\theta)$ [Normalized]')
plt.ylim(-0.1,1)

```

OK so we find a good approximation of the End-To-End spatial correlation. However there remain two points to be fully investigated (but that are not a limitation currently):
- Why this 4.5 factor renormalization ?
- The C(theta) from the simulation seems to be stretched slightly towards lower theta with respect to what it should be...


# Simulating Sub-Bands correlations
Here we need to construct the subband-subband correlations that were found by Louise form spectro-imaging End-To-End simulations. These simualtions can be found in Lyon in /sps/qubic/Data/Sims/Spectro-Imaging-Louise/|

```{python}
### Directory where are the files
#global_dir = Qubic_DataDir(datafile='instrument.py', datadir=os.environ['QUBIC_DATADIR'])
#dirmaps = global_dir + '/doc/FastSimulator/Data/Spectro-Imaging-Louise/'
directory = '/home/lkardum/qubic/qubic/doc/FastSimulator/Spectro-Imaging-Louise/'
```

### koja je razlika izmedju 15 i 16, zasto se zove nfsub kad je to broj frekvencija na kojima mjerimo

```{python}
glob.glob(directory+'residualspatch_*_nfrecon1.pkl')[0]
```

```{python}
import glob
def get_maps_from_louise(directory, nfsub):
    mappatchfile = glob.glob(directory+'residualspatch_*15_nfrecon{}.pkl'.format(nfsub))[0]
    covfile = glob.glob(directory+'coverage_*15_nfrecon{}.pkl'.format(nfsub))[0]
    seenfile = glob.glob(directory+'seenmap_*15_nfrecon{}.pkl'.format(nfsub))[0]
    residualspatch = pickle.load( open( mappatchfile, "rb" ) )
    seenpix = pickle.load( open( seenfile, "rb" ) )
    covpix = pickle.load( open( covfile, "rb" ) )
    
    nside = 256
    residuals_map = np.zeros((nfsub, 12*256**2, 3))
    for i in range(nfsub):
        residuals_map[i,seenpix,:] = residualspatch[i][:,:]
    
    return residuals_map, covpix
```

```{python}
nfsub = 3
maps, cov = get_maps_from_louise(directory,nfsub)

hp.mollview(cov, title='Coverage nfsub={}'.format(nfsub))
plt.figure()
stn = ['I','Q','U']
for i in range(nfsub):
    for s in range(3):
        hp.mollview(maps[i,:,s], sub=(nfsub,3,i*3+s+1), title='Residuals {} {}/{}'.format(stn[s],i+1,nfsub))
plt.tight_layout()
```

The first step is once more to fit the noise profile:

```{python}
all_nfsub = [1,3,5]
est_gp = SymbolicRegressor()

for i in range(len(all_nfsub)):
    plt.figure()
    nfsub = all_nfsub[i]
    maps, cov = get_maps_from_louise(directory,nfsub)
    poly_model = lambda x, a, b, c, d, e, f, g: (
                    a + b * x + f * x ** 2 + g * x ** 3 + c * np.exp(-d * (x - e)))

    for isub in range(nfsub):
        
        sqn = int(np.sqrt(nfsub))
        if (sqn**2) != nfsub:
            sqn += 1
        plt.subplot(sqn,sqn,isub+1)

        xx, rms_tot, rms_I, rms_QU, fitcov, _, fitted_params_I, fitted_params_QU = qss.get_noise_invcov_profile(maps[isub,:,:], cov, QUsep=True, nbins=50,
                                                     fit=True, norm=False,
                                                     allstokes=True)
        plt.plot(xx ** 2, rms_I, 'o', label = 'rms I') 
        plt.plot(xx ** 2, rms_QU, 'x', label = 'rms QU') 
        plt.plot(xx ** 2, poly_model(xx ** 2, *fitted_params_I[0]), label=' Fit I', color = 'blue')
        plt.plot(xx ** 2, poly_model(xx ** 2, *fitted_params_QU[0]), label=' Fit QU / sqrt(2)', color = 'orange')
        plt.xlim(0.9,10.1)
        plt.ylabel('Noise RMS')
        plt.xlabel(r'$ \sqrt{coverage^{-1}}$')
        plt.title('nfsub={}'.format(nfsub))
        plt.legend(fontsize=8)
        plt.tight_layout()
        
        #symbolic reg
        reshaped_xxsq = np.array(xx**2).reshape(-1,1) 
        est_gp.fit(reshaped_xxsq, rms_I)
        fitted_expression = est_gp._program
        print('Noise - inverse coverage I Symbolic reg expression')
        print(fitted_expression)
        
        est_gp.fit(reshaped_xxsq, rms_QU)
        fitted_expression = est_gp._program
        print('Noise - inverse coverage QU Symbolic reg expression')
        print(fitted_expression)


```

nfsub = 1
Polynomial Degree I: 1 (Linear)
Polynomial Degree QU: 1 (Linear)
nfsub = 3
Polynomial Degree I: 5
Polynomial Degree QU: 3
Polynomial Degree I: 7
Polynomial Degree QU: 5
Polynomial Degree I: 4
Polynomial Degree QU: 5
nfsub = 5
Polynomial Degree I: 4
Polynomial Degree QU: 3
Polynomial Degree I: 4
Polynomial Degree QU: 6
Polynomial Degree I: 3
Polynomial Degree QU: 2 (Quadratic)
Polynomial Degree I: 1 (Linear)
Polynomial Degree QU: 2 (Quadratic)
Polynomial Degree I: 1 (Linear)
Polynomial Degree QU: 3


Malo cudno, amo istrazit jel noise opisan drugacijim funkcijama...

```{python}
from gplearn.genetic import SymbolicRegressor

```

```{python}
est_gp = SymbolicRegressor()
reshaped_xxsq = np.array(xx**2).reshape(-1,1) 
est_gp.fit(reshaped_xxsq, rms_I)
```

```{python}
fitted_expression = est_gp._program
print(str(fitted_expression))
```

```{python}

```

```{python}

```

```{python}

```

Now we can calculate to nunu covariance matrix for each case

```{python}
for i in range(len(all_nfsub)):
    nfsub = all_nfsub[i]
    maps, cov = get_maps_from_louise(directory,nfsub)
    cI, cQ, cU, fitcov, noise_norm = qss.get_cov_nunu(maps, cov, QUsep=True)
    valmini = np.min(np.array([cI, cQ/2, cU/2]))
    valmaxi = np.max(np.array([cI, cQ/2, cU/2]))
    mini = -np.max([np.abs(valmini), np.abs(valmaxi)])
    maxi = np.max([np.abs(valmini), np.abs(valmaxi)])
    cvars = [cI, cQ, cU]
    plt.figure()
    for i in range(len(cvars)):
        plt.subplot(1,3,i+1)
        plt.imshow(cvars[i], vmin=mini, vmax=maxi, cmap='bwr')
        plt.xticks(range(nfsub))
        plt.yticks(range(nfsub))
        plt.xlabel('Sub-Frequency #')
        plt.ylabel('Sub-Frequency #')
        plt.colorbar(orientation='horizontal')
        plt.title('Cov I nsub={}'.format(nfsub))
plt.tight_layout()
```

We can equivalently plot the correlation matrices

```{python}
for i in range(len(all_nfsub)):
    nfsub = all_nfsub[i]
    maps, cov = get_maps_from_louise(directory,nfsub)
    cI, cQ, cU, fitcov, noise_norm = qss.get_cov_nunu(maps, cov, QUsep=True)
    corrI = qss.cov2corr(cI)
    corrQ = qss.cov2corr(cQ)
    corrU = qss.cov2corr(cU)
    mini = -1#np.min(np.array([corrI, corrQ, corrU]))
    maxi = 1.#np.max(np.array([corrI, corrQ, corrU]))
    corrvars = [corrI, corrQ, corrU]
    plt.figure()
    for i in range(len(corrvars)):
        plt.subplot(1,3,i+1)
        plt.imshow(corrvars[i], vmin=mini, vmax=maxi, cmap='bwr')
        plt.xticks(range(nfsub))
        plt.yticks(range(nfsub))
        plt.xlabel('Sub-Frequency #')
        plt.ylabel('Sub-Frequency #')
        plt.colorbar(orientation='horizontal')
        plt.title('Correlation I nsub={}'.format(nfsub))
plt.tight_layout()

```

Let's now generate nosie maps following this statistics

First in the case of **NO Correlation** beween sub-bands (sub_bands_cov=None in Qubic_sky.create_noise_maps())

```{python}
seed = None
sky_config = {'cmb': seed}
Qubic_sky = qss.Qubic_sky(sky_config, d)
               
i=len(all_nfsub)
nfsub = all_nfsub[i-1]
maps, cov = get_maps_from_louise(directory,nfsub)
cI, cQ, cU, fitcov, noise_norm = qss.get_cov_nunu(maps, cov, QUsep=True)
corr_mats = [cI, cQ, cU]

myfitcovs = []
for isub in range(nfsub):
    xx, yyfs, _, _, fitcov, _, _, _ = qss.get_noise_invcov_profile(maps[isub,:,:], cov, QUsep=True,
                                                 fit=True, norm=False, allstokes=True)
    myfitcovs.append(fitcov)
    
new_sub_maps = Qubic_sky.create_noise_maps(signoise, cov, nsub=nfsub,
                                            effective_variance_invcov=myfitcovs,
                                            clnoise=None, sub_bands_cov=None, verbose=False)
print(np.shape(new_sub_maps))
if nfsub == 1:
    new_sub_maps = np.reshape(new_sub_maps, (1,12*d['nside']**2,3))
print(np.shape(new_sub_maps))

for isub in range(nfsub):
    sqn = int(np.sqrt(nfsub))
    if (sqn**2) != nfsub:
        sqn += 1
    plt.subplot(sqn,sqn,isub+1)
    xx, yyfs, _, _, fitcov, _, _, _ = qss.get_noise_invcov_profile(new_sub_maps[isub,:,:], cov, QUsep=True,
                                                 fit=False, norm=False, allstokes=False)
    plt.plot(myfitcovs[isub][0,:], np.sqrt(myfitcovs[isub][1,:]), label='End-To-End Fit I sub={}/{}'.format(isub+1,nfsub))
    plt.plot(myfitcovs[isub][0,:], np.sqrt(myfitcovs[isub][2,:]), label='End-To-End Fit QU sub={}/{}'.format(isub+1,nfsub))
    plt.legend(fontsize=9)
    plt.xlim(0.9,10.1)
    plt.ylim(np.min(yyfs)/1.5,np.max(yyfs)*1.5)
plt.tight_layout()
    
cInew, cQnew, cUnew, fitcov, noise_norm = qss.get_cov_nunu(new_sub_maps, cov, QUsep=True)
new_corr_mats = [cInew, cQnew, cUnew]

plt.figure()
stn = ['I','Q', 'U']
mini = -1#np.min(np.array([corr_mats, new_corr_mats]))
maxi = 1.#np.max(np.array([corr_mats, new_corr_mats]))
for s in range(3):
    plt.subplot(2,3,1+s)
    plt.imshow(corr_mats[s], vmin=mini, vmax=maxi, cmap='bwr')
    plt.colorbar(orientation='horizontal')
    plt.title('End-To-End Cov {} nsub={}'.format(stn[s],nfsub))
    plt.subplot(2,3,4+s)
    plt.imshow(new_corr_mats[s], vmin=mini, vmax=maxi)
    plt.colorbar(orientation='horizontal')
    plt.title('FastSim Cov {} nsub={}'.format(stn[s],nfsub))
```

OK it shows zero correlation ! and the profiles are in good agreement !

Now th real case with correlation from End-To-End simulations. Here we use clnoise=None because we want to see the matching in  map space.

```{python}
reload(qss)
seed = None
sky_config = {'cmb': seed}
Qubic_sky = qss.Qubic_sky(sky_config, d)

               

i=4
nbins = 30
nfsub = all_nfsub[i]
maps, cov = get_maps_from_louise(dirmaps,nfsub)
cI, cQ, cU, fitcov, noise_norm = qss.get_cov_nunu(maps, cov, QUsep=True)
corr_mats = [cI, cQ/2, cU/2]


signoise = 103.
print(signoise)

myfitcovs = []
for isub in range(nfsub):
    xx, yyfs, fitcov = qss.get_noise_invcov_profile(maps[isub,:,:], cov, QUsep=True, nbins=nbins,
                                                 label='Input Map {}'.format(i+1), fit=True, norm=False,
                                                 allstokes=True, doplot=False)
    myfitcovs.append(fitcov)
    
new_sub_maps = Qubic_sky.create_noise_maps(signoise, cov, nsub=nfsub,
                                            effective_variance_invcov=myfitcovs,
                                            clnoise=None, sub_bands_cov=corr_mats, verbose=False)

print(np.shape(new_sub_maps))
if nfsub == 1:
    new_sub_maps = np.reshape(new_sub_maps, (1,12*d['nside']**2,3))
print(np.shape(new_sub_maps))

for isub in range(nfsub):
    sqn = int(np.sqrt(nfsub))
    if (sqn**2) != nfsub:
        sqn += 1
    subplot(sqn,sqn,isub+1)
    xx, yyfs, fitcov = qss.get_noise_invcov_profile(new_sub_maps[isub,:,:], cov, QUsep=True, nbins=nbins,
                                                 label='FastSim sub={}/{}'.format(isub+1,nfsub), fit=False, norm=False,
                                                 allstokes=False, doplot=True)
    plot(myfitcovs[isub][0,:], np.sqrt(myfitcovs[isub][1,:]), label='End-To-End Fit I sub={}/{}'.format(isub+1,nfsub))
    plot(myfitcovs[isub][0,:], np.sqrt(myfitcovs[isub][2,:]), label='End-To-End Fit QU sub={}/{}'.format(isub+1,nfsub))
    legend(fontsize=9)
    xlim(0.9,10.1)
    ylim(np.min(yyfs)/1.5,np.max(yyfs)*1.5)
tight_layout()

cInew, cQnew, cUnew, fitcov, noise_norm = qss.get_cov_nunu(new_sub_maps, cov, QUsep=True)
new_corr_mats = [cInew, cQnew/2, cUnew/2]

figure()
stn = ['I','Q/2', 'U/2']
mini = -1.#np.min(np.array([corr_mats, new_corr_mats]))
maxi = 1.#np.max(np.array([corr_mats, new_corr_mats]))
for s in range(3):
    subplot(2,3,1+s)
    imshow(corr_mats[s], vmin=mini, vmax=maxi, cmap='bwr')
    colorbar(orientation='horizontal')
    title('End-To-End Cov {} nsub={}'.format(stn[s],nfsub))
    subplot(2,3,4+s)
    imshow(new_corr_mats[s], vmin=mini, vmax=maxi, cmap='bwr')
    colorbar(orientation='horizontal')
    title('FastSim Cov {} nsub={}'.format(stn[s],nfsub))

```

Let's now check that C(theta) is zero as expected (we have put clnoise=None)

```{python}
### Get the C(theta)
allcth = []
pixgood = (cov/np.max(cov)) > 0.1 
for i in range(nfsub):
    corrected_qubicnoise = qss.correct_maps_rms(new_sub_maps[i,:,:], cov, fit_n200k)
    th, thecth, err = qss.ctheta_parts(corrected_qubicnoise[:,0], pixgood, 0, 20, 20, nsplit=5, degrade_init=128, verbose=False)
    allcth.append(thecth)
    print(i)
```

```{python}
for i in range(nfsub):
    plot(th, allcth[i]/allcth[i][0],'o', label='Fast-Simulation Sub {}'.format(i+1))
plot(th, cth_200k/cth_200k[0],label='From End-To-End simulations')
plot(thth, fct(thth, *results_200k[0]),label='Model from End-To-End Simulations')
legend()
xlabel(r'$\theta$ [deg]')
ylabel(r'$C(\theta$)')
```

Now th real case with correlation from End-To-End simulations and with spatial correlation ON (clnoise=clth). We should not expect a match of the maps RMS.

```{python}
reload(qss)
seed = None
sky_config = {'cmb': seed}
Qubic_sky = qss.Qubic_sky(sky_config, d)

               

i=4
nbins = 30
nfsub = all_nfsub[i]
maps, cov = get_maps_from_louise(dirmaps,nfsub)
cI, cQ, cU, fitcov, noise_norm = qss.get_cov_nunu(maps, cov, QUsep=True)
corr_mats = [cI, cQ/2, cU/2]


signoise = 192.
print(signoise)

myfitcovs = []
for isub in range(nfsub):
    xx, yyfs, fitcov = qss.get_noise_invcov_profile(maps[isub,:,:], cov, QUsep=True, nbins=nbins,
                                                 label='Input Map {}'.format(i+1), fit=True, norm=False,
                                                 allstokes=True, doplot=False)
    myfitcovs.append(fitcov)
    
new_sub_maps = Qubic_sky.create_noise_maps(signoise, cov, nsub=nfsub,
                                            effective_variance_invcov=myfitcovs,
                                            clnoise=clth, sub_bands_cov=[cI, cQ, cU], verbose=False)

print(np.shape(new_sub_maps))
if nfsub == 1:
    new_sub_maps = np.reshape(new_sub_maps, (1,12*d['nside']**2,3))
print(np.shape(new_sub_maps))

for isub in range(nfsub):
    sqn = int(np.sqrt(nfsub))
    if (sqn**2) != nfsub:
        sqn += 1
    subplot(sqn,sqn,isub+1)
    xx, yyfs, fitcov = qss.get_noise_invcov_profile(new_sub_maps[isub,:,:], cov, QUsep=True, nbins=nbins,
                                                 label='FastSim sub={}/{}'.format(isub+1,nfsub), fit=False, norm=False,
                                                 allstokes=False, doplot=True)
    plot(myfitcovs[isub][0,:], np.sqrt(myfitcovs[isub][1,:]), label='End-To-End Fit I sub={}/{}'.format(isub+1,nfsub))
    plot(myfitcovs[isub][0,:], np.sqrt(myfitcovs[isub][2,:]), label='End-To-End Fit QU sub={}/{}'.format(isub+1,nfsub))
    legend(fontsize=9)
    xlim(0.9,10.1)
    ylim(np.min(yyfs)/1.5,np.max(yyfs)*1.5)
tight_layout()

cInew, cQnew, cUnew, fitcov, noise_norm = qss.get_cov_nunu(new_sub_maps, cov, QUsep=True)
new_corr_mats = [cInew, cQnew/2, cUnew/2]

figure()
stn = ['I','Q/2', 'U/2']
mini = -1.#np.min(np.array([corr_mats, new_corr_mats]))
maxi = 1.#np.max(np.array([corr_mats, new_corr_mats]))
for s in range(3):
    subplot(2,3,1+s)
    imshow(corr_mats[s], vmin=mini, vmax=maxi, cmap='bwr')
    colorbar(orientation='horizontal')
    title('End-To-End Cov {} nsub={}'.format(stn[s],nfsub))
    subplot(2,3,4+s)
    imshow(new_corr_mats[s], vmin=mini, vmax=maxi, cmap='bwr')
    colorbar(orientation='horizontal')
    title('FastSim Cov {} nsub={}'.format(stn[s],nfsub))


```

Let's now check that C(theta) is zero as expected (we have put clnoise=None)

```{python}
### Get the C(theta)
allcth = []
pixgood = (cov/np.max(cov)) > 0.1 
for i in range(nfsub):
    corrected_qubicnoise = qss.correct_maps_rms(new_sub_maps[i,:,:], cov, fit_n200k)
    th, thecth, err = qss.ctheta_parts(corrected_qubicnoise[:,0], pixgood, 0, 20, 20, nsplit=5, degrade_init=128, verbose=False)
    allcth.append(thecth)
    print(i)
```

```{python}
for i in range(nfsub):
    plot(th, allcth[i]/allcth[i][0],'o', label='Fast-Simulation Sub {}'.format(i+1))
plot(th, cth_200k/cth_200k[0],label='From End-To-End simulations')
plot(thth, fct(thth, *results_200k[0]),label='Model from End-To-End Simulations')
legend()
xlabel(r'$\theta$ [deg]')
ylabel(r'$C(\theta$)')
```

### Now we can save  the data needed to perform these FastSimualtions so that they can be directly used in qss.get_partial_sky_map_withnoise()

```{python}
### We replace the 88. (from Louise's sims) by 75 which has been found with 
### Nersc 2000000 pointings simualations (much more realistic) for 3 years observations with FI

signoise = 75.
nbins = 30

for nfsub in all_nfsub:
    print('Doing Nfsub = {}'.format(nfsub))
    maps, cov = get_maps_from_louise(dirmaps,nfsub)
    cI, cQ, cU, fitcov, noise_norm = qss.get_cov_nunu(maps, cov, QUsep=True)
    corr_mats = [cI, cQ/2, cU/2]

    myfitcovs = []
    for isub in range(nfsub):
        xx, yyfs, fitcov = qss.get_noise_invcov_profile(maps[isub,:,:], cov, QUsep=True, nbins=nbins,
                                                         label='Input Map {}'.format(i+1), fit=True, norm=False,
                                                     allstokes=True, doplot=False)
        myfitcovs.append(fitcov)
    

    #### Save informations for running simulations in Lyon:
#     data = {'nfsub':nfsub, 'coverage':cov, 'CovI':cI, 'CovQ':cQ, 'CovU':cU, 'signoise':signoise,
#             'effective_variance_invcov':myfitcovs, 'clnoise':clth, 'years':3.}
#     name = 'DataFastSimulator_FI_Duration_3_nfsub_{}.pkl'.format(nfsub)
#     pickle.dump(data , open( global_dir + "doc/FastSimulator/Data/"+name, "wb" ) )

print(signoise)
```

```{python}
#### Debugging
# DataFastSimNew = pickle.load( open( global_dir +'/doc/FastSimulator/Data/DataFastSimulator_FI-150_nfsub_1.pkl', "rb" ) )
# DataFastSimOld = pickle.load( open( global_dir +'/doc/FastSimulator/Data/DataFastSimulator_FI_Duration_3_nfsub_1.pkl', "rb" ) )

# print('New',DataFastSimNew['nfsub'])
# print('Old',DataFastSimOld['nfsub'])
# print()

# print('New',DataFastSimNew['coverage'])
# print('Old',DataFastSimOld['coverage'])
# print()

# print('New',DataFastSimNew['CovI'])
# print('Old',DataFastSimOld['CovI'])
# print()

# print('New',DataFastSimNew['CovQ'])
# print('Old',DataFastSimOld['CovQ'])
# print()

# print('New',DataFastSimNew['CovU'])
# print('Old',DataFastSimOld['CovU'])
# print()

# print('New',DataFastSimNew['signoise'])
# print('Old',DataFastSimOld['signoise'])
# print()

# print('New',DataFastSimNew['clnoise'])
# print('Old',DataFastSimOld['clnoise'])
# print()

# figure()
# subplot(1,2,1)
# plot(DataFastSimNew['clnoise'], label='New')
# plot(DataFastSimOld['clnoise'], label='Old')
# legend()

# subplot(1,2,2)
# fitcov_new = DataFastSimNew['effective_variance_invcov'][0]
# fitcov_old = DataFastSimOld['effective_variance_invcov'][0]
# p1 = plot(fitcov_new[0,:], fitcov_new[1,:], label='Fit I New')
# p2 = plot(fitcov_new[0,:], fitcov_new[2,:], label='Fit QU New')
# plot(fitcov_new[0,:], fitcov_new[1,:], label='Fit I Old', ls='--')
# plot(fitcov_new[0,:], fitcov_new[2,:], label='Fit QU Old', ls='--')
# legend()

```

```{python}

```

```{python}

```

```{python}

```

```{python}

```
