...
Qubic_sky, x0, maps_observed = make_maps(d, seenpix, noise_level=0.2)
...
```{python}
Qubic_sky1, _, maps_observed1 = make_maps(d, seenpix, seed=42, noise_level=0.2)
Qubic_sky2, _, maps_observed2 = make_maps(d, seenpix, seed=42, noise_level=0.2)
input_Dl = Qubic_sky1.input_cmb_spectra

# Make an 2D array containing IQU maps, the shape must be (3, #pixels)
maps1 = maps_observed1[0].T
maps2 = maps_observed2[0].T

# Get cross-spectra
leff, cells_cross, w = Namaster.get_spectra(maps1, 
                                      map2=maps2,
                                      purify_e=True, 
                                      purify_b=False, 
                                      beam_correction=None,
...
i = 2
plt.plot(input_ell[lmin:lmax], input_Dl[lmin:lmax, i], label='Input spectrum')
plt.plot(leff[:lmax], cells[:lmax, i],'ro', label='Auto-spectrum')
plt.plot(leff[:lmax], cells_cross[:lmax, i],'bo', label='Cross-spectrum')
...
### Compare pure or not pure 

```{python}
# Get spectra
leff, cells_pureB, w = Namaster.get_spectra(maps, 
                                      purify_e=False, 
                                      purify_b=True, 
                                      beam_correction=None,
                                      pixwin_correction=None)
leff, cells_notpureB, w = Namaster.get_spectra(maps, 
                                      purify_e=True, 
                                      purify_b=False, 
                                      beam_correction=None,
                                      pixwin_correction=None)
...
for i in range(4):
    plt.subplot(2, 2, i+1)
    plt.plot(input_ell[:lmax], input_Dl[:lmax, i])
    plt.plot(leff[:lmax], cells_pureB[:lmax, i],'o', label='B pure')
    plt.plot(leff[:lmax], cells_notpureB[:lmax, i],'o', label='B not pure')
    plt.xlabel('$\\ell$')
    plt.ylabel('$D_\\ell$')
    plt.title(clnames[i])
    plt.legend()
...
i = 0
plt.figure(figsize=(8, 6))
plt.plot(input_ell[lmin:lmax][::5], input_Dl[lmin:lmax, i][::5], label='Input spectrum')
plt.plot(leff_nobeam[:lmax], cells_nobeam[:lmax, i], 'mo', label='No beam correction')
plt.plot(leff_beam[:lmax], cells_beam[:lmax, i], 'go', label='With beam correction')
plt.xlabel('$\\ell$', fontsize=18)
plt.ylabel(f'$D_\\ell$', fontsize=18)
plt.title(clnames[i])
plt.legend()
plt.tight_layout()

plt.savefig(f'/home/lmousset/QUBIC/These_manuscrit/Figures/Plots/Namaster_{clnames[i]}.pdf')
#tkz.save(f'/home/lmousset/QUBIC/These_manuscrit/tikz/Namaster_{clnames[i]}.tex')
```
...
plt.figure(figsize=(8, 6))
plt.plot(input_ell[lmin:lmax][::5], input_Dl[lmin:lmax, i][::5], label='Input spectrum')
plt.plot(leff_nopw[:lmax], cells_nopw[:lmax, i], 'mo', label='No pixwin')
plt.plot(leff_pw[:lmax], cells_pw[:lmax, i], 'go', label='With pixwin')
plt.xlabel('$\\ell$', fontsize=18)
plt.ylabel(f'$D_\\ell$', fontsize=18)
plt.title(clnames[i])
plt.legend()
...
plt.savefig(f'/home/lmousset/QUBIC/These_manuscrit/Figures/Plots/Namaster_{clnames[i]}_pixwin.pdf')
#tkz.save(f'/home/lmousset/QUBIC/These_manuscrit/tikz/Namaster_{clnames[i]}_pixwin.tex')
```
...
for sim in range(nsim):
    ...
    input_Dl = Qubic_sky.input_cmb_spectra
    maps = maps_observed[0].T
    maps = hp.sphtfunc.smoothing(maps, fwhm=np.deg2rad(d['synthbeam_peak150_fwhm']))

    leff, cells, w = Namaster.get_spectra(maps, purify_e=True, purify_b=False, 
                                          beam_correction=True, pixwin_correction=False)
    _, cells_nobeam, w = Namaster.get_spectra(maps, purify_e=True, purify_b=False, 
                                          beam_correction=False, pixwin_correction=False)
    nsim_cells.append(cells)
    nsim_cells_nobeam.append(cells_nobeam)
nsim_cells = np.array(nsim_cells)
nsim_cells_nobeam = np.array(nsim_cells_nobeam)
...
mean_cells_nobeam = np.mean(nsim_cells_nobeam, axis=0)
std_cells_nobeam = np.std(nsim_cells_nobeam, axis=0)
...
for i in range(4):
    ...
    plt.errorbar(leff[:lmax], mean_cells_nobeam[:lmax, i], std_cells_nobeam[:, i], fmt='g.', label='No beam')
...
nsim_cells_cross = []
for sim in range(nsim):
    Qubic_sky1, _, maps_observed1 = make_maps(d, seenpix, seed=42, noise_level=0.25)
    Qubic_sky2, _, maps_observed2 = make_maps(d, seenpix, seed=42, noise_level=0.25)
    # Make an 2D array containing IQU maps, the shape must be (3, #pixels)
    maps1 = maps_observed1[0].T
    maps2 = maps_observed2[0].T
    input_Dl = Qubic_sky.input_cmb_spectra
   
    leff, cells1, w = Namaster.get_spectra(maps1, purify_e=True, purify_b=False, 
                                          beam_correction=None, pixwin_correction=None)
    leff, cells2, w = Namaster.get_spectra(maps2, purify_e=True, purify_b=False, 
                                          beam_correction=None, pixwin_correction=None)
    _, cells_cross, w = Namaster.get_spectra(maps1, map2=maps2, purify_e=True, purify_b=False, 
                                          beam_correction=None, pixwin_correction=None)
    nsim_cells1.append(cells1)
    nsim_cells2.append(cells2)
    nsim_cells_cross.append(cells_cross)
nsim_cells1 = np.array(nsim_cells1)
nsim_cells2 = np.array(nsim_cells2)
nsim_cells_cross = np.array(nsim_cells_cross)
print(nsim_cells1.shape)
```

```{python}
nsim_cells = np.concatenate((nsim_cells1, nsim_cells2))
print(nsim_cells.shape)
```
...
mean_cells = np.mean(nsim_cells, axis=0)
std_cells = np.std(nsim_cells, axis=0)
mean_cells_cross = np.mean(nsim_cells_cross, axis=0)
std_cells_cross = np.std(nsim_cells_cross, axis=0)
print(mean_cells.shape)
...
i = 2
plt.plot(input_ell[lmin:lmax], input_Dl[lmin:lmax, i], label='Input spectrum')
plt.errorbar(leff[:lmax], mean_cells[:lmax, i], yerr=std_cells[:lmax, i], fmt='ro', label='Auto-spectrum')
plt.errorbar(leff[:lmax], mean_cells_cross[:lmax, i], yerr=std_cells_cross[:lmax, i]*sqrt(2), fmt='bo', 
             label='Cross-spectrum')
plt.xlabel(r'$\ell$', fontsize=18)
plt.ylabel(r'$\langle D_\ell \rangle_r $', fontsize=18)
plt.xticks(np.arange(0., 600, 100))
#plt.yticks(np.arange(0., 0.13, 0.02))
plt.legend()
plt.title(clnames[i])

plt.savefig(save_dir + f'Namaster_{clnames[i]}_auto-cross.pdf')
#tkz.save(save_dir + f'Namaster_{clnames[i]}_auto-cross.tex')
```
...
i = 2
for r in range(50):
    plt.plot(leff[:lmax], nsim_cells[r, :lmax, i], 'o', label='Auto-spectrum')
    plt.plot(leff[:lmax], nsim_cells_cross[r, :lmax, i], 'o', label='Cross-spectrum')
plt.plot(input_ell[lmin:lmax], input_Dl[lmin:lmax, i], label='Input spectrum')
