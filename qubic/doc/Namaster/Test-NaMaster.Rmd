...
```{python}
lmax = 3 * d['nside'] - 1
Namaster = nam.Namaster(msk, lmin=20, lmax=lmax, delta_ell=5)
```

```{python}
themaps = np.array((maps_observed[0, :, 0], maps_observed[0, :, 1], maps_observed[0, :, 2]))

leff, cells, w = Namaster.get_spectra(themaps, d, purify_e=True, purify_b=True)
```
