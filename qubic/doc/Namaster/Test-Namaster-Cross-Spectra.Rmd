...
leff, cl_nonoise, wnnonoise = Namaster.get_spectra(input_maps, mask_apo, 
...
for i in range(nmaps):
    for j in range(i,nmaps):
        print(i,j)
        leff, cls[i,j, :,:], w = Namaster.get_spectra(all_maps[i,:,:].T, mask_apo, 
                                                      map2=all_maps[j,:,:].T,
...
for i in range(nbmc):
    ...
    leff, cls_noise[i,:,:], w = Namaster.get_spectra(qubicnoise.T, mask_apo, 
                                            purify_e=False, purify_b=True, 
                                            verbose=False, w=w,
                                            beam_correction=Qubic_sky.instrument['beams'])
...
for n in arange(nbmc):
    ...
    for i in range(nmaps):
        for j in range(i,nmaps):
            ...
            leff, cls, w = Namaster.get_spectra(all_maps[i,:,:].T, mask_apo, 
                                                map2 = all_maps[j,:,:].T,
                                            purify_e=False, purify_b=True, 
                                            verbose=False, w=w,
                                            beam_correction=Qubic_sky.instrument['beams'])
...
```{python}
a=hist(scls_auto[:,s]/scls_cross[:,s], range=[1,2], bins=15, 
