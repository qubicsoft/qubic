...
for i in range(4):
    ...
    text(np.min(xTES)+dx, thermos_xy[i]+0.6*dy, 'QS:   -', color=qs_color, fontsize=12)
    text(np.min(xTES)+dx, thermos_xy[i]-2.6*dy, 'QP: {0:3d}'.format(32*i+4), color=qp_color, fontsize=12)
...
for i in range(4):
    ...
    text(thermos_xy[::-1][i]+dx, np.min(xTES)+0.6*dy, 'QS:   -', color=qs_color, fontsize=12)
    text(thermos_xy[::-1][i]+dx, np.min(xTES)-2.6*dy, 'QP: {0:3d}'.format(32*i+4+128), color=qp_color, fontsize=12)
