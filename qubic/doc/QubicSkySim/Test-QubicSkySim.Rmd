...
def I(f,T):
    return 2*h*f**3/(c**2*(exp(h*f/(k*T))-1))

x=linspace(10,1000,100)
y_cmb=I(x*10**9,3)
y_dust=x**1.5*I(x*10**9,20)
...
for i in range(len(nus)):
    NETs[i] = ana.NoiseEquivalentTemperature(NEPs[i], nus[i]).NETs

depths_FMM = ana.AnalyticalForecast(nus, NEP_det, NEP_pho)._get_effective_depths(NETs)
```
