...
ax.plot(th_deg, sb2, 'c', label='r = 12mm',alpha=0.6,lw=2)
...
ax.annotate(r"FWHM($\nu_i$)",xytext=(-7.5,0.6),xy=(0,0.5),
            arrowprops=dict(arrowstyle="fancy",fc="b", ec="b"),bbox=dict(boxstyle="round", fc="w"),
            fontsize=15 )
ax.annotate(r"$\theta(\nu_i)$",xy=(8.3,0.33),xytext=(-1.1,0.33),ha="center",va="center",
...
for j,inu in enumerate(NUNU):
    d['filter_nu'] = NUNU[j]
    q = qubic.QubicInstrument(d)
    beam=q.primary_beam(np.radians(np.abs(th_deg)), ph)
    sum_b[j]= beam*sb_noprim(th_deg, NUNU[j], 0)
    #_, sum_b[j] = beam_total(th_deg, ph, inu*1e9, 0)
sum_b_15 = np.zeros((15,len(th_deg),) )
NUNU15=np.linspace(131e9,169e9,15)
for j,inu in enumerate(NUNU15):
    d['filter_nu'] = inu
    q = qubic.QubicInstrument(d)
    beam=q.primary_beam(np.radians(np.abs(th_deg)), ph)
    sum_b_15[j]= beam*sb_noprim(th_deg, inu, 0)
...
for i in range(2):
    d['filter_nu'] = freqs[i]
    q = qubic.QubicInstrument(d)
    allbeams[i, :] = q.primary_beam(np.radians(np.abs(th_deg)), ph)
#e2, sb2 = beam_total(th_deg, ph, nu, 0)
#e1, sb1 = beam_total(th_deg, ph, nu=140e9, rx=0)
#e2, sb2 = beam_total(th_deg, ph, nu=160e9, rx=0)

fig, ax = plt.subplots(nrows=1,ncols=2,figsize=(13,6),gridspec_kw={'wspace':0.03},sharey=True)
plt.rc('font',size=15)
ax=ax.ravel()
ax[0].plot(th_deg, allbeams[0],'b--', alpha=0.6, label=r'$140~\mathrm{GHz}$')
ax[0].plot(th_deg, allbeams[1],'g--', alpha=0.6, label=r'$160~\mathrm{GHz}$')
ax[0].plot(th_deg, allbeams[0]*sb_noprim(th_deg, freqs[0],0), 'b', label=None, lw=2)
ax[0].plot(th_deg, allbeams[1]*sb_noprim(th_deg, freqs[1],0), 'g', label=None, lw=2)
ax[0].grid()
...
sumleg=ax[1].plot(th_deg, np.sum(sum_b,axis=0)/max(np.sum(sum_b,axis=0)), 'k', lw=2)
#sumleg15=ax[1].plot(th_deg, np.sum(sum_b_15,axis=0)/max(np.sum(sum_b_15,axis=0)), 'k', lw=2)
blo=[]
for i in range(0,len(NUNU),2):
    blo+=ax[1].plot(th_deg, sum_b[i]/max(np.sum(sum_b,axis=0)), c=next(color), )
...
leg = Legend(ax[1], blo[:], [r'MonoSB @$130~$GHz','\t \t $140~$GHz',
                          '\t \t $150~$GHz','\t \t $160~$GHz','\t \t $170~$GHz'],
