...
open_horns=list(np.arange(1, 3)) # between 1 and 64 (real instrument numbers)
...
hp.gnomview(np.log10(sb_aber / np.max(sb_aber)), rot=[0,90], reso=15, 
            title='Aber, log10(sb/max(sb))', sub=(324))
hp.gnomview(sb_ideal, rot=[0,90], reso=10, title='Ideal', sub=(325))
hp.gnomview(np.log10(sb_ideal / np.max(sb_ideal)), rot=[0,90], reso=15, 
