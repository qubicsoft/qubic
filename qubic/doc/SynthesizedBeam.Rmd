...
```{python}
# save_dir = '/home/martin/QUBIC/qubic/qubic/scripts/Spectroimagery_paper/'
# save_dir = '/home/lmousset/QUBIC/Qubic_work/SpectroImagerie/paper_plot/'
save_dir = '/home/lmousset/QUBIC/These_manuscrit/tikz/'
```
...
# plt.savefig(save_dir + 'beam_cut_2TESb.pdf', bbox_inches='tight')
# import tikzplotlib as tkz
# tkz.save(save_dir + 'beam_cut_2TESb.tex')
```
...
# plt.savefig(save_dir + 'synth_beam_many_freq.pdf', bbox_inches='tight')

```

#### Same plot but in two figures 

```{python}
th_deg = np.linspace(-20, 20, nn)
fig = plt.figure(figsize=(10, 8))
ax = fig.gca()
ax.plot(th_deg, allbeams[0],'--', c = plt.cm.jet(np.linspace(0,1,6))[::-1][0],
           label=r'${:.1f}~$GHz'.format(freqs[0]/1e9))
ax.plot(th_deg, allbeams[1],'--', c = plt.cm.jet(np.linspace(0,1,6))[::-1][4],
           label=r'${:.1f}~$GHz'.format(freqs[1]/1e9))
ax.plot(th_deg, allbeams[0]*sb_noprim(th_deg, freqs[0],0), 
           c = plt.cm.jet(np.linspace(0,1,6))[::-1][0],
           label=None, lw=2)
ax.plot(th_deg, allbeams[1]*sb_noprim(th_deg, freqs[1],0),
           c = plt.cm.jet(np.linspace(0,1,6))[::-1][4],
           label=None, lw=2)
ax.grid()
ax.legend(loc='best',fontsize=13)
ax.set_xlabel(r'$\theta$ [deg]', fontsize=18)
ax.set_ylabel('Synthesized beam', fontsize=18)
ax.set_xlim(-20, 20)
ax.tick_params(axis='both',bottom=True, top=True, left=True, right=True,direction='in')

ax.plot([14.7, 14.7], [-0.02, 0.2], 'k--')

# Arrow for the first peak
ax.annotate(' ', xy=(7.5,0.33), xytext=(-0.3,0.33), ha="center", va="center",
            arrowprops=dict(arrowstyle="<->",), rotation=0)
ax.text(4, 0.35, r"$\lambda/\Delta$", ha="center", va="center", fontsize=15)

# Arrow for the second peak
ax.tick_params(axis='both',bottom=True, top=True, left=True, right=True,direction='in')
ax.annotate(' ', xy=(14.9,0.15), xytext=(-0.3,0.15), ha="center", va="center",
            arrowprops=dict(arrowstyle="<->",), rotation=0)
ax.text(5, 0.17, r"$2\lambda/\Delta$", ha="center", va="center", fontsize=15)

fig.tight_layout()
plt.savefig(save_dir + 'beam_cut_lambda.pdf')
```
...
sumleg = ax.plot(th_deg, np.sum(sum_b, axis=0), 'k', lw=2)
blo = []
for i in range(0, len(NUNU), 2):
    blo += ax.plot(th_deg, sum_b[i], c=next(color))

ax.set_xlabel(r'$\theta$ [deg]', fontsize=18)
ax.set_ylabel('Arbitrary units', fontsize=18)
ax.legend(sumleg, [r'PolySB'], fontsize=13, loc='upper left', )
ax.set_xlim(-12, 12)

# Create the second legend and add the artist manually.
from matplotlib.legend import Legend
leg = Legend(ax, blo[:], [r'MonoSB @${:.1f}~$GHz'.format(NUNU[0]/1e9),
                             '\t \t ${:.1f}~$GHz'.format(NUNU[2]/1e9),
                             '\t \t ${:.1f}~$GHz'.format(NUNU[4]/1e9),
                             '\t \t ${:.1f}~$GHz'.format(NUNU[6]/1e9),
                            '\t \t ${:.1f}~$GHz'.format(NUNU[8]/1e9)],
...
plt.savefig(save_dir + 'beam_cut_poly.pdf')
```

#### Sum the synthetic beams
