...
#     ax1.set_title('TES {}'.format(index))
    ...
    # Simulation
    hp.gnomview(sb[det], min=0, max=1e6, rot=(0, 90, 180), reso=20, sub=(121), 
                title='', cbar=False)
    hp.graticule()
    ...
    hp.visufunc.projscatter(lon, lat, color='r', marker='+', s=700, lonlat=True, label='Highest pixel')
    plt.legend()
...
ax2.scatter(xONAFP, yONAFP, c=allmain_peaks, marker='s', s=200, alpha=0.8, cmap='jet')
#ax2.set_xlabel('X_ONAFP')
#ax2.set_ylabel('Y_ONAFP')

det0 = 199
x0 = xONAFP[det0]
y0 = yONAFP[det0]
point, = ax2.plot(x0, y0, 'ro')
steps = np.arange(det0, det0+9, 1)

anim = FuncAnimation(fig, animate, steps, init_func=init, interval=1500, blit=True, repeat=False)
