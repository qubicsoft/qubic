...
```{python}
#First test for G, G is not a scalar, need to be changed
invN_d = invN(d)

def gain_operator(d_i, out):
    print(np.dot(d_i.ravel(), invN_d.ravel()))
    out[...] = _dot(d_i, invN_d, invN.commin)

G = Operator(gain_operator, shapein=d.shape, shapeout=d.shape[0] , dtype='float64')
```
...
A_list = []
for freq in frequencies:
    A_list.append(Operator(lambda betas, out, freq=freq : function_A(betas, freq, out), 
...
class Transposed_Jacobian(Operator):
    ...
    def direct(self, input_vector, output):
        ...
        product = c[:self.npixel]*input_vector[:,0] + c[self.npixel:2*self.npixel]*input_vector[:,1] + c[2*self.npixel:3*self.npixel]*input_vector[:,2]
...
```{python}
skycmb = pysm3.Sky(nside=nside, preset_strings=['c1'], output_unit='mK_CMB')
skydust = pysm3.Sky(nside=nside, preset_strings=['d1'], output_unit='mK_CMB')
skycmb = np.array(skycmb.get_emission(frequencies[-1] * u.Hz))
skydust = np.array(skydust.get_emission(frequencies[-1] * u.Hz))
skydust_beta = pysm3.Sky(nside=nside_beta, preset_strings=['d1'], output_unit='mK_CMB')
...
def grad_operator(c, out):
    W = H_list[0] * A_list[0](c)
    for i in range(1, len(frequencies)):
        W = W + (H_list[i] * A_list[i](c))
    W = invN * W - (invN(d))
    ...
    for i in range(1, len(frequencies)):
        generate_transposed_jacobian.direct(c, frequencies[i], output_operator)
        transposed_jacobian = output_operator.item()
        X = X + transposed_jacobian(H_list[i].T * W)
    
    out[...] = X

grad_chi_squared = Operator(grad_operator, shapein=6*12*nside**2 + 12*nside_beta**2, 
                            shapeout=6*12*nside**2 + 12*nside_beta**2, dtype='float64')
```

```{python}
(grad_chi_squared(true_c) == 0).all()
```

```{python}
Max_iterations = [100]#[0,1,2,3,4,5,7,9,11,13,15,18,21,24,27,30,33,36,39, 100]
...
for i in range(6):
    hp.mollview(true_c[i*npixel:(i+1)*npixel], sub=(7,3,3*i+1), title='True spectral indexes')
    hp.mollview(x_PR_cg[i*npixel:(i+1)*npixel], sub=(7,3,3*i+2))
    hp.mollview(true_c[i*npixel:(i+1)*npixel] - x_PR_cg[i*npixel:(i+1)*npixel], sub=(7,3,3*i+3), title='True spectral indexes')
hp.mollview(true_c[6*npixel:], sub=(7,3,3*6+1))
hp.mollview(x_PR_cg[6*npixel:], sub=(7,3,3*6+2))
hp.mollview(true_c[6*npixel:] - x_PR_cg[6*npixel:], sub=(7,3,3*6+3))
plt.show()
```

```{python}
c = np.ones(6*12*nside**2 + 12*nside_beta**2)
v = np.ones((12*nside**2, 3))

output_operator = np.empty((), dtype=object)
generate_transposed_jacobian.direct(c, frequencies[0], output_operator)
transposed_jacobian = output_operator.item()
transposed_jacobian(v).shape
```

```{python}
npixel = 12*nside**2
nbeta = 12*nside_beta**2
transposed_jacobian(v)[6*npixel:]
```
