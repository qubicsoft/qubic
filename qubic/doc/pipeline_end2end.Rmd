...
CMBmap = False
if CMBmap:
    x0 = FitsArray(dictmaps + 'CMB_nside256_nfsub10.fits')
else:
    x0 = FitsArray(dictmaps + 'nside128_nfsub14.fits')
...
# See what we have
print('--------- TOD with shape: {} - Done ---------'.format(np.shape(TOD)))
```
...
```{python}
maps_convolved.shape
```

```{python}
# Pixels that are not seen enough (less than a given threshold are put as unseen)
