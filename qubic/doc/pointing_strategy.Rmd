...
```{python}
# If you want to save some plots, choose the directory
dir_save = '/home/lmousset/QUBIC/These_manuscrit/'
```
...
hwp = p.angle_hwp
hwp0 = np.where(hwp>50., hwp-10., hwp*1)
...
#tkz.save(dir_save + '/tikz/HWPangles.tex')
...
#plt.suptitle('Not equally spaced: 0°, 30°, 40°', fontsize=20)
plt.suptitle('Equally spaced: 0°, 30°, 60°', fontsize=20)
for istokes in range(3):
    std = np.std(diff[np.invert(unseen), istokes])
    print(std)
    if istokes == 0:
        min = -0.2
        max = 0.2
    ...
    hp.gnomview(diff[:, istokes], cmap='jet',rot=center, sub=(1,3,istokes+1), reso=rr,
                title='Residuals ' + stokes[istokes] + f' - STD = {std:.4f}',
                unit='$\mu K$', min=min, max=max, notext=True)
plt.tight_layout()
#plt.savefig(dir_save + 'Figures/Plots/HWP_0-30-60.pdf', bbox_inches='tight')
```

# Compare repeat and not repeat pointing
...
p_rand = qubic.get_pointing(d)
s_rand = qubic.QubicScene(d)
q_rand = qubic.QubicInstrument(d)
a_rand = qubic.QubicAcquisition(q_rand, p_rand, s_rand, d)

cov_rand = a_rand.get_coverage()
TOD_rand, maps_convolved_rand = a_rand.get_observation(x0[0], noiseless=d['noiseless'])
maps_recon_rand, nit, error = a_rand.tod2map(TOD_rand, d, cov=cov_rand)

diff_rand = maps_recon_rand - maps_convolved_rand
maxcov_rand = np.max(cov_rand)
unseen_rand = cov_rand < maxcov_rand * 0.15

maps_convolved_rand[unseen_rand, :] = hp.UNSEEN
maps_recon_rand[unseen_rand, :] = hp.UNSEEN
diff_rand[unseen_rand, :] = hp.UNSEEN
```
...
for istokes in [1, 2]:
    std = np.std(diff[np.invert(unseen), istokes])
    std_rand = np.std(diff_rand[np.invert(unseen_rand), istokes])
    print(std, std_rand)
    hp.gnomview(diff[:, istokes], cmap='jet',rot=center, sub=(2, 2, istokes), reso=rr,
                title='Repeat strategy \n Residuals ' + stokes[istokes] + f' - STD = {std:.4f}',
                unit='$\mu K$', min=-0.01, max=0.005, notext=False)
    hp.gnomview(diff_rand[:, istokes], cmap='jet',rot=center, sub=(2, 2, istokes+2), reso=rr,
                title='Random strategy \n Residuals ' + stokes[istokes] + f' - STD = {std_rand:.4f}',
                unit='$\mu K$', min=-0.4, max=0.4, notext=False)
plt.tight_layout()
plt.savefig(dir_save + 'Figures/Plots/repeat_random_strategy.pdf', bbox_inches='tight')
```
