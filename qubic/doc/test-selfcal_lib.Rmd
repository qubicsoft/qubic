...
```{python}
# Look at the difference between fringes for 2 punctual source positions
plt.figure()
plt.imshow(fringes[:, :, 0] - fringes[:, :, 1], origin='lower')
plt.pause(0.5)
```
...
ax.plot(x, y, 'go', markersize=40, alpha=0.2)

a = 1
for i,j in zip(x,y):
    corr = -0.002 # adds a little correction to put annotation in marker's centrum
    ax.annotate(str(a),  xy=(i + corr, j + corr), fontsize=14)
    a+=1
plt.xticks(fontsize=14)
plt.yticks(fontsize=14)
ax.set_xlabel('$X_{GRF}$ [m]', fontsize=20)
ax.set_ylabel('$Y_{GRF}$ [m]', fontsize=20)
```
...
sb_proj = np.zeros((248, 200, 200))
for tes in range(248):
    sb_proj[tes, :, :] = hp.gnomview(sb[tes, :], rot=(0, 90, 90), reso=10,
...
#     ax1.annotate(str(tes_index[tes]),  xy=(x, y), fontsize=10, color='r')
    ...
    ax1.set_title('TES {}'.format(tes_index[tes]))

    ax2.imshow(sb_proj[tes], vmin=0., vmax=1e6, animated=True)
    

def init():
    ax2.set_title('Synthetic beam on the sky')
...
steps = np.arange(114, 248, 17)
ani = FuncAnimation(fig, animate, steps, init_func=init, interval=200, blit=True, repeat=False)
