...
class QubicInstrument(Instrument):
    ...
    def get_projection_operator(self, sampling, scene, verbose=True):
        ...
        return QubicInstrument._get_projection_operator(
            ...
            self.synthbeam, horn, primary_beam,self.thetafits, self.phifits, self.valfits,self.use_file,self.jcedit,self.numpeaks, verbose=verbose)

    @staticmethod
    def _get_projection_operator(
            rotation, scene, nu, position, synthbeam, horn, primary_beam, thetafits, phifits, valfits, use_file, jcedit,numpeaks, verbose=True):
