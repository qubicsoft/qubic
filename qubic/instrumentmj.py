...
class QubicInstrument(Instrument):
    ...
    def synthbeam_file(self, d):
        if self.jcedit==False:
            ...
            thetafits,phifits,valfits, numpeaks, mheader = self.calibration.get('synthbeam_jc')
    ...
    def get_projection_operator(self, sampling, scene, verbose=True):
        ...
        return QubicInstrument._get_projection_operator(
            ...
            self.synthbeam, horn, primary_beam,self.thetafits, self.phifits, self.valfits,self.use_file,self.jcedit,self.numpeaks,self.freqs, verbose=verbose)

    @staticmethod
    def _get_projection_operator(
            rotation, scene, nu, position, synthbeam, horn, primary_beam, thetafits, phifits, valfits, use_file, jcedit,numpeaks, freqs, verbose=True):
        ...
        if use_file == True:
            if jcedit == True:
                thetas, phis, vals = QubicInstrument.jchedit(scene, nu, position, synthbeam, horn, primary_beam, thetafits, phifits, valfits, numpeaks)
                print('from fits file using JC edit')
            else:
                if len(freqs) >=2:
                    thetafit=thetafits[freqid].reshape((np.shape(thetafits)[1],np.shape(thetafits)[2]))
                    phifit=phifits[freqid].reshape((np.shape(thetafits)[1],np.shape(thetafits)[2]))
                    valfit=valfits[freqid].reshape((np.shape(thetafits)[1],np.shape(thetafits)[2]))

                    print(np.shape(thetafit))
                    thetas, phis, vals, = thetafit, phifit, valfit
                else:
                    thetas=thetafits
                    phis=phifits
                    vals=valfits
        ...
        freqs=np.array([isfreq]).astype(int)

        new_hdul = fits.HDUList()
        new_hdul.append(fits.ImageHDU(thetas))
        new_hdul.append(fits.ImageHDU(phis))
        new_hdul.append(fits.ImageHDU(vals))
        new_hdul.append(fits.ImageHDU(freqs))
        
        
        new_hdul.writeto('CalQubic_Synthbeam_Analytical_'+nuu+'_FI.fits', overwrite=True)
        '''
    ...
    @staticmethod
    def jchedit(scene, nu, position, synthbeam, horn, primary_beam, theta, phi, val, numpeaks):
        factor = 150e9/nu
