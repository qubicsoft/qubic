...
class QubicInstrument(Instrument):
    ...
    def __init__(self, d, FRBW=None):
        ...
        if sbeam == False:
            ...
            self.sbeam_fits = 'CalQubic_SynthBeam_Analytical_220_FI.fits'
            d['synthbeam'] = 'CalQubic_SynthBeam_Analytical_220_FI.fits'
        ...
        #here are the parts which go to calibration.py
        ...
        #should now have 3 arrays called mtheta,mphi and mval in self
        #also have self.use_file to specify if you use the fits file or not
        #print ('mytheta:')
        #print(self.mtheta)
    ...
    def synthbeam_file(self, d):
        ...
        #get array of theta,phi,val
        thetafits,phifits,valfits, mheader = self.calibration.get('synthbeam')
    ...
    def get_projection_operator(self, sampling, scene, verbose=True):
        ...
        return QubicInstrument._get_projection_operator(rotation, scene, self.filter.nu, self.detector.center, self.synthbeam, horn, primary_beam,self.thetafits, self.phifits, self.valfits,self.use_file, verbose=verbose)

    @staticmethod
    def _get_projection_operator(rotation, scene, nu, position, synthbeam, horn, primary_beam, thetafits, phifits, valfits, use_file, verbose=True):
        ...
        new_hdul.append(fits.ImageHDU(thetas))
        new_hdul.append(fits.ImageHDU(phis))
        new_hdul.append(fits.ImageHDU(vals))
        nuu=str(nu)
        nuu=nuu[0:3]
...
class QubicMultibandInstrument:
    ...
    def __init__(self, d):
        ...
        center_detector -- boolean, optional
        ...
        #get whether analytical or calibrated beams are being used
        self.analytical = d1.get(('analytical'))
        self.analytical=bool(self.analytical)
        ...
        if not d['center_detector']:
            ...
            for i in range(self.nsubbands):
                ...
                # Here is where the synthetic beam cfits file is created
                nuu=filter_nus[i]
                nuu=str(nuu)
                nuu=nuu[0:3]
                ...
                if usefiles == True:
                    ...
                    if boolID == True:
                        d1['synthbeam']='CalQubic_Synthbeam_'+beam_type+'_'+nuu+'_'+file_ID+'_FI.fits'
                    else:
                        d1['synthbeam']='CalQubic_Synthbeam_'+beam_type+'_'+nuu+'_FI.fits'
                else:
                        d1['synthbeam']='CalQubic_SynthBeam_Analytical_220_FI.fits'
