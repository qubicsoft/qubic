...
from qubic.lib.AnalyticalSolution.utils.maps import local_to_full_ring
...
def train_network_localedges(network, optimizer, scheduler, loader, edge_coobs_local, P_operator, seen_indexes_ring, epochs=101, λ_pix = 10., λ_geo = 1.0, λ_coobs = 1.0, λ_phys = 10., weigh_QU = 1., S_I  = 1e17, S_QU = 1e18, device = 'cpu'):
    npix = P_operator.shapein[0]
    for epoch in range(epochs):
        network.train()
        tot = 0.
        for batch in loader:
            ...
            loss_pix = 1000. * λ_pix * F.mse_loss(pred, true)   # scaled MSE 
            loss_geo = 1000. * λ_geo * edge_mse_iqu(pred, true, batch.edge_index, weigh_QU)   #  edge MSE 
            loss_coobs = 1000. * λ_coobs * edge_mse_iqu(pred, true, edge_coobs_local, weigh_QU)
            ...
            # back to physical units
                pred_phys = torch.stack([pred[:,0]/S_I,
                                        pred[:,1]/S_QU,
                                        pred[:,2]/S_QU], dim=1)
                full_ring = local_to_full_ring(pred_phys, npix, seen_indexes_ring)          # (npix,3) RING
                tod_mod = torch.tensor(
                    P_operator(full_ring.detach().cpu().numpy())[:,:,0].flatten(),
                    dtype=torch.float32, device=device)
