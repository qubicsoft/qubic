...
class QubicPolyAcquisition:
    def __init__(self, multiinstrument, sampling, scene, d):
        ...
        if d["MultiBand"] and d["nf_sub"] > 1:
            ...
            raise ValueError("If you do not use a multiband instrument,you should use the QubicAcquisition classwhich is done for the monochromatic case.")
        for a in self[1:]:
            a.comm = self[0].comm
        self.scene = scene
        self.d = d
    ...
    def warnings(self, d):
        ...
        if d["filter_nu"] == 220e9:
            if d["beam_shape"] == "gaussian":
                warnings.warn("The nu dependency of the gausian beam FWHM is not a good approximation in the 220 GHz band.")
            elif d["beam_shape"] == "fitted_beam":
                warnings.warn("Beam and solid angle frequency dependence implementation in the 220 GHz band for the fitted beam does not correctly describe the true behavior")
...
class QubicFullBandSystematic(QubicPolyAcquisition):
    ...
    def get_hwp_operator(self, angle_hwp):
        ...
        """
        return Rotation3dOperator("X", -4 * angle_hwp, degrees=True, shapein=self.Proj[0].shapeout)
