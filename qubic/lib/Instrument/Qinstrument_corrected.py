...
class QubicInstrument(Instrument):
    ...
    def _get_noise_photon_nep(self, scene):
        ...
        if nu_low == self.nu1_down: #150GHz band
    ...
    def NEP_environment(self, noise, names, nu_low, nu_up, return_only=False, sampling=None):
        ...
        noise.P_phot_env = noise.gp[ib2b] * eff_factor * noise.omega_coldstop * noise.S_det * (k * T) ** 4 / c**2 / h**3 * K1
        noise.NEP_phot2_env = 4 * noise.omega_coldstop * noise.S_det * (k * T) ** 5 / c**2 / h**3 * eff_factor * (I1 + I2 * eff_factor)
    ...
    def NEP_lpefilter_220(self, noise, i, nu_low, nu_up, return_only=False, sampling=None):
        ...
        if noise.emissivities[i] == 0.0:
            ...
            noise.P_phot[i] = noise.g[i] * eta * (k * T) ** 4 / c**2 / h**3 * L1 #* noise.sec_beam
            noise.NEP_phot2[i] = 2 * noise.g[i] * eta * (k * T) ** 5 / c**2 / h**3 * (J1 + eta * J2) #* noise.sec_beam
