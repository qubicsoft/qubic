...
import healpy as hp
import numpy as np
from pyoperators import MPI, ReshapeOperator
...
class Chi2Parametric:
    def __init__(self, preset, d, betamap, seenpix_wrap=None):
        ...
        if np.ndim(self.d) == 3:
            ...
            if self.preset.qubic.params_qubic["instrument"] == "UWB":  # ?
                ...
                self.d150 = self.d[:, : self.nsub].copy()
                self.d220 = self.d[:, self.nsub : self.nfreq].copy()
                _sh = self.d150.shape
    ...
    def __call__(self, x):
        if self.constant:
            ...
            if self.preset.qubic.params_qubic["instrument"] == "UWB":
                ysim = np.zeros(self.nsample_ndet)
                for ic in range(self.ncomp):
                    ysim += A[:, ic] @ self.d[ic]
            else:
                ysim = np.zeros(int(self.nsample_ndet * 2))
                # print(A.shape)
                # stop
                for ic in range(self.ncomp):
                    ysim[: int(self.nsample_ndet)] += A[: self.nsub, ic] @ self.d[ic, : self.nsub]
                    ysim[int(self.nsample_ndet) : int(self.nsample_ndet * 2)] += A[self.nsub : self.nfreq, ic] @ self.d[ic, self.nsub : self.nfreq]
        else:
            ...
            if self.preset.qubic.params_qubic["instrument"] == "UWB":
                ...
                A150 = Atot[:, 0, : self.nsub, 1].ravel()
                A220 = Atot[:, 0, self.nsub : self.nfreq, 1].ravel()

                ysim[: int(self.nsample_ndet)] = (A150 @ self.dfg150) + self.dcmb150
                ysim[int(self.nsample_ndet) : int(self.nsample_ndet * 2)] = (A220 @ self.dfg220) + self.dcmb220
...
class Chi2Parametric_alt:  # work in progress?
    def __init__(self, preset, d, A_blind, icomp, seenpix_wrap=None):
        ...
        self.icomp = icomp
        self.nsub = len(self.nfreq)
        self.self.fsub = int(self.nfreq / self.preset.comp.params_foregrounds["bin_mixing_matrix"])
        self.ncomp = len(self.preset.comp.components_out)
        ...
        # else:
        #     self.nfreq = self.d.shape[1]
        #     self.d150 = self.d[:, :int(self.nfreq/2)].copy()
        #     self.d220 = self.d[:, int(self.nfreq/2):self.nfreq].copy()
        #     _sh = self.d150.shape
        #     _rsh = ReshapeOperator(self.d150.shape, (_sh[0]*_sh[1], _sh[2], _sh[3]))
        #     self.d150 = _rsh(self.d150)
        #     self.d220 = _rsh(self.d220)
    ...
    def get_mixingmatrix_comp(self, x):
        ...
        for ii in range(self.preset.comp.params_foregrounds["bin_mixing_matrix"]):
            A_blind[ii * self.self.fsub : (ii + 1) * self.self.fsub, self.icomp] = A_comp[ii * self.self.fsub : (ii + 1) * self.self.fsub]
        return A_blind
    ...
    def __call__(self, x):
        if self.constant:
            ...
            for i in range(self.ncomp - 1):
                for j in range(self.nsub):
                    ...
                    d_220 += A[self.nsub : self.nfreq, (i + 1)][j] * tod_comp_220[i, j]
        ...
        #         ysim = np.zeros(int(self.nsample_ndet*2))
        #         Atot = self._get_mixingmatrix(self.betamap[self._index])
        #         A150 = Atot[:, 0, :int(self.nfreq/2), 1].ravel()
        #         A220 = Atot[:, 0, int(self.nfreq/2):self.nfreq, 1].ravel()
...
class Chi2Blind:
    def __init__(self, preset):
        self.preset = preset
        self.ncomp = len(self.preset.comp.components_out)
        self.nfreq = self.preset.qubic.joint_out.qubic.nsub
        self.nsample_ndet = self.preset.qubic.joint_out.qubic.ndets * self.preset.qubic.joint_out.qubic.nsamples
    ...
    def _fill_A(self, x):
        self.fsub = int(self.nfreq / self.preset.comp.params_foregrounds["bin_mixing_matrix"])
        A = np.ones((self.nfreq, self.ncomp - 1))
        k = 0
        for i in range(self.preset.comp.params_foregrounds["bin_mixing_matrix"]):
    ...
    def _reshape_A_transpose(self, x):
        self.fsub = int(self.nfreq / self.preset.comp.params_foregrounds["bin_mixing_matrix"])
        x_reshape = np.ones(self.nfreq)

        for i in range(self.preset.comp.params_foregrounds["bin_mixing_matrix"]):
            x_reshape[i * self.fsub : (i + 1) * self.fsub] = np.array([x[i]] * self.fsub)
        return x_reshape

    def _qu(self, x, tod_comp):
        ### Fill mixing matrix if self.fsub different to 1
        x = self._fill_A(x)
        ...
        if self.preset.qubic.params_qubic["instrument"] == "DB":
            ...
            A150 = x[: self.nsub * (self.ncomp - 1)].copy()
            A220 = x[self.nsub * (self.ncomp - 1) : self.nfreq * (self.ncomp - 1)].copy()
            ...
            for i in range(self.ncomp - 1):
                for j in range(self.nsub):
                    if i + 1 == icomp:
                        ...
                        d_220 += x[self.nsub : self.nfreq][j] * tod_comp_220[i, j]
                    else:
                        d_150 += A[: self.nsub, (i + 1)][j] * tod_comp_150[i, j]
                        d_220 += A[self.nsub : self.nfreq, (i + 1)][j] * tod_comp_220[i, j]
