...
class ForeGroundModels(CMBModel):
    ...
    def dust_model(self, ell, fnu1, fnu2):
        return (
            self.params["Foregrounds"]["Ad"][0]
            * self.params["Foregrounds"]["deltad"][0]
            * fnu1
            * fnu2
            * (ell / 80) ** self.params["Foregrounds"]["alphad"][0]
        )

    def sync_model(self, ell, fnu1, fnu2):
        return (
            self.params["Foregrounds"]["As"][0]
            * self.params["Foregrounds"]["deltas"][0]
            * fnu1
            * fnu2
            * (ell / 80) ** self.params["Foregrounds"]["alphas"][0]
        )

    def dustsync_model(self, ell, fnu1d, fnu2d, fnu1s, fnu2s):
        m = (
            self.params["Foregrounds"]["eps"][0]
            * np.sqrt(
                abs(
                    self.params["Foregrounds"]["Ad"][0]
                    * self.params["Foregrounds"]["As"][0]
            ...
            * (fnu1d * fnu2s + fnu1s * fnu2d)
            * (ell / 80)
            ** (
                (
                    self.params["Foregrounds"]["alphad"][0]
                    + self.params["Foregrounds"]["alphas"][0]
                )
                / 2
            )
        )
    ...
    def scale_dust(self, nu, temp=20):
        ...
        comp = c.Dust(
            nu0=self.params["Foregrounds"]["nu0_d"],
            temp=temp,
            beta_d=self.params["Foregrounds"]["betad"][0],
        )
    ...
    def scale_sync(self, nu):
        ...
        comp = c.Synchrotron(
            nu0=self.params["Foregrounds"]["nu0_s"],
            beta_pl=self.params["Foregrounds"]["betas"][0],
        )
