...
class FocalPlane:
	...
	def create_subplots(self, tes, asic, ax, **kwargs):
		...
		if np.sum(tes == np.array([4,36,68,100])) != 0:
			...
			ax[i[0][0], i[1][0]].annotate('{}'.format(tes), xy=(0, 0),  xycoords='axes fraction', fontsize=9, color='black',
															fontstyle='italic', fontweight='bold', xytext=(0.05,0.85),backgroundcolor='w')
		self.bar.update()
