...
class Spectra:
    ...
    def __init__(self, filename, covcut=0.2):
        ...
        self.seenpix = (
            self.dictionary["coverage"] / self.dictionary["coverage"].max() > covcut
        )
    ...
    def _get_BB_spectra(self, map, map2=None, fwhm=0):
        BB = self.namaster.get_spectra(
            map=map, map2=map2, verbose=False, beam_correction=np.rad2deg(fwhm)
        )[1][:, 2]
        return BB
        ...
        for irec in range(nmaps):
            for jrec in range(irec, nmaps):
                ...
                if self.fwhm[irec] > self.fwhm[jrec]:
                    ...
                    map1 = C(maps[irec])
                    map2 = maps[jrec]
                BBspectra[irec, jrec, :] = self._get_BB_spectra(
                    map=map1.T, map2=map2.T, fwhm=fwhm
                )
...
class Spectrumold:
    ...
    def __init__(self, file, verbose=True):
        ...
        self.path_spectrum = os.path.join(
            os.path.dirname(os.path.dirname(file)), "spectrum"
        )
        ...
        if (
            ...
            and not self.params["QUBIC"]["convolution_out"]
        ):
            ...
            def f_beta(nu, nu_0, beta):
                return (
                    (nu / nu_0) ** beta
                    * (
                        np.exp(scipy.constants.h * nu * 1e9 / (scipy.constants.k * 20))
                        - 1
                    )
                    / (
                        np.exp(
                            scipy.constants.h * nu_0 * 1e9 / (scipy.constants.k * 20)
                        )
                        - 1
                    )
                )
            ...
            for i in range(self.nrec):
                corrected_fwhm = np.sum(
                    ...
                    * f_beta(
                        self.Q.allnus[i * self.fsub : (i + 1) * self.fsub], 353, 1.53
                    )
                    * fwhm[i * self.fsub : (i + 1) * self.fsub]
                ) / (
                    np.sum(
                        h_list[i * self.fsub : (i + 1) * self.fsub]
                        * f_beta(
                ...
                ) / np.sum(h_list[i * self.fsub : (i + 1) * self.fsub])
    ...
    def _get_allfwhm(self):
        ...
        for i in range(self.nfreq):
            ...
            if (
                self.params["QUBIC"]["convolution_in"] is False
            ):  # and self.params['QUBIC']['reconvolution_after_MM'] is False:
                allfwhm[i] = 0
            elif (
                self.params["QUBIC"]["convolution_in"] is True
            ):  # and self.params['QUBIC']['reconvolution_after_MM'] is False:
    ...
    def compute_auto_spectrum(self, map, fwhm):
        ...
        DlBB = self.namaster.get_spectra(
            map=map.T, map2=None, verbose=False, beam_correction=np.rad2deg(fwhm)
        )[1][:, 2]
        return DlBB
        ...
        for i in range(self.nfreq):
            for j in range(i, self.nfreq):
                ...
                if i == j:
                    ...
                    power_spectra_array[i, j] = self.compute_auto_spectrum(
                        maps[i], self.allfwhm[i]
                    )
                    print(power_spectra_array[i, j, :3])
                    # stop
                else:
                    # Compute the cross-spectrum
                    power_spectra_array[i, j] = self.compute_cross_spectrum(
                        maps[i], self.allfwhm[i], maps[j], self.allfwhm[j]
                    )
