...
jupyter:
  ...
  kernelspec:
    display_name: Python [conda env:qubic] *
    language: python
    name: conda-env-qubic-py
---
...
dictionary = glob.glob(rep + f'*{freq}*{nfrecon}*withCO*.dict')[0]
print(dictionary)
...
fits_noiseless_withCO = np.sort(glob.glob(rep + f'*FI{freq}*nfrecon{nfrecon}*withCO*_noiselessTrue*.fits', 
                              recursive=True))
fits_noiseless_noCO = np.sort(glob.glob(rep + f'*FI{freq}*nfrecon{nfrecon}*noCO*_noiselessTrue*.fits', 
                              recursive=True))

# Get seen map (observed pixels)
#seenmap = rmc.get_seenmap(fits_noiseless_noCO[0])
#npix = seenmap.size
#print('Npix:', npix)
```
...
noiseless_noCO, noiseless_conv_noCO, _ = rmc.get_maps(fits_noiseless_noCO[0])
noiseless_withCO,noiseless_conv_withCO, _ = rmc.get_maps(fits_noiseless_withCO[0])
print('Noiseless shape', noiseless_noCO.shape)
...
hp.gnomview(noiseless_noCO[band, :, istk], 
            cmap='jet', sub=(1, 3, 1), rot=center, reso=rr,
            title = 'NoCO', notext=True)
hp.gnomview(noiseless_withCO[band, :, istk], 
            cmap='jet', sub=(1, 3, 2), rot=center, reso=rr,
            title = 'With CO', notext=True)
```

# Add noise with the fast simulator
...
for r in range(nreals):
    maps_noise[r, :, :, :], coverage = Qubic_sky.get_partial_sky_maps_withnoise(noise_only=True, 
                                                                    spatial_noise=False)     

```

```{python}
#noisy_noCO = maps_noise + noiseless_noCO
#noisy_withCO = maps_noise + noiseless_withCO
```
...
# Create the input sky used in simu
...
istk = 0
a = np.mean((maps_noise + noiseless_withCO)[:, :, pix, istk], axis=0)
b = np.mean((maps_noise + noiseless_noCO)[:, :, pix, istk], axis=0)
delta_a = np.std((maps_noise + noiseless_withCO)[:, :, pix, istk], axis=0)/np.sqrt(nreals)
delta_b = np.std((maps_noise + noiseless_noCO)[:, :, pix, istk], axis=0)/np.sqrt(nreals)
...
ax0.plot(nus_rec, noiseless_conv_noCO[:, pix, istk],
       color='b', ls='--', label='Input SED - no CO')
ax0.plot(nus_rec, noiseless_conv_withCO[:, pix, istk], 'r--', label='Input SED - with CO')
ax0.errorbar(nus_rec, b,
             yerr=delta_b,
             color='b', fmt='o', label='Rec. SED - no CO')

ax0.errorbar(nus_rec, a,
             yerr=delta_a,
...
```{python}
fig.savefig('/home/lmousset/QUBIC/Papiers/Proceeding_Blois2022/Mousset_Proceeding_Blois2022/figures/CO_line.pdf'
)
```
