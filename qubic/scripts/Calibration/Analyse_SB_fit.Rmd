...
```{python}
def show_flatmap_fit_res(directory, dirfit, TESNum, cbar=False, defname='fit_sb_TESNum_'):
...
show_flatmap_fit_res(dirfiles, dir_sym , 93, cbar=True)
```
...
```{python}
integ = data_sym['integ'].copy()
integ[~okfwhmgauss] = np.nan
integ
```
