...
class FlagToMask_beta(): 
    def __init__(self):
        """
        Author: E. Manzan
        Created: 2023-07-20 17:15:00 CET
        Description:
            Beta version of FlagToMask. It returns either a 2D mask of dimension [N_det, N_timestamp] or a 1D mask (as original version)
            based on the dimension of the flag array 
        ==============
        Arguments:
            flagarray: 
                    Array of 64-bit integers for a specific flag. It can be use a 1D or 2D array of flags. (1D = [N_tstamps]; 2D=[N_det, N_tstamps])
            userflag: Default values for each key = False
                    Dictionary with boolean values with user requirements 
        =========================
        Return:
            Mask: 
                Binary mask. 1D = [N_tstamps]; 2D=[N_det, N_tstamps]
        =========================
        """
    ...
    def __call__(self, flagarray, useropts):
        ...
        def masksample(iflagarray, BitIds=BitIds): #Mask one sample through a function
            # zero if flag happens
            return [not bool((int(iflagarray) & int(2**bit)) >> int(bit)) for bit in BitIds]
        
        #if flag array is 1D: use original flagging method
        if flagarray.ndim==1:
            MaskData = list(map(masksample, flagarray))

            return np.prod(MaskData, axis = 1) #product of 0s (flag) and 1s (ok): one 0 kills them all
        
        #if flag array is 2D: return a 2D mask
        elif flagarray.ndim==2: 
            Mask_fp = self.maskfocalplane(flagarray, BitIds) 

            #return 2D binary mask of dim [N_det, N_tstamps]
            return np.prod(Mask_fp, axis = 0)
        else:
            raise ValueError('Flag arrays must be 1D or 2D. Wrong dimension is used!')
            

    def maskfocalplane(self, flag_array, BitIds_array): #Mask entire focal plane through a function
        # Create a 3D empty bool mask of dim [N_bit_to_use, N_det, N_tstamps]
        Mask_det_allbits = np.ones((len(BitIds_array), flag_array.shape[0], flag_array.shape[1]), dtype=bool)

        #loop over flagging categories
        for bit_idx, bit in enumerate(BitIds_array):
            power_of_bit_array = 2**bit*np.ones_like(flag_array)
            bit_array = bit*np.ones_like(flag_array)
            Mask_det_allbits[bit_idx] = ~( np.right_shift(np.bitwise_and( flag_array, power_of_bit_array.astype(np.int64)), bit_array.astype(np.int64) ) ).astype(bool)            

        return Mask_det_allbits
