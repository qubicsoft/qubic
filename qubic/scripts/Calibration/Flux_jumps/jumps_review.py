...
class badIV:
    ...
    def __init__(self, directory: str, n_times: int, TES_number: int = 256, filename: str = "IVdict2025.yaml"):
    ...
    def select_badIV(self):
        ...
        if counts:
            ...
            if self.n_times > max_times:
                raise ValueError(f"n_times={self.n_times} exceeds maximum possible value ({max_times})")
...
class fluxjumps:
    ...
    def __init__(self, thr: Union[float, list, np.ndarray], window_size: int): 
    ...
    def find_candidates(self, tod_haar: np.ndarray, thr: np.ndarray):
        ...
        Iterates through threshold values and identifies the first threshold that
        detects jumps. Returns boolean mask indicating jump locations.
    ...
    def jumps_detection(self, todarray: np.ndarray, consec: bool = True, 
        ...
        jumps, thr_used = self.find_candidates(tod_haar, self.thr)
        ...
        if nc_cond and nc > self.MAX_JUMPS_BEFORE_RETHRESHOLD:
            thr = np.array([self.HIGH_THRESHOLD_VALUE])
            tod_haar = self.haar_function(todarray)
        ...
        if nc == 0:
            return 0, np.array([], dtype=int), np.array([], dtype=int), thr_used

        # Step 4: Find start and end positions of jumps
        if clust is None:
            return 0, np.array([], dtype=int), np.array([], dtype=int), thr_used
            
        xc, xcf = self.initial_start_end(nc, idx_jumps, tod_haar, thr_used, clust)
        ...
        return nc_unique, np.array(xc_unique), np.array(xcf_unique), thr_used
