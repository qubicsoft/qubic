...
#         plt.suptitle(f'TES {TES} - ASIC {ASIC} - {freq} GHz')
#         hp.gnomview(real, sub=(121), reso=15, title='Measurement', min=0., max=None)#norm=SymLogNorm(1e3))
#         hp.gnomview(fake, sub=(122), reso=15, title='Simulation', min=0., max=None)#norm=SymLogNorm(1e4))
#         hp.graticule()
#         hp.visufunc.projscatter(0., 0., color='r', marker='+', lonlat=True, s=100)
...
for idx in range(ndet):
    ...
    hp.gnomview(real, sub=(121), reso=13, title='Measurement', min=0., max=1e4)#norm=SymLogNorm(1e3))
    hp.gnomview(fake, sub=(122), reso=13, title='Simulation', min=0., max=1e6)#norm=SymLogNorm(1e4))
    hp.graticule()
    hp.visufunc.projscatter(0., 0., color='r', marker='+', lonlat=True, s=100)
    #plt.savefig(mydir + date + f'_data_simu_TES{TES}-ASIC{ASIC}-{freq}GHz.pdf', bbox_inches='tight')
```
...
for i in range(ndet):
    print(i)
    #invcov[i, :, :] = np.identity(npix_patch)/10
...
def get_chi2(theta, data, invcov, d, s, radius=20, verbose=False):
    ...
    for idet in range(ndet):
        M = model(theta, idet, d, s, radius=radius, nu=freq * 1e9, PIrot=True, doplot=False)
