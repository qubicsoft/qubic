...
# === 2019 April
# date = '2019-avril'
# data_dir = mydir + date + '/synth_beam_150/Healpix/'
# myname = date + '_Healpix_maps.pdf'
# # Selection of good TES
# PIX_ok = np.arange(1, 257)#[6, 9, 16, 22, 26, 27, 37, 50, 51, 58, 73, 74, 75, 76, 80, 83, 86, 87, 93, 107, 
...
for PIX in PIX_ok:
    ...
    if TES not in [4, 36, 68, 100]:
        ...
        print(FP_index, index_q)
        myindex_q.append(index_q)
        mypix.append(PIX)
...
#         hp.gnomview(patch_mask, reso=10, title=f'PIX {mypix[i]} - Index_q {myindex_q[i]}', min=0)
...
```{python}
def model(theta, index_q, d, s, radius=15, doplot=False):
    fl, phi = theta
    print('theta:', theta)
...
for i, idet in enumerate(myindex_q):
    M = model(theta_fake, idet, d, s, doplot=False)
    gains_fake[i] = np.random.rand(1) * 5
...
for idx in range(ndet):
    ...
    plt.suptitle(f'PIX {PIX_ok[idx]} - Index_q {myindex_q[idx]}')
    hp.gnomview(real, sub=(121), reso=15, title='Real', min=0., max=1e6)#norm=SymLogNorm(1e3))
    hp.gnomview(fake, sub=(122), reso=15, title='Fake', min=0., max=1e6)#norm=SymLogNorm(1e4))
    hp.graticule()
    hp.visufunc.projscatter(0., 0., color='r', marker='+', lonlat=True, s=100)
...
```{python}
plt.figure()
q.horn.plot(edgecolor_open='k', facecolor_open='grey')
plt.axis('off')
plt.savefig('/home/lmousset/Desktop/horn_array.png')
```
...
q = qubic.QubicInstrument(d)
plt.figure()
q.detector.plot(facecolor='grey', fill=True)
plt.axis('off')
plt.savefig('/home/lmousset/Desktop/detector_array.png')
```
