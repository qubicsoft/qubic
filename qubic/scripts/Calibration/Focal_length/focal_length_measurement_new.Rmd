...
```{python}
from pyoperators import Spherical2CartesianOperator, Cartesian2SphericalOperator
from qubic.instrument import _pack_vector

theta, phi, _, _, _ = scal.get_coords_peaks(q, s, d, verbose=True)
thetaphi = _pack_vector(theta, phi) 
print(thetaphi[:10, 1, :])

direction = Spherical2CartesianOperator('zenith,azimuth')(thetaphi)
print(direction[:10, 1, :])
print(peak_vec[:10, :, 1])

azel = Cartesian2SphericalOperator('azimuth,elevation', degrees=True)(direction)
print(azel[:10, 1, :])
```

```{python}
print(peak_azel_simu[:10, :, 1])
...
def get_chi2_focal(focal, sigma, existing_peaks, peak_azel, nine_peak_alldet):
    ...
    for p in existing_peaks:
        ...
        # az, el -> theta phi
        th_simu = np.deg2rad(90 - el_simu_cut)
        phi_simu = np.deg2rad(az_simu_cut)
        th_data = np.deg2rad(90 - el_data_cut)
        phi_data = np.deg2rad(az_data_cut)
        
        # unit vector
        u_simu = hp.ang2vec(th_simu, phi_simu)
        u_data = hp.ang2vec(th_data, phi_data)
        print(u_simu.shape)
        
        ndet = u_simu.shape[0]
        alpha = np.zeros(ndet) # angle between u_simu and u_data for each det
        for i in range(ndet):
            alpha[i] = np.rad2deg(np.arccos(u_simu[i] @ u_data[i]))
            print(alpha[i])

        chi2 += (alpha**2/sigma**2).sum() # sum over detectors and peaks
