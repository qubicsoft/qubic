...
```{python}
def doit(myds, read_data, lowcut, highcut, notch, t0, tf):
    myw=None
    t, folded_bothasics, param_est, res_mich, res_louise = fl.analyse_fringes(myds, 0, myw, 
                                                                        lowcut=lowcut, highcut=highcut, notch=notch,
                                                                        t0=t0, tf=tf, 
                                                                        stable_time=period/6, read_data=read_data)
    ...
    for ites in range(256):
        ...
        resid[ites,:] = folded_bothasics[ites - 1, :]-ft.simsig_fringes(t, stable_time, param_est[ites - 1, :])

    ### RMS residuals for each TES with clipping
    std_resid = np.zeros(256)
    for i in range(256):
        mm, ss = ft.meancut(resid[i,:],3)
        std_resid[i] = ss
    ...
    if doplot==True:
        ...
        plt.plot(t, ft.simsig_fringes(t, stable_time, param_est[tes - 1, :]), label='fit')
        plt.plot(np.arange(0, 6 * stable_time, stable_time) + x0_est, amps_est, 'ro', label='amplitudes')
        plt.plot(t, resid[tes-1,:], label='Residuals (clipped): RMS={0:6.4f}'.format(std_resid[tes-1]))
...
#         colorbar()
    ...
    return fringe_louise, ft.image_asics(all1=std_resid), m_std, s_std, amp_est_TES
...
t0 = None
tf = None
full_fringe, full_std_resid, full_m_std, full_s_std, amp_est_TES = doit(myds, read_data, lowcut, highcut, notch, t0, tf)
...
for js in range(split):
    ...
    fringe, std_resid, m_std, s_std, amp_est_TES = doit(myds, read_data, lowcut, highcut, notch, all_tmin[js], all_tmax[js])
...
t0 = None
tf = 180
fringe_cut, _,_,_,_ = doit(myds, read_data, lowcut, highcut, notch, t0, tf)
