...
In this notebook, I use an analytical model from the fringes, defined in the library `selfcal_lib.py`.  
...
allInvCov = [make_inverse_covariance(errs[k], verbose=True) for k in range(nimages)]
```
...
def get_chi2(params, allinvcov, alldata, BLs, nu_source=150e9, returnA=False):
    nimages = len(BLs) 
    focal = params[0]
...
fl_min, fl_max = 0.27, 0.31
th_min, th_max = np.deg2rad(-3.), np.deg2rad(3)
