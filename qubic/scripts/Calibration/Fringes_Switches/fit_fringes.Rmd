...
In this notebook, I use an analytical model from the fringes, defined in the library `selfcal_lib.py` to fit the fringes measurements.  
...
for k in range(nimages):
    ...
    fringes2D = flib.make2Dfringes_data(allfringes[k] * allokTES[k])
    fringes2D_conv = flib.astropy_convolution(fringes2D, sigma=0.7)
    flib.plot_fringes_imshow(fringes2D_conv, normalize=True, fig=fig, ax=ax0, cmap=cmap_bwr, 
...
```{python}
# Make a list with an inverse covariance matrix for each image
...
c = ax.pcolor(np.rad2deg(all_th), all_fl, chi2_grid, vmin=0, vmax=1e5)#, norm=SymLogNorm(99e9))
ax.set_xlabel('Theta')
...
def plot_residuals(q, xONAFP, yONAFP, BL, data, okTES, PowerPhi, errors, gains, cmap='bwr'):
    ...
    init2D = flib.make2Dfringes_QubicSoft(data*okTES, q)
    init2D_conv = flib.astropy_convolution(init2D, sigma=0.7)
    flib.plot_fringes_imshow(init2D_conv, normalize=True, fig=fig, ax=axs[2], cmap=cmap, 
                             title='Data with Astropy convolution', mask=flib.make_mask2D_thermometers_TD())
    
    correct2D = flib.make2Dfringes_QubicSoft(correct*okTES, q)
    correct2D_conv = flib.astropy_convolution(correct2D, sigma=0.7)
    flib.plot_fringes_imshow(correct2D_conv, normalize=True, fig=fig, ax=axs[3], cmap=cmap, 
...
cmap_bwr=flib.make_cmap_nan_black('bwr')
for k in range(nimages):
    print(okTES[k].shape, fringes[k].shape)
    plot_residuals(q, xONAFP, yONAFP, BLs[k], fringes[k], okTES[k], PowerPhi[k], errs[k], A, cmap=cmap_bwr)
