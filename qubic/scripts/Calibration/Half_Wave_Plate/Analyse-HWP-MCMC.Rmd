...
for TESNum in range(1, 257):
    ...
    if len(ff) != 0:
        resfit, resmcmc, samples = pickle.load(open(chain_file,'rb'))
...
def DBSCAN_cut(values, errors, doplot=False, min_samples=40):
    ...
    if doplot:
        for i in range(sh[1]):
            ...
            for k in range(len(unique_labels)):
                ...
                xlabel('Param {}'.format(i))
                ylabel('Error Param {}'.format(i))
                if i==0:
                    legend()
    return (labels == 0)
...
```{python}
ok = DBSCAN_cut(means[:,1:], errs[:,1:], doplot=True, min_samples=55)
```
...
myok = ok.copy()
subplot(1,2,1)
a=hist(means[myok,1]/errs[myok,1], range=[0,5], bins=30)
subplot(1,2,2)
a=hist(means[myok,1]/errs[myok,1], range=[0,3], bins=30, cumulative=True, density=True)
```
...
title('Passing cuts: {} TES out of 248'.format(np.sum(ok)))
```
