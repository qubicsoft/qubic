...
for j in range(nparts_samples):
    ...
    endchunk_samp = int((j + 1) * 170595 / nparts_samples) if j != nparts_samples-1 else 170594
    print(endchunk_samp)
    maskchunk = (p.time > p.time[inichunk_samp]) & (p.time < p.time[endchunk_samp])
    print("Number of samples per chunk ", maskchunk.sum())
    pchunks.append(p[inichunk_samp:endchunk_samp])
plt.figure(figsize = (14,6))
colors = sns.color_palette(palette = 'tab10', n_colors = 10)
for j in range(nparts_samples):
...
for i in range(nparts_samples):
    atod_chunks.append(qubic.QubicAcquisition(q, pchunks[j], s, d))
    #atod = qubic.QubicMultibandAcquisition(q, p, s, d, nus_edge_in)
...
if d['nf_sub'] == 1:
    ...
    arec = qubic.QubicAcquisition(q, p[:-5], s, d)
...
hp.gnomview(maps_recon, rot = (116.4,40), sub = 132, title = 'Reconstructed',
...
hp.gnomview(maps_convolved, rot = (116.4,40),  sub = 131, title = 'Convolved (1 chunk)',
           reso = 12, xsize = 250, ysize = 200)
hp.graticule(dpar = 8, dmer = 8, color = 'w', verbose = False)
hp.gnomview(cov, rot = (116.4,40), sub = 133, title = 'Coverage+moon',
           reso = 12, xsize = 250, ysize = 200)
hp.graticule(dpar = 8, dmer = 8, color = 'w', verbose = False)
