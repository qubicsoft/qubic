...
# To save the plots
#save_plot_dir = os.environ['HOME']
#save_plot_dir = '/home/lmousset/QUBIC/Qubic_work/SpectroImagerie/paper_plot/'
#print('Plot will be saved at:', save_plot_dir)

# If you work on the JupyterLab CC-in2p3 plateforme, maybe you need to define this global variable here
#os.environ['QUBIC_PEAKS'] = '/sps/qubic/Users/lmousset/Synthesized_beams/'
...
machine = os.uname()[1]
print(machine)
if 'cc.in2p3' in machine:
    print('==========================================')
    print('You are at the CC-IN2P3')
    top_dir = '/sps/qubic'
    user_dir = 'Users/lmousset'
#     user_dir = os.environ['HOME']
    synthbeam_topdir = 'Synthesized_beams'
    ...
    fitted_directory = os.sep.join([top_dir,user_dir,synthbeam_topdir,synthbeam_subdir,freq_subdir[freq_name],'FitSB_JC'])

elif (machine=='apcdhcp52.in2p3.fr') or (machine=='MacJCH-2.local'):
    print('==========================================')
...
elif machine=='moussetDell':
    print('You are on Louise laptop')
    top_dir = '/home/lmousset/QUBIC/Qubic_work/SpectroImagerie'
    save_plot_dir = os.sep.join([top_dir, 'paper_plot'])
    fittedpeakfile = os.sep.join([top_dir, 'SpectroImRealData','fitted_peaks.fits'])
    directory = os.sep.join([top_dir, 'SpectroImRealData', '150GHz-2019-04-06'])
    fitted_directory = os.sep.join([top_dir, 'SpectroImRealData', '150GHz-2019-04-06', 'FitSB_JC'])
...
#     out_dir = os.sep.join([top_dir, user_dir])
    out_dir = os.sep.join([top_dir, 'SpectroImRealData'])
...
ax1.plot(allnums, maxpeakamps)
ax1.plot(allnums[np.array(detsgood)-1], maxpeakamps[np.array(detsgood)-1], 'ro')
ax1.set_xlabel('TES#')
ax1.set_ylabel('Highest peak for each TES')

ax2.plot(allnums, np.log10(maxpeakamps))
ax2.plot(allnums[np.array(detsgood)-1], np.log10(maxpeakamps[np.array(detsgood)-1]), 'ro')
ax2.set_xlabel('TES#')
ax2.set_ylabel('log10(Highest peak) for each TES')

h = ax3.hist(maxpeakamps, bins=30)
ax3.set_xlabel('Highest peak for each TES')

h = ax4.hist(np.log10(maxpeakamps), range=[2.5, 6], bins=50)
ax4.set_xlabel('Log10(Highest peak) for each TES')
...
### Choose the detectors and the number of sub-bands and run the map-making 
...
all_maps_recon_custom_new, qcut, cov, nus, nus_edge = do_some_dets(detnums, 
                                                                   d, 
                                                                   directory, 
                                                                   fittedpeakfile, 
...
#     maxi = mm+3*ss
    ...
    hp.gnomview((all_maps_recon_custom_new[k,:]), reso=reso/2, rot=center, cmap = 'viridis', 
                title='Ntes={0:} \n nu=[{1:5.1f},{2:5.1f}]'.format(len(detnums), nus_edge[k], nus_edge[k+1]),
                min=mini, max=maxi, sub=(1,nf_sub_rec,k+1))
fig.tight_layout()

# Coverage
hp.gnomview(cov, reso=reso/2, rot=center, min=0, max=1, title='Coverage')

# Kill the pixel outside the coverage
okpix = cov > 0.3
...
#     maxi = mm+3*ss
    mini = -0.01
    maxi = 0.03
    mymap = hp.gnomview((all_maps_recon_custom_new_cut[k,:]), 
                        reso=reso/2, 
                        rot=center, 
                        cmap='viridis', 
                        title=r'$\nu=[{0:5.1f},{1:5.1f}]$'.format(nus_edge[k], nus_edge[k+1]),
                        min=mini, max=maxi, 
                        sub=(2,5,k+1),
...
fig.tight_layout()
fig.show()
# fig.savefig(save_plot_dir + '/SpecImRealdata_maps_150GHz_FitSBJC.pdf', bbox_inches='tight')
...
def mygaussian(x, pars, extra_args=None):
    """pars: list
            Parameters for the Gaussian: [offset, amplitude, mean, sigma]."""
...
for i in range(len(maps)):
    ...
    flux[i], err_flux[i], mag, err_mag = aperture_photometry(maps[i], mycenter, radius, subs=False,
                                                             doplot=True, stampsize=100, magnitude_zero=0, cmap=None)
...
```{python}
# Fit with the four parameters free
...
###!!!!! Put 9 to reproduce the same plot as Steve
P = 8 # Number of horns on one side
thsig = nu0/((P-1) * 2.355)
...
print('Fit result:', fitg[1])
print('Errors:', fitg[2])

xxx = np.linspace(130, 170, 100)
plt.figure()
plt.errorbar(nus,flux, yerr=err_flux, xerr=dnu, fmt='ro')
...
```{python tags=c()}
# New fit, the mean and the sigma of the Gaussian model (p2 and p3) are fixed.  
...
# Normalization by height of the Gaussian (from the offset to the max)
norm = np.max(mygaussian(xxx, fitg[1]))-fitg[1][0]#-fitg[1][0])
print(norm)
...
ax = fig.gca()
ax.errorbar(nus, flux-fitg[1][0], 
             yerr=err_flux, xerr=dnu, fmt='ro',
             label='Measurement')
ax.plot(xxx, (mygaussian(xxx, fitg[1]))-fitg[1][0], 
         label=r'Expected: $\nu_0$={:3.1f} GHz'.format(nu0) + '\n' 
                 + r'  $\quad \quad \quad \quad \quad \sigma=${:3.1f} GHz'.format(thsig))
...
ax.set_ylabel('Source Flux')
# plt.ylim(-0.5, np.max(flux-fitg[1][0])*1.3)
# ax.set_ylim(-0.4, 0.8)
ax.set_xlim(130, 170)
ax.legend(loc='upper right', fontsize=16)

# plt.savefig(save_plot_dir + '/SpecImRealdata_spectre_150GHz_FitSBJC_sigma9.pdf')
```
...
```{python}
### Plot for Paper QUBIC III (Steve)
# okpix = cov > 0.3
# all_maps_recon_custom_new_cut = all_maps_recon_custom_new.copy()
# all_maps_recon_custom_new_cut[:,~okpix] = 0
...
# ylabel('Source Flux', fontsize=40)
# tick_params(axis='both',labelsize=40)
