...
reload(sbfit)
# Selecting TES number in absolute (1-248) notation:
#fullTES = np.arange(92,160,48, dtype = int)+1 #[28, 37, 60, 96, 101, 122, 124, 137, 140, 149]
fullTES = np.array([28, 37, 60, 96, 101, 122, 124, 137, 140, 149]) #
#fullTES = np.array([28, 122, 137]) #
#thermos_index = np.array([4, 36, 68, 100, 4+128, 36+128, 68+128, 100+128]) 
#thermos_index = np.array([7])
#fullTES = np.delete(fullTES, [7])
...
for j, iTES in enumerate(fullTES):
    ...
    FitsArray(thexypeaks[j]).save(dirfiles + \
                                  'Simulations/Flat/peaks/fitted_peaks_flat_TES{}ASIC{}.fits'.format(fulltes[j],
                                                                                               fullasic[j]))
    
    print('Flat done. ',(time.time()-t0)/60 , 'minutes')
    ...
    FitsArray(thphpeaksnew[j]).save(dirfiles + \
                                    'Simulations/Healpix/peaks/fitted_peaks_hp_TES{}ASIC{}.fits'.format(fulltes[j],
                                                                                            fullasic[j]))

    print('Healpy done. ',(time.time()-t0)/60 , 'minutes')
    ...
    hpnest[j, msk] = hp.UNSEEN
    funcpr(hpnest[j], nest = False, min = 0, cbar= False, rot = (azcen_fov, elcen_fov),
                title = 'Fitting (hp) TES = {}, asic = {}'.format(fulltes[j], fullasic[j]),
           xsize = 200, ysize = 180, hold = True, reso = 10)
    hp.projscatter(thphpeaksnew[j,0,:], thphpeaksnew[j,1,:], color = 'r', marker = '.')
    hp.graticule(verbose = 0)

    im = ax[1].imshow(np.reshape(tod, (len(el), len(az))), 
                 extent = [np.min(az) * np.cos(np.radians(elcen_fov)),
                                   np.max(az) * np.cos(np.radians(elcen_fov)),
                                   np.min(el), np.max(el)],  )
    ax[1].set_ylabel('El[°]')
    ax[1].set_xlabel('Az[°]')
    ax[1].plot(thexypeaks[j,0,:], thexypeaks[j,1,:], 'r.')
    ax[1].set_title('Fitting (flat) TES {} asic {}'.format(fulltes[j], fullasic[j]))
```
...
for j in range(len(fullTES)):
    ...
    FitsArray(np.reshape(tod, 
                     (len(el), len(az))) ).save(directory + \
                    'Simulations/Flat/synth_beam/flat_TES{}ASIC{}.fits'.format(fulltes[j], fullasic[j]))
    FitsArray(hpnest[j]).save(directory + \
                    'Simulations/Healpix/synth_beam/hp_TES{}ASIC{}.fits'.format(fulltes[j], fullasic[j]))
#Save all peaks in a single calibration file
FitsArray(thexypeaks).save(os.environ['QUBIC_PEAKS'] + \
                                  +'150GHz-2019-04-06/Simulations/Flat/flatpeaks.fits')
FitsArray(thphpeaksnew).save(os.environ['QUBIC_PEAKS'] + \
                                  +'150GHz-2019-04-06/Simulations/Healpix/healpypeaks.fits')
...
with PdfPages('multipage_pdf.pdf') as pdf:
    for j in range(8):
        ...
        funcpr(hpnest[j], nest = False, min = 0, cbar= False, rot = (azcen_fov, elcen_fov),
               ...
               xsize = 200, ysize = 180, hold = True, reso = 10)
        hp.projscatter(thphpeaksnew[j,0,:], thphpeaksnew[j,1,:], color = 'r', marker = '.')
        #hp.projscatter(np.radians(50), np.radians(-10), color = 'y', s = 160, marker = '.')
        ...
        ax[1].set_ylabel('El[°]')
        ax[1].set_xlabel('Az[°]')
        ax[1].plot(thexypeaks[j,0,::-1], thexypeaks[j,1,::-1], 'r.')
        ax[1].set_title('Fitting (flat) TES {} asic {}'.format(fulltes[j], fullasic[j]))

        #axes(ax[2])
        #hp.gnomview(cov, rot = (azcen_fov, elcen_fov), reso = 10, hold = True, cbar = False,
        #           xsize = 300, ysize = 250, title = "Coverage map (>25%)")
        #hp.graticule(verbose = 0, alpha = 0.4)


    ### Map-making in flat projections (from JCh instrument and Realistic_MultiBandReconstruction.Rmd)
```

```{python}
#MM flat maps
fittedpeaksfile = dirfiles + 'Simulations/Flat/peaks/fitted_peaks_flat_TES{}ASIC{}.fits'.format(fulltes[j],
                                                                                               fullasic[j])
def do_some_dets(fullTES, d, fittedpeaksfile, 
                 fittedpeaksfile, custom=False, 
