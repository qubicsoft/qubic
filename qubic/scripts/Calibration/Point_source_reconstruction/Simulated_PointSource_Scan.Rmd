...
def create_hall_pointing(d, az, el, hor_center, angspeed_psi = 0, maxpsi = 0, period = 0,
    ...
    if doplot:
        pixs = hp.ang2pix(d['nside'], np.radians(90 - theta), np.radians(phi))
        mapaThPh = np.ones((12*256**2))
        mapaThPh[pixs] = 100
        hp.mollview(mapaThPh, title = "Projection in horizontal coords")
    ...
    coords = rotation(np.asarray([phi.T, theta.T]).T)
...
#subplot(121)
#plot(np.sin(np.radians(theta2))*np.cos(np.radians(phi2)),
...
period = 0.1
p = create_hall_pointing(d, az, el, hor_center = [azcen_fov, elcen_fov],
...
hp.gnomview(cov, rot = [0,50], reso = 14)
```
...
ax[2].set_ylabel(r'$\delta$[°]', fontsize = 20)
