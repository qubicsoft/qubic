...
for j, iTES in enumerate(fullTES):
    ...
    hpnest, thphpeaksnew, absmaxpx = InitSearch(simulation = True, maps = convmap, threshold = 3,
    ...
    axes(ax[0])
    funcpr= hp.gnomview
    funcpr(hpnest, nest = False, min = 0, cbar= False, rot = (azcen_fov, elcen_fov),
    ...
    im = ax[1].imshow(np.reshape(TOD[fullqpix[j]], (len(el), len(az))), 
