...
for j, iTES in enumerate(fullTES):
    ...
    hpnest[j], thphpeaksnew[j] = InitSearch(simulation = True, maps = convmap, threshold = 3,
                                            filterbeam = 1.5, delta = 8, doplot = False, 
                                                factor_corrector = 1.4)
...
for j in range(2):
    fig, ax = subplots(nrows = 1, ncols = 3, figsize = (14,8))
    tod = TOD[fullqpix[j]]
    axes(ax[0])
    funcpr= hp.gnomview
    msk = np.where(cov == hp.UNSEEN, )
    ...
    funcpr(hpnest[j], nest = False, min = 0, cbar= False, rot = (azcen_fov, elcen_fov),
                title = 'Fitting (hp) TES = {}, asic = {}'.format(fulltes[j], fullasic[j]),
           xsize = 300, ysize = 250, hold = True, reso = 10)
    hp.projscatter(thphpeaksnew[j].T, color = 'r', marker = '.')
...
```{python}
#MM flat maps
def do_some_dets(fullTES, d, directory + 'Simulations/Flat/imgflat_TESNum_{}.fits', 
