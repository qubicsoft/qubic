...
fa(azmap).save(savedirtest + 'azimuth.fits')
fa(elmap).save(savedirtest + 'elevation.fits')
```
...
for i in range(4):
    ...
    FitsArray(mapdata[i,:,:]).save(savedirtest+'imgflat_TESNum_{}.fits'.format(i+1))
```

Test load the maps...ok!

```{python}
flattest, az, el = sbfit.get_flatmap(1, savedirtest, 
                                     azmin = azmin, azmax = azmax, 
                                     remove = None)
imshow(flattest)
```
