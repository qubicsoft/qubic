...
def check_dir(dirfile):
    if os.path.isdir( dirfile): 
        print('dirfile: {} exist'.format(dirfile)) 
    else: 
...
Directories = {"RawTodDir": data_dir,
...
dirs = np.sort(glob.glob(Directories["RawTodDir"] + keyword))

Directories.update({"sweeps_data": dirs})
```
...
QubicFP = qubicfp()
QubicFP.read_qubicstudio_dataset(Directories["sweeps_data"][0])
```
...
t_src, data_src = md.read_calsource_data(QubicFP, date = day,
                      keyword = keyword, datadir = Directories["RawTodDir"],
                      datacal = Directories["calsource_data"], verbose = True)
```
...
if verbose: QubicFP.read_qubicstudio_dataset(Directories["sweeps_data"][3])
```
