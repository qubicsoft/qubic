...
if confg == "new":
    ...
    day = '2019-04-06'
    keyword = '*ScanMap*'
    dirfile = "/home/martin/QUBIC/qubiccalibration/Data/Calib-TD/calsource/"
...
####READING CALSOURCE DATA
#### If you use data previous to Nov 2019, you have to read data in a different way. 
#Otherwise, you can use calsource() method 
day = '2020-03-16'
keyword = '*ScanFast*'
...
##### Reading old data (2019, previous to qubicpack existance)
...
if confg == 'old':
    dirfile = "/home/martin/QUBIC/qubiccalibration/Data/Calib-TD/calsource/"
    filename = glob.glob(dirfile + "*{}*.fits".format(day.replace("-", "")))
    t_src, data_src = [], []
    for ila in filename:
        hdufile = pyfits.open(ila)
        hdu = QubicFP.read_calsource_fits(hdufile[1])
        t_src.append(np.array(QubicFP.hk['CALSOURCE']['timestamp']))
        data_src.append( QubicFP.hk['CALSOURCE']['Value'])

for i in range(len(t_src)):
    plt.plot(t_src[i], data_src[i])
...
############ Power spectrum
spectrum_f, freq_f = ft.power_spectrum(tod_time, tod_data, 
                                       rebin = True)
plt.plot(freq_f, f.gaussian_filter1d(spectrum_f,1), 
     label = 'Raw Data')
plt.yscale('log')
plt.xscale('log')
...
plt.xlim(xmin, xmax)
plt.ylim(ymin, ymax)
plt.title('TES {} ASIC {}'.format(TESNum,
                                  asic))

for i in range(10):
    plt.plot([1./period*i,1./period*i],[ymin,ymax],'k--',alpha=0.3)

plt.plot([lowcut, lowcut], [ymin, ymax], 'k')
plt.plot([highcut, highcut], [ymin,ymax], 'k')
plt.legend()

########## New Power spectrum
nharm = 10
notch = np.array([[1.724, 0.005, nharm]])
newdata = ft.filter_data(tod_time, tod_data, lowcut, highcut, 
                         notch = notch, rebin = True, 
                         verbose = True, order = 5)
spectrum_f2, freq_f2 = ft.power_spectrum(tod_time, newdata, rebin=True)
plt.plot(freq_f2, f.gaussian_filter1d(spectrum_f2, 1), label = 'Filtered data')
for i in range(nharm):
    plt.plot([notch[0,0]*(i+1),notch[0,0]*(i+1)], [ymin,ymax], 'm:')
...
```{python}
plt.plot((tod_time - t0) / 3600, dl.renorm(newdata), 
     label='Data TES {} ASIC {}'.format(TESNum, asic))
plt.plot((t_src - t_src[0])/3600, dl.renorm(data_src) - 5, label = 'source')
plt.xlabel('Hours')
plt.legend()
plt.ylim(-20,20)
...
if demodulate: 
    ...
    temp_amps = dl.demodulate_methods([QubicFP.timeaxis(axistype = 'pps', asic = 1), 
                          ...
                          1./period, 
                          src_data_in = [tod_time, np.interp(tod_time, t_src, data_src)],
                          method = 'demod_quad', 
                          remove_noise = True,
    ...
    for asic in [1,2]:
        tod_time = QubicFP.timeaxis(axistype = 'pps', asic = asic)
        src = [tod_time, np.interp(tod_time, t_src, data_src)]
        for i in range(128):
            print('Mapmaking for Asic {} TES {}'.format(asic, i + 1))    
            tod_data = QubicFP.timeline(TES = i + 1, asic = asic)
    ...
    for i in range(256):
        FitsArray(amps_demod[i,:]).save(savedirtest + \
                                        'Demod_TOD/tod_data_demodulated_TESNum_{}.fits'.format(i+1))    
    FitsArray(newt_demod).save(savedirtest + 'Demod_TOD/tod_time_demodulated.fits')
    
elif not demodulate:
    print("Reading demodulated data")
    amps_demod = []
    for i in range(256):
        amps_demod.append(np.array(FitsArray(savedirtest + \
                                             'Demod_TOD/tod_data_demodulated_TESNum_{}.fits'.format(i+1))))
    newt_demod = np.array(FitsArray(savedirtest + 'Demod_TOD/tod_time_demodulated.fits'))
...
if ComputeFlatMaps:
    ...
    mapdata, azmap, elmap = dl.coadd_flatmap(amps_demod, newaz, newel, 
                                            ...
                                            naz = naz, nel = nel)
    FitsArray(azmap).save(savedirtest + 'Demod_TOD/azimuth.fits')    
    FitsArray(elmap).save(savedirtest + 'Demod_TOD/elevation.fits')    
    for i in range(256):
        FitsArray(mapdata[i,:,:]).save(savedirtest + 'Flat/imgflat_TESNum_{}.fits'.format(i+1))  
    
elif not ComputeFlatMaps:
    azmap = np.array(FitsArray(savedirtest + 'Demod_TOD/azimuth.fits'))
    elmap = np.array(FitsArray(savedirtest + 'Demod_TOD/elevation.fits'))
    mapdata = np.zeros((256, len(azmap[0]), len(elmap[0])))
    for i in range(256):
        mapdata[i,:,:] = np.array(FitsArray(savedirtest + 'Flat/imgflat_TESNum_{}.fits'.format(i+1)))
```
