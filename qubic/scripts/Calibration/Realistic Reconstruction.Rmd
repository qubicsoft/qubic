...
```{python}
essai = mymaskedmap(detnums, d, directory, fittedpeakfile, mask, tol=1e-3)
#mm = essai(2,np.array([np.radians(0.), np.radians(90.), np.radians(0.)]))
nnth=11
nnph=11
thvals = np.linspace(85,95,nnth)
phvals = np.linspace(-5,5,nnph)
...
imshow(scf.gaussian_filter(ch2,0.01), extent=[np.min(thvals), np.max(thvals), np.min(phvals), np.max(phvals)]
       ,vmin=0.03, vmax=0.06)
xlabel('theta')
ylabel('phi0')
colorbar()
...
import scipy.ndimage.filters as scf
plotminarray(scf.gaussian_filter(ch2,0.5), 
