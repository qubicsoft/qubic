...
def thph_qsoft(qinst, scene, soft_pix, 
    ...
    #                                              qTD.detector[PIXcentral].center, qTD.synthbeam, 
...
for j, PIXNum in enumerate([1,30,60,90,120,150,180,210,240]):
    ...
    hpnest, thphpeaksnew, absmaxpx = fit_hpmap(PIXNum, dirfiles, newcenter, plot = False,
    ...
    fit, xypeaks = sbfit.fit_sb(flatmap, az, el, sbfitmodel3, verbose = False, resample = False, 
                            doplot = False, extra_title = 'TES #{} ASIC {}'.format(TESNum, ASIC))

    axes(ax1[1])
    title('Flat projection (TES#{} ASIC {})'.format(TESNum, ASIC))
...
```{python}
figure(figsize=(8,8))
for j, PIXNum in enumerate([232,216,200,154,165,152]):
    if PIXNum < 129:
    ...
    th_cent, ph_cent = th_cent_all[0,0], ph_cent_all[0,0]
    PIXq, TESNum, ASIC = q_pix(PIXNum)
    plot(qTD.detector[PIXq].center[0,0],qTD.detector[PIXq].center[0,1],'ks',ms=9)
