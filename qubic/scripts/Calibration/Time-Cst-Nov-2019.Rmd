...
def analyse_loop(tofit, errors, t, fitfunct, fixpars=None, fixvals=None, doplot=False):
...
def analyse_twopasses(tofit, errors, t, fitfunct, doplot=False):
    # First Pass
    ...
    ok = DBSCAN_cut(results) & (allpars[:,1] < 0.29) & (allpars[:,2] < 0.29)

    return allpars, allerrpars, allchi2_ndf, ok

def plot_result_asic(allpars, ok, pngname=None, savefilename=None):
    subplot(2,3,1)
...
########## Folding all Asic 1
reload(ft)
folded1, t1, folded_nonorm1, dfolded1, dfolded_nonorm1, newdata1= ft.fold_data(data['t_data 1'], data['data 1'], 
...
```{python}
########## Folding all Asic 2
folded2, t2, folded_nonorm2, dfolded2, dfolded_nonorm2,newdata2= ft.fold_data(data['t_data 2'], data['data 2'], 
                                                                            period, lowcut, highcut, 
                                                                            nbins, median=True, rebin=True, 
                                                                              verbose=False, return_error=True)
...
bla = ft.do_minuit(t, tofit[TESNum-1,ok], errors[TESNum-1,ok],
	...
	rangepars=[[0.1,0.5], [0., 0.5], [0., 0.5], [0.,4], [-2*amplitude,2*amplitude], [0,offset*2]], fixpars=[1,0,0,0,0,0], 
	force_chi2_ndf=True, verbose=False, print_level=0,nsplit=5)
params =  bla[1]
err = bla[2]
print(params)
plot(t, function(t, bla[1]), 
...
def run_all_anaysis(thedir, period, nbins, lowcut, highcut):
    ...
    tofit = folded_nonorm
    errors = dfolded_nonorm
    allpars, allerrpars, allchi2_ndf, ok = analyse_twopasses(tofit, errors, t, ft.simsig_asym, doplot=False)
    
    rc('figure',figsize=(15,8))
    name = 'TimeCst-'+str.split(thedir,'/')[-1]
    pngname = '/Users/hamilton/Qubic/Calib-TD/TimeConstants/'+name+'_results.png'
    print('Saving plot in {}'.format(pngname))
    savefilename = '/Users/hamilton/Qubic/Calib-TD/TimeConstants/'+name+'_data.txt'
    print('Saving data in {}'.format(savefilename))
    plot_result_asic(allpars, ok, pngname=pngname, savefilename=savefilename)
...
allpars, allerrpars, allchi2_ndf, ok= run_all_anaysis(thedir, period, nbins, lowcut, highcut)
```
...
for ifile in range(len(dirs)):
    allpars, allerrpars, allchi2_ndf, ok= run_all_anaysis(dirs[ifile], period, nbins, lowcut, highcut)
```
