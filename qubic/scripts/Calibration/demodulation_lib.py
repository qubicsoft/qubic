...
def demodulate(indata, fmod, lowcut=None, highcut=None, verbose=False):
    ...
    if np.array(indata['t_src']).ndim==0:
        ...
        if (lowcut is None) & (highcut is None):
            ...
            dataf = filter_data(indata['t_data'], indata['data'], lowcut, highcut)
            new_src = filter_data(indata['t_data'], np.interp(indata['t_data'], indata['t_src'], indata['data_src']), lowcut, highcut)
...
def array_info(ar):
    mini = np.min(ar)
    maxi = np.max(ar)
    return 'Shape = {0:15} Min={1:6.3g} Max={2:6.3g} Rng={3:6.3g} Mean={4:6.3g} Std={5:6.3g}'.format(str(np.shape(ar)),mini, maxi, maxi-mini, np.mean(ar), np.std(ar))


def read_data_azel_src(dirfile, AsicNum, TESNum=None, 
                       calsource_dir='/qubic/Data/Calib-TD/calsource/',verbose=False):
    a = qp()
