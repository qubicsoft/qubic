...
def mypipe(mytod, tt, azt, elt, scantype, doplot=False, 
           nsplines_global=40,
           nbspl_scan=5,
           nbins_scan=30, highpass=False, order=5):
    
    # 1. High-Pass or Remove a slow spline to the data
    if highpass:
        mytod = ft.filter_data(tt, mytod, lowcut = highpass, highcut = None, rebin=True, verbose=False, notch=None, order=order)
    else:
...
mysimtod = mypipe(simtod, tt, azt, elt, scantype, doplot=False, 
                  nsplines_global=nsplines_global,
                  highpass=highpass,
                  nbspl_scan=nbspl_scan,
                  nbins_scan=nbins_scan)
...
sky=pysm3.Sky(nside=64, preset_strings=['d0'], output_unit="uK_CMB")
...
hp.gnomview(mydust, rot=[np.mean(ll), np.mean(bb)], reso=15, title='PySM 150 GHz', sub=[1,3,3], max=1000)
hp.graticule()
...
hp.gnomview(mymedmap, rot=[np.mean(ll), np.mean(bb)*0], reso=res, min=-nsig*ss, max=nsig*ss, title='Median', sub=[1,3,2])
hp.graticule()

hp.gnomview(mapscan_sim, rot=[np.mean(ll), np.mean(bb)*0], reso=res, title='PySM 150 GHz', sub=[1,3,3])
hp.graticule()

```

```{python}
plot(tesnums[oktes], allcalib[oktes], 'ro')
```
