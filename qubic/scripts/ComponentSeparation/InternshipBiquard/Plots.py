...
class Plots:
    ...
    def diff_2_maps(map1, map2, seenpix, ifreq, rot, reso, title1, title2, Stokesparameter='IQU'):
        ...
        if Stokesparameter == 'IQU':
            ...
            for istk in range(3):
                sig = np.std(map2[ifreq, istk, seenpix])

                hp.gnomview(map1[ifreq, istk, :], rot=rot, reso=reso,
                            title=title1 + '\n {} Stokes parameter'.format(Stokes[istk]), sub=(3, 3, 3 * istk + 1),
                            min=-nsig * sig, max=nsig * sig)
                hp.gnomview(map2[ifreq, istk, :], rot=rot, reso=reso,
                            title=title2 + '\n {} Stokes parameter'.format(Stokes[istk]), sub=(3, 3, 3 * istk + 2),
                            min=-nsig * sig, max=nsig * sig)
                residue = map1[ifreq, istk, :] - map2[ifreq, istk, :]

                hp.gnomview(residue, rot=rot, reso=reso,
                            title='Residue \n {} Stokes parameter'.format(Stokes[istk]), sub=(3, 3, 3 * istk + 3),
                            min=-nsig * sig, max=nsig * sig)

        elif Stokesparameter == 'QU':
            plt.rc('figure', figsize=(16, 10))
            for istk in range(2):
                sig = np.std(map2[ifreq, istk + 1, seenpix])

                hp.gnomview(map1[ifreq, istk + 1, :], rot=rot, reso=reso,
                            title=title1 + '\n {} Stokes parameter'.format(Stokes[istk + 1]), sub=(2, 3, 3 * istk + 1),
                            min=-nsig * sig, max=nsig * sig)
                hp.gnomview(map2[ifreq, istk, :], rot=rot, reso=reso,
                            title=title2 + '\n {} Stokes parameter'.format(Stokes[istk + 1]), sub=(2, 3, 3 * istk + 2),
                            min=-nsig * sig, max=nsig * sig)
                residue = map1[ifreq, istk + 1, :] - map2[ifreq, istk, :]
                hp.gnomview(residue, rot=rot, reso=reso,
                            title='Residue \n {} Stokes parameter'.format(Stokes[istk + 1]), sub=(2, 3, 3 * istk + 3),
                            min=-nsig * sig, max=nsig * sig)
            ...
            for istk in range(1):
                ...
                hp.gnomview(map1[ifreq, istk, :], rot=rot, reso=reso,
                            title=title1 + '\n {} Stokes parameter'.format(Stokes[istk]), sub=(1, 3, 3 * istk + 1),
                            min=-nsig * sig, max=nsig * sig)
                hp.gnomview(map2[ifreq, istk, :], rot=rot, reso=reso,
                            title=title2 + '\n {} Stokes parameter'.format(Stokes[istk]), sub=(1, 3, 3 * istk + 2),
                            min=-nsig * sig, max=nsig * sig)
                residue = map1[ifreq, istk, :] - map2[ifreq, istk, :]
                hp.gnomview(residue, rot=rot, reso=reso,
                            title='Residus \n {} Stokes parameter'.format(Stokes[istk]), sub=(1, 3, 3 * istk + 3),
                            min=-nsig * sig, max=nsig * sig)
    ...
    def diff_2_hist(map1, map2, seenpix, ifreq, label1, label2, ranges=None, bins=100, Stokesparameter='IQU'):
        ...
        if Stokesparameter == 'IQU':
            ...
            for istk in range(3):
                ...
                _, _, _ = plt.hist(map1[ifreq, istk, seenpix], histtype='step', color=color[1], label=label1, bins=bins,
                                   range=ranges[istk])
                _, _, _ = plt.hist(map2[ifreq, istk, seenpix], histtype='step', color=color[0], label=label2, bins=bins,
                                   range=ranges[istk])
            ...
            for istk in range(2):
                plt.subplot(2, 3, istk + 1)
                _, _, _ = plt.hist(map1[ifreq, istk + 1, seenpix], histtype='step', color=color[1], label=label1,
                                   bins=bins, range=ranges[istk + 1])
                _, _, _ = plt.hist(map2[ifreq, istk, seenpix], histtype='step', color=color[0], label=label2, bins=bins,
                                   range=ranges[istk + 1])
        ...
        for istk in range(3):
            for i in range(nb_component):
                hp.gnomview(map_list[i][ifreq, istk, :], rot=rot, reso=reso,
                            sub=(3, nb_component, nb_component * istk + (i + 1)),
