...
plot(nus, vals)
errorbar(nus_s4, sed_s4, yerr=err_sed_s4, xerr=s4bi.s4_config['bandwidth']/2, capsize=4, fmt='ro', label='CMB-S4')
axvline(x=nubreak,ls=':', color='k')
yscale('log')
xscale('log')
xlabel('Frequency [GHz]')
ylabel(r'Spectrum [$\mu K_{CMB}$]')
legend()
...
plot(nus, vals)
errorbar(nus_s4, sed_s4, yerr=err_sed_s4, xerr=s4bi.s4_config['bandwidth']/2, capsize=4, fmt='ro', label='CMB-S4')
errorbar(nus_qp, sed_qp, yerr=err_sed_qp, xerr=qp_config['bandwidth']/2, fmt='bo', capsize=4, label='Bolometric Interferometer')
axvline(x=nubreak,ls=':', color='k')
yscale('log')
xscale('log')
xlabel('Frequency [GHz]')
...
reload(mcmc)
reload(fgbuster)
reload(fgbuster.component_model)


dust_ref_freq = 353.
sync_ref_freq = 70.
dust_model = fgbuster.component_model.Dust(dust_ref_freq, temp=20)
print(dust_model.params)
sync_model = fgbuster.component_model.Synchrotron(sync_ref_freq)
...
ll = mcmc.LogLikelihood(xvals=nus_s4, yvals=sed_s4, errors=err_sed_s4,
        p0=p0, model=model_2beta, fixedpars=fixedpars, flatprior=flatpriors)
nbmc = 5000
samp = ll.run(nbmc)
flat_samples_s4 = samp.get_chain(discard=nbmc//2, thin=32, flat=True)
...
# Same with Loop over some parameters
