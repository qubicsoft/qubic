...
def display_maps(inmaps, bigtitle=None, mytitle='', figsize=(16,10), nsig=3, 
                 rot=None, reso=15, moll=False, add_rms=False):
    ...
    for i in range(nf):
        for j in range(nstk):
            ...
            if moll:
                hp.mollview(maps[i,j,:], sub=(nf,3,3*i+j+1), min = -nsig*ss, max=nsig*ss,
                       title=thetitle)
            else:
                hp.gnomview(maps[i,j,:], sub=(nf,3,3*i+j+1), min = -nsig*ss, max=nsig*ss,
...
```{python}
def apply_fgb(maps, freqs, fwhms, verbose=True, 
    ...
    if truth:
        resid_dust = dustrec - truth[0]
        resid_dust[:,~pixok] = hp.UNSEEN
        resid_cmb = cmbrec - truth[1]
        ...
        if plot_residuals:
            ...
            tight_layout()
    if truth:
        return r.x[0], dustrec, cmbrec, sigs_dust, sigs_cmb
    else:
