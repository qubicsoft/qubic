...
def get_cov_for_weighted(n_freq, depths_i, depths_p, coverage, nside=256):
    ...
    return noise_cov
def ParamCompSep(config, prop, nside, skyconfig, model, noise=True, delta_beta=0.05, fix_temp=None, nubreak=260, x0=[], break_width=1., fixsync=True, iib=1):
    ...
    print()
    comp=get_comp_for_fgb(nu0=85, model=model, fix_temp=fix_temp, x0=x0, bw=break_width, fixsync=fixsync)
...
props=[0, 0.5, 1]#0.1, 0.3, 1]#np.linspace(0, 1, 2)
...
pickle.dump([leff, cl, param_comb, tabseed, db, props, bw, sys.argv], open('/pbs/home/m/mregnier/sps1/QUBIC+/d0/cls/results/cls_iib{:.0f}_QU{}{}_truenub{:.0f}_{}reals_{}.pkl'.format(iib, name_T, name_s, nubreak, N, ite), "wb"))
