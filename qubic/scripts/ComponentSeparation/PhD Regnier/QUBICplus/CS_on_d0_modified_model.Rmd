...
def ParamCompSep(config, nside, skyconfig, model, noise=True, delta_beta=0.05, fix_temp=None):
    ...
    r=fgbuster.basic_comp_sep(comp, instr, inputs[:, :, pixok], method='TNC', options={'maxiter':500})
    print('Done! \n')
    print(r.x)
...
hp.gnomview(s4, rot=center, reso=15, cmap='jet', sub=(2, 5, 4), title='')#,
            #title='True - S4 -> dr : {:.5g}'.format((np.std(s4[pixok])/0.345)**2))
hp.gnomview(bi, rot=center, reso=15, cmap='jet', sub=(2, 5, 5), title='')#,
