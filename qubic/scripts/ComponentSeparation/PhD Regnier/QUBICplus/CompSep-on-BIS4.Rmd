...
```{python}
noise_est_s4=mapS4_noisy[8, istk]-allcomp_s4[0][istk]-allcomp_s4[1][istk]-allcomp_s4[2][istk]
noise_est_bi=mapqp_noisy[30, istk]-allcomp_bi[0][istk]-allcomp_bi[1][istk]-allcomp_bi[2][istk]
```
...
```{python}
comp = [fgb.component_model.Dust_2b(nu0=270., temp=20), 
        fgb.component_model.CMB(), 
        fgb.component_model.Synchrotron(nu0=270, beta_pl=-3)]
...
r_qp=separate(comp, instr, mapqp_noiseless[:, :, pixok])

# CMB-S4
instr = get_instrument('CMBS4')
instr.fwhm = np.ones(9)*ref_fwhm*60
r_s4=separate(comp, instr, mapS4_noiseless[:, :, pixok])
```

```{python}
print('True   -> ', [1.44, 1.94, nubreak, -3])
print('BI-S4 -> ', r_qp.x)
print('CMB-S4 -> ', r_s4.x)
```
...
all_comp_true=s4bi.get_component_maps_from_parameters({'cmb':42, 'dust':'d02b', 'synchrotron':'s0'}, 
                                                      ...
                                                      betapl=-3)
'''
all_comp_s4=s4bi.get_component_maps_from_parameters({'cmb':42, 'dust':'d02b', 'synchrotron':'s0'}, s4_config, 85, 
                                   fsky=0.03, 
                                   nside=256, 
                                   betad0=r_s4.x[0], 
                                   betad1=r_s4.x[1], 
...
all_comp_bi=s4bi.get_component_maps_from_parameters({'cmb':42, 'dust':'d02b', 'synchrotron':'s0'}, qp_config, 85, 
                                   fsky=0.03, 
                                   nside=256, 
                                   betad0=r_qp.x[0], 
                                   betad1=r_qp.x[1], 
...
```{python}
hp.gnomview(mapS4_noiseless[3, 1], rot=center, reso=15, cmap='jet', sub=(1, 3, 1))
hp.gnomview(mapdust_s4_est[1], rot=center, reso=15, cmap='jet', sub=(1, 3, 2))
hp.gnomview(mapdust_bi_est[1], rot=center, reso=15, cmap='jet', sub=(1, 3, 3))
```
...
```{python}
plt.scatter(s4_config['frequency'], np.mean(mapS4_noiseless[:, :, pixok], axis=2)[:, 0])
plt.plot(np.linspace(10, 300, 100), np.linspace(10, 300, 100)**(1.44)/200)
plt.plot(np.linspace(10, 300, 100), np.linspace(10, 300, 100)**(2.64)/80900)
plt.yscale('log')
plt.xscale('log')
```
...
k=1
for i in range(3):
    hp.gnomview(all_comp_true[i][0, istk], rot=center, reso=15, cmap='jet', sub=(3, 3, k), title=comp[i])
    hp.gnomview(all_comp_2beta_s4[i][istk], rot=center, reso=15, cmap='jet', sub=(3, 3, k+3), title=comp[i])
    hp.gnomview(all_comp_2beta_bi[i][istk], rot=center, reso=15, cmap='jet', sub=(3, 3, k+6), title=comp[i])
    k+=1
...
k=1
for i in range(3):
    res_s4=all_comp_2beta_s4[i][istk] - all_comp_true[i][0, istk]
    res_s4[~pixok]=hp.UNSEEN
    
    res_bi=all_comp_2beta_bi[i][istk] - all_comp_true[i][0, istk]
    res_bi[~pixok]=hp.UNSEEN
...
plt.scatter(0, 0, color='red', s=50, label='S4')
plt.scatter(all_comp_2beta_s4[1][1, pixok], all_comp_true[1][0, 1, pixok], color='red', s=5, alpha=1)
...
plt.scatter(0, 0, color='blue', s=50, label='BI')
plt.scatter(all_comp_2beta_bi[1][1, pixok], all_comp_true[1][0, 1, pixok], color='blue', s=10, alpha=1)
plt.plot([-1,2], [-1, 2], '--k', lw=3, label=r'$y=x$')
