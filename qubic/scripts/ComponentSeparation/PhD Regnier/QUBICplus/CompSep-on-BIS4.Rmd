...
```{python}
comp = [fgb.component_model.Dust(nu0=145.), fgb.component_model.CMB(), fgb.component_model.Synchrotron(nu0=145)]
...
mapres_qp[:, pixok] = mapqp_noisy[15,:,pixok].T - r_qp.s[0] - r_qp.s[1] - r_qp.s[2]
resqp=mapres_qp[istk]
resqp[~pixok]=hp.UNSEEN

mapres_s4 = np.ones((3, 12*256**2))*hp.UNSEEN
mapres_s4[:, pixok] = mapS4_noisy[5,:,pixok].T - r_s4.s[0] - r_s4.s[1] - r_s4.s[2]
resS4=mapres_s4[istk]
resS4[~pixok]=hp.UNSEEN


plt.figure(figsize=(12, 10))
hp.gnomview(resS4, rot=center, reso=15, sub=(3, 2, 1), title='Residuals for CMB-S4', 
                                                                    cmap='jet')
hp.gnomview(resqp, rot=center, reso=15, sub=(3, 2, 2), title='Residuals for BI', 
                                                                    cmap='jet')

hp.gnomview(resqp - noiseqp[15, istk], rot=center, reso=15, sub=(3, 2, 6), title='BI diff', 
                                                                    cmap='jet')

hp.gnomview(noiseS4[5, istk], rot=center, reso=15, sub=(3, 2, 3), title='True noise for CMB-S4', 
                                                                    cmap='jet')
hp.gnomview(noiseqp[15, istk], rot=center, reso=15, sub=(3, 2, 4), title='True noise for BI', 
                                                                    cmap='jet')

hp.gnomview(resS4 - noiseS4[5, istk], rot=center, reso=15, sub=(3, 2, 5), title='S4 diff', 
                                                                    cmap='jet')
plt.show()
```
...
plt.subplot(1, 2, 1)
plt.scatter(resS4[pixok], noiseS4[5, istk, pixok], s=1)
plt.plot([-1,1], [-1, 1], '-r')
plt.subplot(1, 2, 2)
plt.scatter(resqp[pixok], noiseqp[15, istk, pixok], s=1)
plt.plot([-2,2], [-2, 2], '-r')
plt.show()
```
...
```{python}
comp = [fgb.component_model.Dust_2b(nu0=145.), 
        fgb.component_model.CMB(), 
        fgb.component_model.Synchrotron(nu0=145)]

comp[0].defaults=[1., 1., 200, 20]
...
all_comp_true=s4bi.get_component_maps_from_parameters({'cmb':42, 'dust':'d02b', 'synchrotron':'s0'}, s4_config, 145, 
                                   fsky=0.03, 
                                   nside=256, 
                                   betad0=1.34, 
                                   betad1=1.74, 
...
all_comp_qp=s4bi.get_component_maps_from_parameters({'cmb':42, 'dust':'d02b', 'synchrotron':'s0'}, qp_config, 145, 
                                   fsky=0.03, 
                                   nside=256, 
                                   betad0=r_qp.x[0], 
                                   betad1=r_qp.x[1], 
...
plt.figure(figsize=(12, 10))
hp.gnomview(all_comp_true[comp][0, istk], rot=center, reso=15, sub=(2, 3, 1), cmap='jet', min=np.min(all_comp_true[comp][0, istk, pixok]), 
            max=np.max(all_comp_true[comp][0, istk, pixok]),
            title='True dust map - {} - (1)'.format(stk[istk]), unit=r'$\mu K_{CMB}$')
hp.gnomview(all_comp_s4[comp][0, istk], rot=center, reso=15, sub=(2, 3, 2), cmap='jet', 
            title='Reconstructed by CMB-S4 - {} - (2)'.format(stk[istk]), min=np.min(all_comp_true[comp][0, istk, pixok]), 
            max=np.max(all_comp_true[comp][0, istk, pixok]), unit=r'$\mu K_{CMB}$')
hp.gnomview(all_comp_qp[comp][0, istk], rot=center, reso=15, sub=(2, 3, 3), cmap='jet', 
            title='Reconstructed by BI-S4 - {} - (3)'.format(stk[istk]), min=np.min(all_comp_true[comp][0, istk, pixok]), 
            max=np.max(all_comp_true[comp][0, istk, pixok]), unit=r'$\mu K_{CMB}$')

res_truetrue=all_comp_true[comp][0, istk]-all_comp_true[comp][0, istk]
res_s4true=all_comp_s4[comp][0, istk]-all_comp_true[comp][0, istk]
res_bitrue=all_comp_qp[comp][0, istk]-all_comp_true[comp][0, istk]
...
hp.gnomview(res_s4true, rot=center, reso=15, sub=(2, 3, 5), cmap='jet', 
            min=-np.std(all_comp_true[comp][0, istk, pixok]), 
            max=np.std(all_comp_true[comp][0, istk, pixok]), title='(1) - (2)', unit=r'$\mu K_{CMB}$')
hp.gnomview(res_bitrue, rot=center, reso=15, sub=(2, 3, 6), cmap='jet', 
            min=-np.std(all_comp_true[comp][0, istk, pixok]), 
            max=np.std(all_comp_true[comp][0, istk, pixok]), title='(1) - (3)', unit=r'$\mu K_{CMB}$')
plt.show()
```

```{python}
noises4 = mapS4_noisy[5] - all_comp_s4[0][0] - all_comp_s4[1][0] - all_comp_s4[2][0]
noiseqp = mapqp_noisy[15] - all_comp_qp[0][0] - all_comp_qp[1][0] - all_comp_qp[2][0]
```

```{python}
hp.gnomview(noises4[1], rot=center, reso=15, cmap='jet', sub=(1, 2, 1), title='CMB-S4')
hp.gnomview(noiseqp[1], rot=center, reso=15, cmap='jet', sub=(1, 2, 2), title='BI')
```
