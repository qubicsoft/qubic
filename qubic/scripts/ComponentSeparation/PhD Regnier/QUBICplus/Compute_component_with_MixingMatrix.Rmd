...
```{python}
def get_maps(skyconfig, covmap, beta=[1.44, 1.64, 242, 145]):
...
```{python}
mean_dust=np.mean(map_dust_s4_noiseless[:, 0, pixok], axis=1)
mean_dust_nu0=np.mean(map_dust_s4_noiseless[5, 0, pixok], axis=0)

mean_cmb=np.mean(map_cmb_s4_noiseless[:, 0, pixok], axis=1)
mean_cmb_nu0=np.mean(map_cmb_s4_noiseless[5, 0, pixok], axis=0)

mean_allmaps = np.mean(map_cmbdust_s4_noiseless[:, 0, pixok], axis=1)
mean_allmaps_nu0 = np.mean(map_cmbdust_s4_noiseless[5, 0, pixok], axis=0)





mean_dust_bi=np.mean(map_dust_bi_noiseless[:, 0, pixok], axis=1)
mean_dust_bi_nu0=np.mean(map_dust_bi_noiseless[15, 0, pixok], axis=0)

mean_cmb_bi=np.mean(map_cmb_bi_noiseless[:, 0, pixok], axis=1)
mean_cmb_bi_nu0=np.mean(map_cmb_bi_noiseless[15, 0, pixok], axis=0)

mean_allmaps_bi = np.mean(map_cmbdust_bi_noiseless[:, 0, pixok], axis=1)
mean_allmaps_bi_nu0 = np.mean(map_cmbdust_bi_noiseless[15, 0, pixok], axis=0)
```

Here we define 2 functions, the first is to get dictionary results of component separation and the second to get plot of maps. In the component separation definition, we see that we define others defaults parameters. Indeed, $x_0$ is the parameter for the first step of Likelihood maximization but the defaults avlues are ${beta}^0_d = 1.44$, ${beta}^1_d = 1.64$, $\nu_{break}=200$ GHz and $T=20$ K. Those values are near to the real value, to test the method, we define other set parameters to see if the algorithm converge to the same at the end. 

```{python}
def get_components_from_fgb(inputs, name, pixok):
...
```{python}
rs4 = get_components_from_fgb(map_cmbdust_s4, 'CMBS4', pixok)
```

```{python}
rbi = get_components_from_fgb(map_cmbdust_bi, 'Qubic+', pixok)
```
...
```{python}
#plot_after_compsep(map_dust_s4_noiseless[0], rs4.s[0], rbi.s[0])
```
...
```{python}
comp=[fgbuster.component_model.Dust_2b(nu0=145, temp=20, nubreak=242), 
      fgbuster.component_model.CMB()]
...
#A_maxL_bi = A_ev_bi(rbi.x[[0, 1]])
#A_maxL_s4 = A_ev_s4(rs4.x[[0, 1]])
A_maxL_true = A_ev_bi([1.44, 1.64])
```

```{python}
nus=np.linspace(20, 300, 1000)
ind=np.where(nus > 145)[0][0]
dust_model_2b = fgbuster.component_model.Dust_2b(nu0=145, beta_d0=1.44, beta_d1=1.64, nubreak=242, temp=20)
...
ax.plot(nus, sed_dust_modified*mean_dust_nu0)
ax.plot(nus, sed_cmb*mean_cmb_nu0)
ax.plot(nus, sed_cmb*mean_cmb_nu0+sed_dust_modified*mean_dust_nu0, ':k')

#ax.scatter(qp_config['frequency'], (A_maxL_bi[:, 0] + A_maxL_bi[:, 1])*mean_allmaps_nu0, s=30, marker='x', label=r'$A^{BI}_{max \mathcal{L}}$')
#ax.scatter(s4_config['frequency'], (A_maxL_s4[:, 0] + A_maxL_s4[:, 1])*mean_allmaps_nu0, s=30, marker='x', label=r'$A^{S4}_{max \mathcal{L}}$')
ax.scatter(qp_config['frequency'], (A_maxL_true[:, 0])*mean_dust_nu0/sed_dust[ind], s=30, marker='d', label=r'$A^{True value}_{max \mathcal{L}}$')
ax.scatter(qp_config['frequency'], (A_maxL_true[:, 1])*mean_cmb_nu0, s=30, marker='d', label=r'$A^{True value}_{max \mathcal{L}}$')
ax.scatter(qp_config['frequency'], (A_maxL_true[:, 1])*mean_cmb_nu0+(A_maxL_true[:, 0])*mean_dust_nu0, s=30, marker='d', label=r'$A^{True value}_{max \mathcal{L}}$')


ax.scatter(qp_config['frequency'], mean_allmaps_bi)
ax.scatter(s4_config['frequency'], mean_allmaps)


ax.set_yscale('log')
ax.set_xscale('log')
ax.set_xlabel('Frequency [GHz]', fontsize=20)
ax.set_ylabel(r'SED [$\mu$K]', fontsize=20)

#ax.legend(fontsize=20, frameon=False)
ax.axvline(145, ls='--', color='black', alpha=0.5)
...
def compute_rdots(comp, r, instr, inputs):
...
mysolution_s4=compute_rdots(comp, rs4.x[[0, 1, 2]], instr_s4, map_cmbdust_s4_noiseless)
mysolution_bi=compute_rdots(comp, rbi.x[[0, 1, 2]], instr_bi, map_cmbdust_bi_noiseless)
```

```{python}
plot_1stk(map_dust_s4_noiseless[5], mysolution_s4[0], mysolution_bi[0], 1)
#plot_1stk(map_cmbdust_bi_noiseless[15], mysolution_s4[0]+mysolution_s4[1], mysolution_bi[0]+mysolution_bi[1], 1)
```
