...
name=str(sys.argv[7])
Alens=float(sys.argv[8])

myconf=get_list_config(name=name, nsub=number_of_subbands)
config=combine_config(myconf)
print(config)

if config['fsky'] == 0.02 or config['fsky'] == 0.1:
    radec = [-30, 320]
else:
    radec = [-30, -30]

object=forecast_tools.ForecastMC(config, r=r, Alens=Alens, radec=radec)  # S4 -> [-30, -30]   BK -> [-30, 320]
...
leff, clBB, like, maxL, sigma_r, CL95, cmb_est, dust_est, seed, cl_fg, index_est = object.RUN_MC(N, dust_model=dust_model, dict_dust=presets_dust, nside_fgb=nside_out)
...
print(np.mean(clBB, axis=0))
print()
print('dl true -> ', dltrue)

print()
print()
print('r = {:.3e} +/- {:.3e}'.format(maxL, sigma_r))
print()
print('sigma(r) : {:.3e}'.format(sigma_r - maxL))
print('95% C.L. : {:.3e}'.format(CL95))
...
output = open('/pbs/home/m/mregnier/sps1/QUBIC+/forecast_decorrelation/results/'+
                'forecast_r{:.3f}_Alens{:.1f}_{}_model{}_decorrelation{}_nfsub{}_{}reals.pkl'.format(r, Alens, name, dust_model, correlation_length, number_of_subbands, N), 'wb')
