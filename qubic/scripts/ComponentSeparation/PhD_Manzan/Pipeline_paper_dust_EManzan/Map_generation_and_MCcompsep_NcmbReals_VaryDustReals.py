...
#file_name = '{}_template_Comp'.format(call_number)
...
#---------------------------------------------------
print()
print('#### SKY AND INSTRUMENT DEFINITION ####')
...
for i in range(num_of_instrum):
    ...
    instrum_sky_maps, _ = instrum_class[i].get_fg_maps(N_SAMPLE_BAND=N_sample_band, Nside_patch=Nside_fit, verbose=True, temp_is_fixed = temp_is_fixed)
...
#comp_maps = np.zeros( (N, num_of_instrum, 2, n_comp, n_stk, npix) )
...
for n in range(N):
    ...
    for i in range(num_of_instrum):
        ...
        sky_maps, _, cmb_maps = instrum_class[i].get_sky_maps(N_SAMPLE_BAND=N_sample_band, Nside_patch=Nside_fit, same_resol=0, verbose=True, coverage=False, noise=False) 
        for j in range(2):
            #def input map: cmb, fg, noise
            input_map = sky_maps.copy() # cmb and fg maps
            ...
            cmb_input_maps[n,i,j,:,:] = cmb_maps[1:,pixok]#.T
            ...
            savefig('./Delta_maps/Delta_cmb_map_'+file_name+'_seed{}_{}_nsidefit{}_Nbpintegr{}_iter{}of{}.png'.format(seed_vec[n], instrum_config[i]['name'],  Nside_fit, N_sample_band, n+1, N))            
            ...
            #save reconstructed component maps
            comp_map = result.s.copy()
        ...
        print('Estimated Dls: ', Dls[n,i,:,:])
        #print('Noise Dls: ', Dls_noise[n,:,:])
        
        # Save results in pkl file
        print('Saving results to file... \n')
        pickle.dump([leff, Dls[:,i,:,:], seed_vec],
                    open('./results/cls_VaryingCMBandFG_'+file_name+'_{}_nsidefit{}_Nbpintegr{}_iter{}.pkl'.format(instrum_config[i]['name'], Nside_fit, N_sample_band, N), "wb"))
        pickle.dump([cmb_rec_maps[:,i,:,:,:]], open('./results/CMB_outmap_VaryingCMBandFG_'+file_name+'_{}_nsidefit{}_Nbpintegr{}_iter{}.pkl'.format(instrum_config[i]['name'], Nside_fit, N_sample_band, N), "wb"))
        pickle.dump([cmb_input_maps[:,i,:,:,:]], open('./results/CMB_inmap_VaryingCMBandFG_'+file_name+'_{}_nsidefit{}_Nbpintegr{}_iter{}.pkl'.format(instrum_config[i]['name'], Nside_fit, N_sample_band, N), "wb"))
        pickle.dump([param_maps[:,i]], open('./results/params_VaryingCMBandFG_'+file_name+'_{}_nsidefit{}_Nbpintegr{}_iter{}.pkl'.format(instrum_config[i]['name'], Nside_fit, N_sample_band, N), "wb"))        
