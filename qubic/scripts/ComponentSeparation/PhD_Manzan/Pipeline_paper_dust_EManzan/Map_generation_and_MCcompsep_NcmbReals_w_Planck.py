...
name_param_file = str(sys.argv[2]) # Name of the parameter_file containing all the simulation settings
...
# save all the simulation settings
...
save_figs = eval(d['save_fig']) # Bool value. If true, save plots of residual cmb and parameter maps
...
n_stk = d['n_stk'] # number of stokes parameters for COMPSEP: if n_stk = 2, the compsep is on Q and U, not I. Otherwise n_stk = 3. The input maps (see later) will have 3 stokes components in any case
stk = d['stk'] # stokes quantities name (e.g. I,Q,U or Q,U)
...
Nside_fit = int(d['Nside_fit']) # If the foregrounds have spatially varying parameters, then FGB will estimate the parameters on sky patches with this Nside. Put it to zero if you want the foreground parameters to be scalars

#Specify which instrument you want to simulate
sim_S4 = eval(d['sim_S4']) # Boolean
sim_BI = eval(d['sim_BI']) # Boolean
n_subbands = d['n_subbands'] # list with number of sub-bands to simulate for each BI case (e.g. BI3+BI5+BI7 will be [3,5,7])
tot_n_subs_to_sim = len(n_subbands) # total number of BI configurations 

#Specify which sky you want to simulate
add_noise = eval(d['add_noise']) # If True, sim. is with Noise (added in MC loop)
spectra_type = d['spectra_type'] # cross or auto. This is the type of CMB Dls that will be computed. If cross, the noise depth will be increased by sqrt(2)
...
temp = d['temp']
Add_Planck = eval(d['Add_Planck']) # if True, add high-nu Planck maps and consider dust T as a parameter
...
file_name = '{}_template_patchs4_Comp'.format(call_number)
#file_name = '{}_template_Planck{}_Comp'.format(call_number, Add_Planck)
...
mukarcmin_P = 0.5*(mukarcmin_EE + mukarcmin_BB)
...
if sim_S4: 
    ...
    #print(s4_config['depth_p'])

#define the pixok set
if Add_Planck == True:
    covmap = hp.fitsfunc.read_map('./Planck_data/S4_plus_points_mask.fits', field=[0], h=False)
else:
...
n_par = utilitylib.get_num_params(skyconfig, temp_is_fixed=temp_is_fixed) #number of params
params = utilitylib.get_name_params(n_par, temp_is_fixed=temp_is_fixed) #name of params
param_maps_th = np.zeros( (n_par, hp.nside2npix(Nside_fit)) )
        ...
        for m_idx, m in enumerate(preset_strings):
            if 'd' in m: #it's dust
                ...
                if temp_is_fixed:
                    ...
                    try:
                        param_maps_th[0,:] = hp.ud_grade(sky.components[m_idx].mbb_index.value, Nside_fit) * sky.components[m_idx].mbb_index.unit
                        print('Save beta dust parameters')
                    except:
                         print('Skip saving dust parameters because dust is d12 or d5 or d7')
                else:
                    ...
                    try:
                        ...
                        for spectral_param in [sky.components[m_idx].mbb_index, sky.components[m_idx].mbb_temperature]:
                            param_maps_th[k,:] = hp.ud_grade(spectral_param.value, Nside_fit) * spectral_param.unit
                            k+=1
                        print('Save beta and T dust parameters')
                    except:
                         print('Skip saving dust parameters because dust is d12 or d5 or d7')                            
                    #pass                
            ...
            if 's' in m: #it's synch
                ...
                print('Save synch parameter')
                param_maps_th[-1,:] = hp.ud_grade(sky.components[m_idx].pl_index.value, Nside_fit) * sky.components[m_idx].pl_index.unit
...
for i in range(num_of_instrum):
    ...
    instrum_class.append(qubicplus_skygen_lib.BImaps(skyconfig, sky, covmap, instrum_config[i], r, Alens, corr_l, nside=nside, spectra_type=spectra_type)) 
    #skyconfig has only fg at this level

#create fg noiseless maps
sky_maps = []
for i in range(num_of_instrum):
    print()
...
# save Planck HM maps
if Add_Planck == True:
    print('Importing Planck data...')
    HM_Planck_maps = []
    for i in range(2):
        inputfile = './Planck_data/HFI_SkyMap_353-psb_{}_R3.01_halfmission-{}.fits'.format(nside, i+1)
        maps_tmp = hp.fitsfunc.read_map(inputfile, field=[0,1,2], h=False)
        print(maps_tmp.shape)
        HM_Planck_maps.append(maps_tmp[np.newaxis, :, :])
        print(HM_Planck_maps[i].shape)
    print('...Done')
#---------------------------------------------------
print()
print('#### BINNING ####')
...
fgb_instrum = []
for i in range(num_of_instrum):
    fgb_instrum.append(utilitylib.configure_fgb_instrument(instrum_config[i], N_sample_band, Add_Planck)) #define instrument for fgb

options={'maxiter': 100000} #additional comp.sep. options
A_matrix = fgbuster.mixingmatrix.MixingMatrix(*comp)
# tol=1e-18
...
npixok = int(np.sum(covmap)) #int(npix*fsky) # -1 
...
for n in range(N):
    ...
    for i in range(num_of_instrum):
        ...
        for j in range(2):
            ...
            input_map_tmp = sky_maps[i].copy() #fg maps
            input_map_tmp += cmb_maps
            #create noise realization
            if add_noise:
                print('Adding noise to the input maps...')
                noise_map = instrum_class[i].gen_noise_maps(coverage=False)
                input_map_tmp += noise_map
            #add Planck data if required
            if Add_Planck == True:
                print('Compsep with addition of Planck data')
                input_map = np.concatenate((input_map_tmp, HM_Planck_maps[j]), axis=0) #[Nfreq +1, Nstk, Npix]
                print(input_map.shape)
            else:
                input_map = input_map_tmp.copy()
            ...
            result = fgbuster.basic_comp_sep(comp, fgb_instrum[i], data, nside=Nside_fit, options=options, tol=1e-18, method='TNC') 
            ...
            residual_maps_baseline[:, pixok] = cmb_rec_maps[n,i,j,:,:] - cmb_input_maps[n,i,j,:,:] #tot_residual_maps[n,i,j,:,:]
            if save_figs == True:
                #save global residual map image
                plotfilename = './Delta_maps/Delta_cmb_map_'+file_name+'_{}_seed{}_{}_nsidefit{}_Nbpintegr{}_iter{}of{}.png'.format(j+1,seed_vec[n], instrum_config[i]['name'],  Nside_fit, N_sample_band, n+1, N)
                utilitylib.plot_and_save_cmb_residual_maps(n_stk, residual_maps_baseline, center, instrum_config[i], stk, plotfilename)
                
            #save params
            if n_par > 0 :
                if Nside_fit != 0: #spatially varying parameters
                    param_maps[n,i,j,:,:] = result.x
                    #save residual map image
                    if save_figs == True:
                        plotfilename = './Delta_maps/Delta_params_map_'+file_name+'_{}_seed{}_{}_nsidefit{}_Nbpintegr{}_iter{}of{}.png'.format(j+1, seed_vec[n],instrum_config[i]['name'],  Nside_fit, N_sample_band, n+1, N)
                        utilitylib.plot_and_save_param_residual_maps(n_par, param_maps_th, param_maps[n,i,j,:,:], center, instrum_config[i], params, plotfilename)
                    #upgrade params to nside of the input maps to eval mixing matrix
                    param_maps_hd = hp.ud_grade(result.x, nside)
                    print('HD param maps: ', param_maps_hd.shape)
                    #eval mixing matrix
                    A_beta = A_matrix.eval(instrum_config[i]['frequency'], *param_maps_hd[:,pixok]) #matrix [Npixok, Nf, Ncomp]
                    print('Mixing matrix: ', A_beta.shape)
                    #print(A_beta)
                    #eval chi square map
                    chisq = utilitylib.eval_redu_chi_square_map(n_stk, npix, pixok, instrum_config[i], n_comp, n_par, A_beta, recons_maps, input_map, skyconfig, j, center, save_figs)
                    if save_figs == True:
                        plotfilename = file_name+'_{}_seed{}_{}_nsidefit{}_Nbpintegr{}_iter{}of{}.png'.format(j+1, seed_vec[n],instrum_config[i]['name'],  Nside_fit, N_sample_band, n+1, N)
                        utilitylib.plot_and_save_chisquare_maps(n_stk, chisq, center, instrum_config[i], stk, './Delta_maps/Chisquare_map_'+plotfilename)
                        utilitylib.plot_and_save_chisquare_hist(n_stk, chisq, pixok, instrum_config[i], stk, './Delta_maps/Chisquare_hist_'+plotfilename)
                        
                else: #spatially constant parameters
                    param_maps[n,i,j,:] = result.x
                    print(result.x)
                    #eval mixing matrix
                    A_beta = A_matrix.eval(instrum_config[i]['frequency'], *param_maps[n,i,j,:]) #matrix [Nf, Ncomp]
                    print('Mixing matrix: ', A_beta.shape)
                    #print(A_beta)
                    #eval chi square map
                    chisq = utilitylib.eval_redu_chi_square_map(n_stk, npix, pixok, instrum_config[i], n_comp, n_par, A_beta, recons_maps, input_map, skyconfig, j, center, save_figs)
        ...
        DlsBB_theoretic_r = likelihoodlib.get_DlsBB_from_CAMB(leff, r, Alens, nside=nside, coverage=covmap, cov_cut=0.1, lmin=lmin,
                                                              delta_ell=delta_l, lmax = lmax) 
...
'''
    ...
    #save global residual map at nu0
    freq_idx = np.where(instrum_config[i]['frequency']==float(nu0))[0][0]
    print('freq idx = ', freq_idx)

    for k in range(n_comp):
        tot_residual_maps[n,i,j,:,:] += recons_maps[k,:,pixok].T

    tot_residual_maps[n,i,j,:,:] -= input_map[freq_idx,1:,pixok].T #.copy()
    tot_input_maps[n,i,j,:,:] = input_map[freq_idx,1:,pixok].T

    tot_residual_maps_baseline = np.zeros( (n_stk, npix) )
    tot_residual_maps_baseline[:,~pixok] = hp.UNSEEN
    tot_residual_maps_baseline[:, pixok] = tot_residual_maps[n,i,j,:,:]
    #save global residual map image
    figure(figsize=(16,10))
    for i_stk in range(n_stk):
        hp.gnomview(tot_residual_maps_baseline[i_stk,:], rot=center,
                    reso=15, title=instrum_config[i]['name']+': Tot. residual '+stk[i_stk],
                    norm='hist', sub=(1,n_stk,i_stk+1))
    savefig('./Delta_maps/Delta_global_map_testnoise6_'+file_name+'_seed{}_{}_nsidefit{}_Nbpintegr{}_iter{}of{}.png'.format(seed_vec[n],
                                                                                                                            instrum_config[i]['name'],
