...
def create_noisemaps(nus, nside, depth_i, depth_p, npix, spectra_type='cross'):
    ...
    for ind_nu, nu in enumerate(nus):
        ...
        elif spectra_type == 'cross':
            #generate noise for all pixels from normal distrib with sigma=noise/pixel. Multiply by sqrt(2)
            #because we will take the cross-spectra: we assume we split time acquisition in two and create two maps to use to
            #calc. cross-spectra. Those maps have sqrt(2) higher noise level
            N[ind_nu, 0] = np.random.normal(0, 1., npix)*sig_i#*np.sqrt(2)
            N[ind_nu, 1] = np.random.normal(0, 1., npix)*sig_p#*2 #Multiply by: sqrt(2) because from P to Q and U
            N[ind_nu, 2] = np.random.normal(0, 1., npix)*sig_p#*2
            
        else:
            print('ERROR: specify the spectra type to set noise level!')
            return
        '''
...
class BImaps(object):
    ...
    def get_cmb_from_r_Alens(self):
        ...
        if self.save_figs:
            ...
            for i_stk in range(3):
                hp.gnomview(maps[i_stk,:], rot=center, reso=15, title='Input CMB '+stk[i_stk], norm='hist', sub=(1,3,i_stk+1))
    ...
    def get_sky_maps(self, N_SAMPLE_BAND=100, Nside_patch=0, same_resol=None, verbose=False, coverage=False, noise=False):
        ...
        for i in self.skyconfig.keys():
            if i == 'cmb':
                cmbmap = self.get_cmb_from_r_Alens() #self.get_cmb() #self.get_cmb_from_r_Alens() #get_cmb_from_r_Alens() 
