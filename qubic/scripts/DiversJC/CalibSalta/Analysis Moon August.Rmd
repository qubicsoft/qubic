...
# Analysis of the Moon Scan data from August 2022
### JCH - Aug. 2022
...
```{python}
# 2022-08-10_22.13.55__MoonScan_Speed_VE14
mydatadir = '/Volumes/HD JCH SSD/Qubic/QubicData/Calib-TD/'
thedate = '2022-08-11'
thedirs = glob.glob(mydatadir + '/' + thedate + '/*MoonKen*')
thedata = thedirs[0]
print(thedata)
### Read data
a = qubicfp()
...
dt0 = datetime.strptime('22-08-11 20:14:58', '%y-%m-%d %H:%M:%S')
print(dt0)
...
for i in range(len(allTESNum)):
    ...
    if (i%(n0**2))==0: figure()
    hp.gnomview(allmaps[i,:]-tdt.get_mode(allmaps[i,ok]), reso=10, sub=(n0,n0,(i%(n0**2))+1), min=-5e3, max=2.5e4, title=allTESNum[i], rot=[np.mean(newazt), np.mean(newelt)])
```
