...
azqubic = 120
newazt = (azt + azqubic - azmoon)
newelt = -(elt - elmoon)+40
...
reso = 10.
xs = 201
hp.gnomview(allmaps[idx,:], reso=reso, rot=[0, 0], xsize=xs, title='TES#{}'.format(mynum), min=-5e3, max=2.5e4, sub=(1,4,1))
...
badpix = mm==hp.UNSEEN
mm[badpix] = 0
# rot=np.array([1.-3.22, -5+4.44]) reso=7 => 30
# rot=np.array([1., -5]) reso=3 => 50
mapxy = hp.gnomview(mm, reso=reso, rot=np.array([1., -4]), return_projected_map=True, xsize=xs, no_plot=True).data
...
print(maxx, maxy)
plot(maxx,maxy,'r+', ms=30)
guess = np.array([np.max(mapxy),maxx, maxy, 0.])


errpix = xx*0+ss
errpix[mapxy==0] *= 1e5
resfit = ft.do_minuit(np.ravel(xx), np.ravel(mapxy), np.ravel(errpix), guess, 
...
for i in range(len(allTESNum)):
    ...
    mm = allmaps[idx,:].copy()
    mm[mm==hp.UNSEEN] = 0
    mapxy = hp.gnomview(mm, reso=reso, rot=[1., -4], return_projected_map=True, xsize=xs, no_plot=True).data
    ...
    maxy = np.mean(yy[maxi])
    if doplot:
        axvline(x=maxx, color='r', ls='--', label='Input')
        axhline(y=maxy, color='r', ls='--')
    guess = np.array([np.max(mapxy),maxx, maxy, 0.])

    errpix = xx*0+ss
    errpix[mapxy==0] *= 1e5
    resfit = ft.do_minuit(np.ravel(xx), np.ravel(mapxy), np.ravel(errpix), guess, verbose=False,
                          functname=gauss2d, extra_args=[xx,yy], rangepars = [[0.,1e8], [maxx-3, maxx+3], [maxy-3, maxy+3], [0., 10.]])

    fitted = np.reshape(gauss2d(x, resfit[1], extra_args=[xx,yy]), (xs, xs))
    allFWHM[i] = resfit[1][3]*2.35
    allxy[i,:] = resfit[1][1:3]
    print('TES#{}: FWHM = {}'.format(mynum, resfit[1][3]*2.35))
    if doplot:
        axvline(x=resfit[1][1], color='g', ls='--', label='Fitted')
        axhline(y=resfit[1][2], color='g', ls='--')
        subplot(1,3,2)
        imshow(fitted, origin='lower', extent=[np.min(x), np.max(x), np.min(y), np.max(y)], vmin=mm-ss, vmax=np.max(mapxy))
        xlabel('Degrees')
        ylabel('Degrees')
...
```{python}
a = hist(allFWHM,bins=20, range=[0.5,1.5])
print(a)
xx = 0.5*(a[1][:-1]+a[1][1:])
yy = a[0]
...
# Offsets
there is an inversion somewhere between az and el...
...
```{python}
ok = (allFWHM > 0.8) & (allFWHM < 1.2)
mytesn = np.array(allTESNum)[ok]
...
plot(allxy[ok,0], allxy[ok,1], 'ro', label='Moon peaks')
legend()

maz, saz = ft.meancut(allxy[ok,0] - xycreidhe[:,1], 3)
mel, sel = ft.meancut(allxy[ok,1] - xycreidhe[:,0], 3)
