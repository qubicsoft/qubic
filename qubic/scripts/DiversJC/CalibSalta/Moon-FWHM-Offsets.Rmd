...
def fit_one_tes(mymap, xs, reso, rot=np.array([0., 0.]), doplot=False, verbose=False, guess=None, distok=3, mynum=None):
    ...
    if doplot:
        plot(resfit[1][1], resfit[1][2], 'rx', ms=30, mew=2, label='Fit')
        legend()
...
for i in range(len(allTESNum)):
    ...
    print('TES#{0}: FWHM = {1:5.2f}'.format(i, resfit[1][3]*2.35))
...
def rot_trans_scale_pts(x, pars, extra_args=None):
...
moon_guess = np.reshape(rot_trans_scale_pts(np.ravel(newcreidhe), resfit[1]), (256,2))
...
yy = a[0]
notzero = a[0] != 0
resfit = ft.do_minuit(xx[notzero], yy[notzero], np.sqrt(yy[notzero]), guess, verbose=False,
                          functname=gauss)
plot(xxx, gauss(xxx, resfit[1]), label='$\Delta_{az}^{Peak}$'+' = {0:5.2g} +/- {1:5.2g}'.format(resfit[1][1], resfit[1][2]), lw=3)
legend()
...
resfit = ft.do_minuit(xx[notzero], yy[notzero], np.sqrt(yy[notzero]), guess, verbose=False,
                          functname=gauss)
plot(xxx, gauss(xxx, resfit[1]), label='$\Delta_{El}^{Peak}$'+' = {0:5.2g} +/- {1:5.2g}'.format(resfit[1][1], resfit[1][2]), lw=3)
legend()
...
resfit = ft.do_minuit(xx[notzero], yy[notzero], np.sqrt(yy[notzero]), guess, verbose=False,
                          functname=gauss)
plot(xxx, gauss(xxx, resfit[1]), label='$\Delta_{tot}^{Peak}$'+' = {0:5.2g} +/- {1:5.2g}'.format(resfit[1][1], resfit[1][2]), lw=3)
legend()
...
for i in range(256):
    text(mynewrot[i,0]+dec, mynewrot[i,1]+dec, '{}'.format(i+1), fontsize=8)
xlabel('$\Delta_{az}$ [deg.]')
ylabel('$\Delta_{el}$ [deg.]')
legend()
