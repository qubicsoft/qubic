...
for day in days:
    ...
    el_min = 30.
    el_max = 50.
    n_elevations = int(d['duration']*3600/backforthdt) * 0 + 170
    el_step = np.floor((el_max - el_min) / n_elevations * 100) / 100
    ...
    plot(p.azimuth, p.elevation, alpha=0.5, 
         label = '{} scans with Δaz = {} deg, Δel = {} deg el_min={} deg, el_max={} deg, az_center={:5.1f} deg'.format(n_elevations, 
                        d['delta_az'], el_step, el_min, el_max, d['fix_azimuth']['az']))
    ...
    fill_between([door_az-az_span/2-15, door_az+az_span/2-15], 
    ...
    if save_html:
        ...
        Html_file.write('<h4>  - Delta_el = {} deg </h4>\n'.format(el_step))
        Html_file.write('<h4>  - el_min={} deg, el_max={} deg </h4>'.format(el_min, el_max))
        ...
        print(' * $\Delta$el = {} deg'.format(el_step))
        print(' * el_min={} deg, el_max={} deg'.format(el_min, el_max))
