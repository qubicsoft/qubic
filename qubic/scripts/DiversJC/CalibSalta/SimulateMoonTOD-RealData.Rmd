...
# Display Results
hp.gnomview(mapsb, reso=4, rot=[-deltaph, -deltath], sub=(1,2,1), title='Az/El wrt Moon [QP #{} ; QS #{}]'.format(TESNum, indexQS+1))


### Centered on L.O.S
dth_tes = np.degrees(np.pi/2-newths[indexQS])
dph_tes = np.degrees(newphs[indexQS]-np.pi)
hp.gnomview(mapsb, reso=5, rot=[-deltaph-dph_tes , -deltath-dth_tes], sub=(1,2,2), title='L.O.S. centered [QP #{} ; QS #{}]'.format(TESNum, indexQS+1))
hp.graticule()
