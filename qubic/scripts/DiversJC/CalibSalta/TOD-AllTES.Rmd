...
ok = (idx > 1.5e6) & (idx < 2e6+10000)
#ok = (idx > 299000) & (idx < 305000)


dd = -alltod[numTES-1,:][ok]
mytt = tt[ok]

newdd, flags = tdt.jumps_correction(dd, threshold=15, size_haar=51, doplot=False, verbose=False, method='lin')

figure()
idx = np.arange(len(dd))
subplot(2,1,1)
plot(idx, dd, label='Data')
plot(idx[flags>0], dd[flags>0], 'r.', label='Flagged bad')
legend()
xlabel('Samples')
ylabel('ADU')
title('Raw TOD')
