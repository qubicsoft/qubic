...
```{python code_folding=c(), editable=FALSE, deletable=FALSE, run_control={'frozen': True}, eval=FALSE}
...
# Now let's put unknown inter-calibrations
...
plot(xpix,
     mymap - truey,
     label='Full Minuit Residuals σ={0:5.3g}'.format(np.std(mymap - truey)),
     alpha=0.5)
xlabel('pixel')
ylabel('sky')
legend(fontsize=10)
...
subplot(2, 3, 5)
plot(myintercals - intercal,
     label='Full Minuit Residuals σ={0:5.3g}'.format(
...
# Next exercise: Fit inter-calibration separately as they are linear unknowns
...
class simulate_signal_intercal:
    ...
    def give_intercals(self, sky):
        ...
        intercal_rec = np.append(
            1,
            np.diag(np.linalg.inv(matrix) @ (myD @ (self.d[1:, :].T @ myw))))
...
plot(xpix,
     mymap2 - truey,
     label='Minuit + Ana Residuals σ={0:5.3g}'.format(np.std(mymap2 - truey)))
plot(xpix,
     mymap - truey,
     label='Full Minuit Residuals σ={0:5.3g}'.format(np.std(mymap - truey)),
     alpha=0.5)
...
subplot(2, 3, 5)
plot(myintercals2 - intercal,
     label='Minuit + Ana Residuals σ={0:5.3g}'.format(
...
print('Init - myH(true) (=> should be 0)  RMS=',
      np.std(np.reshape(alltod_intercal, (ndet, nsamples)) - myD_true))
print('Init - myH(fit)   RMS=',
      np.std(np.reshape(alltod_intercal, (ndet, nsamples)) - myD_fit))
...
- So I suspect that my problem is due to a bad implementation of the w weighting. Maybe just multiplying as I do is not valid.
- 2022-09-29 12:01:02 Not sure the above is correct... I have made severl checks and the implementaiton seems OK. I now beleive the method itself is not optimal, but can't understand why.


# Alternative method: following [Stompor et al., 2009]
We use the method proposed in [Stompor et al., 2009] (https://arxiv.org/pdf/0804.2645.pdf) for maximum-likelihood estimation of component separation. Here, intercalibrations are the analogous of the spectral behaviur of the astrophysical components. 
