...
The first step here is to identify which are the frequencies (in the TOD domain) that are relevant for the signal we are about to reconstruct. Our signal is the synthesized beam which does not have features smaller that the peak width (arounf 1 degree on the sky for the TD, 0.39 degrees for the FI). So this corresponds to features in azimuth of:
$$\cos(30) \times 1 deg = 0.87~deg~(el=30)$$
$$\cos(70) \times 1 deg = 0.34~deg~(el=70)$$

As a consequence we do not want to remove any feature in the TOD larger than the smallest value.

The angular velocity here is 0.75 degree/sec in azimuth, so this corresponds to 0.45 seconds, or 2.2 Hertz.

This means that we expect no signal beyond 2 Hertz, we can then filter these modes out.
...
mytod_butter_BP = ft.butter_bandpass_filter(mytod, fcuts[0], fcuts[1], sampling, order=2)
p5_butter_BP, ff = ft.power_spectrum(tt, mytod_butter_BP, rebin=True)
...
plot(ff, p5_butter_BP, label='After P5 (Brute Force Band-Pass)')
xscale('log')
yscale('log')
xlabel('Frequency [Hz]')
ylabel('PSD')
...
mapsb_butter_BP, mapcount = healpix_map(azt[scantype != 0], elt[scantype != 0], mytod_butter_BP[scantype != 0], nside=nside)
```
...
display_one(mapsb_butter_BP, anatype='Butter BP', sub=(2,3,6), nlo=nlo, nhi=nhi)

```

We see tnat the gain is not huge... This si mostly due to the presence of the bright peaks. There are ways too overcome this, such as masking the regggions where the signal is strong in the TOD, then repacing them by some constrained noise realization, applying the filtering and putting the signal back. This is usually not too efficient unfortunately.
