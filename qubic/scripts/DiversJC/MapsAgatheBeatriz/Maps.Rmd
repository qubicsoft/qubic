...
```{python}
def getP(maps, mask):
    P = np.sqrt(maps[:,1]**2 + maps[:,2]**2) / maps[:,0]
    PQ = P.copy()
    PQ[~mask] = hp.UNSEEN
    return P, PQ
...
def show_gnom_around_pix(themap, pix, reso, sub=None, no_plot=True, xsize=None):
    ...
    mm = hp.gnomview(themap, rot=rot, reso=reso, sub=sub, return_projected_map=True, no_plot=True, xsize=xsize)
    return mm
...
### Order from the highest to smallest
mymaxpix = maxpix.copy()

show_gnom_around_pix(themap[:,0], mymaxpix[0], reso, xsize=npixmap, no_plot=False)
show()

nb = len(maxpix)#*0+300
for i in range(nb):
    if (i%100) == 0: print(i)
    mm += show_gnom_around_pix(themap[:,0], mymaxpix[i], reso, xsize=npixmap) / nb
    mmQ += show_gnom_around_pix(themap[:,1], mymaxpix[i], reso, xsize=npixmap) / nb
    mmU += show_gnom_around_pix(themap[:,2], mymaxpix[i], reso, xsize=npixmap) / nb
...
subplot(2,4,1)
imshow(mm, extent=[-npixmap/2*reso/60, npixmap/2*reso/60, -npixmap/2*reso/60, npixmap/2*reso/60], 
...
subplot(2,4,2)
imshow(newQ, extent=[-npixmap/2*reso/60, npixmap/2*reso/60, -npixmap/2*reso/60, npixmap/2*reso/60], origin='lower', 
...
subplot(2,4,3)
imshow(newU, extent=[-npixmap/2*reso/60, npixmap/2*reso/60, -npixmap/2*reso/60, npixmap/2*reso/60], origin='lower', 
       cmap='jet', vmin=-0.5, vmax=0.5)
xlabel('Degrees')
ylabel('Degrees')
title('Stokes U')
...
P = np.sqrt(newQ**2 + newU**2)
subplot(2,4,4)
imshow(P, extent=[-npixmap/2*reso/60, npixmap/2*reso/60, -npixmap/2*reso/60, npixmap/2*reso/60], origin='lower',
      cmap='jet', vmin=-0.5, vmax=0.5)
xlabel('Degrees')
ylabel('Degrees')
...
subplot(2,2,1)
imshow(Qr, extent=[-npixmap/2*reso/60, npixmap/2*reso/60, -npixmap/2*reso/60, npixmap/2*reso/60], origin='lower', 
       cmap='jet', vmin=-0.5, vmax=0.5)
xlabel('Degrees')
ylabel('Degrees')
colorbar()
...
subplot(2,2,2)
imshow(Ur, extent=[-npixmap/2*reso/60, npixmap/2*reso/60, -npixmap/2*reso/60, npixmap/2*reso/60], origin='lower', cmap='jet', vmin=-0.5, vmax=0.5)
xlabel('Degrees')
ylabel('Degrees')
colorbar()
title('Ur')
...
xlabel('Degrees')
ylabel('Degrees')
imshow(Qr, extent=[-npixmap/2*reso/60, npixmap/2*reso/60, -npixmap/2*reso/60, npixmap/2*reso/60], origin='lower', cmap='jet', vmin=-0.4, vmax=0.4)
colorbar()
Pr = sqrt(Qr**2 + Ur**2)
plot_vector_resample(xx, yy, newQ, newU, P, nrs=nrs)
title('Qr & Pol. vectors')
grid()
