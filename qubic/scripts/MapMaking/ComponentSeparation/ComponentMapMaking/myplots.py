...
noisy = True
band = 150220
comp_name = ['CMB', 'DUST', 'SYNC']
nc = len(comp_name)

path = os.getcwd()
path_i = f'QUBIC_CMB_DUST_SYNC_band{band}_convolution{convolution}_noise{noisy}_nsidefit{nside_fit}/'
fullpath = path + '/' + path_i

analysis = p.Analysis(fullpath, N, nside, nc, nside_fit, thr=0.02, convolution=convolution, path_to_save=f'PLOTS_'+path_i)
...
#analysis.gif_maps(fps=fps, center=center, reso=reso, istk=1, comp_name=comp_name, figsize=(9, 9), min=-4, max=4, rmin=-8, rmax=8)
#analysis.gif_maps(fps=fps, center=center, reso=reso, istk=2, comp_name=comp_name, figsize=(9, 9), min=-4, max=4, rmin=-8, rmax=8)
