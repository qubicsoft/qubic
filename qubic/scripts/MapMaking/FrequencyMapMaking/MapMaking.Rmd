...
component_acquisition.QubicIntegratedComponentsMapMaking(d, [c.Dust(nu0=150)], 3).get_operator(np.array([1.54, 20]), convolution=True)
```
...
TOD_QUBIC = qubic_acquisition.generate_tod(config=sky_config, map_ref=mysky_model, beta=beta_model, A_ev=mm.MixingMatrix(comp).evaluator(allnus), noise=noise, 
...
xx, yyI, yyQ, yyU = qss.get_angular_profile(np.array([r[0, :, 0], r[0, :, 1], r[0, :, 2]]).T, nbins=20, separate=True, center=center, thmax=30)
```

```{python}
plt.plot(xx, yyU)
plt.axhline(np.std(C(n_pl), axis=0)[2])
plt.yscale('log')
#plt.xlim(0, 50)
```
