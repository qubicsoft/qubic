...
```{python}
print(qubic_acquisition_recon.allnus)
print(qubic_acquisition_recon.nueff)
```
...
TOD_QUBIC = qubic_acquisition_recon.generate_tod(config=sky_config, map_ref=mysky_model, beta=beta_model, A_ev=mm.MixingMatrix(comp).evaluator(allnus), noise=noise, 
                                                     bandpass_correction=True, convolution=convolution)
```
...
for i in range(Nf_recon):
    ...
    mysolution = solution_qubic_planck['x'][i].copy()
...
```{python}
from qubic import QubicSkySim as qss
#r = solution_qubic_planck['x'] - mean_sky[0]
#print(r.shape)
xx, yyI, yyQ, yyU = qss.get_angular_profile(np.array([r[0, :, 0], r[0, :, 1], r[0, :, 2]]).T, nbins=30, separate=True, center=center, thmax=60)
```

```{python}
plt.plot(xx, yyQ, 'or')
plt.axhline(np.std(n_pl, axis=0)[2])
plt.yscale('log')
```
