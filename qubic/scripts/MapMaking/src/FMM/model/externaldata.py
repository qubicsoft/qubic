...
class PipelineExternalData:
    ...
    def __init__(self, file, params, noise_only=False):
    ...
    def _get_depth(self, nus):
        ...
        for mynu in nus:
            ...
            if is_bicep:
                ...
                index = (
                    self.noise["Bicep"]["frequency"].index(mynu)
                    if mynu in self.noise["Bicep"]["frequency"]
                    else -1
                )
                if index != -1:
                ...
                index = (
                    self.noise["Planck"]["frequency"].index(mynu)
                    if mynu in self.noise["Planck"]["frequency"]
                    else -1
                )
                if index != -1:
    ...
    def _get_cmb(self, seed):

        cmbmodel = CMBModel(self.params, None)
        mycls = cmbmodel.give_cl_cmb()
    ...
    def _get_noise(self, nu):
        ...
        out = (
            np.random.standard_normal(
                np.ones((12 * self.params["SKY"]["nside"] ** 2, 3)).shape
            )
            * sigma
        )
        return out
    ...
    def run(self, fwhm=False, noise=True):
        ...
        for inu, nu in enumerate(self.external_nus):
            ...
            self.maps[inu] = self._get_ave_map(
                nu,
                nu * self.params["PLANCK"]["bandwidth_planck"],
                nb=self.params["PLANCK"]["nsub_planck"],
            )
