...
BBsignal = np.mean(files._reads_all_files("Dls", verbose=verbose), axis=0)[comp_index, :, :nbins][:, comp_index, :nbins]
BBnoise = files._reads_all_files("Nls")[:, :, comp_index, :nbins][:, comp_index, :, :nbins]
...
fit = FitEllSpace(ell, BBsignal, BBnoise, model=sky.modelCMM, parameters_file=fit_params, sample_variance=sample_variance, fsky=fsky, dl=dl, diagonal=diagonal)
