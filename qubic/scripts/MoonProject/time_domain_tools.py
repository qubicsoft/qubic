...
def remove_drifts_spline(tt, tod, nsplines=20, nresample=1000, nedge=100, doplot=False):
	...
	if doplot:
		...
		# scatter(tt, tod, s=1, label='Input TOD')
		plot(tt, p(tt), color="red", label='Linear function from edges')
	...
	# splfit = MySplineFitting(tt, tod, tod*0+1, nsplines) # l'ordi crashe oups
