...
rng = [5, 5, 5]
for s in range(3):
    hp.gnomview(residuals_maps[:,s], rot=center, reso=res, sub=(1,3,s+1), title='QUBIC End-To-End - 3 years '+st[s],
...
xx, yyI, yyQ, yyU = qss.get_angular_profile(new_maps, nbins=30,
                                              label='{}'.format(newns), separate=True)
...
title('QUBIC End-To-End - 3 years')
legend()
ylim(0,50)
...
p=plot(th[1:],cth_200k[1:]/cth_200k[0],'o', label='QUBIC End-To-End - 3 Years')
