...
```{python}
np.shape(fit_n3k)
print(np.sqrt(fit_n200k[1,0:10]))
print(np.sqrt(fit_n3k[1,0:10]))

blo = np.min(np.array([fit_n3k[1,:], fit_n200k[1,:]]), axis=0)
print(np.shape(blo))
fitmin = np.array([fit_n200k[0,:], blo])

plot(fit_n3k[0,:], np.sqrt(fit_n3k[1,:]), label='3K')
plot(fit_n200k[0,:], np.sqrt(fit_n200k[1,:]), label='200k')
plot(fit_n200k[0,:], np.sqrt(fitmin[1,:]), label='Min')
xlim(0.99,1.5)
ylim(0.8,1)
legend()
...
#leff, mcl_noise_qubic, scl_noise_qubic = run_mc(nbmc, Namaster, d, signoise, cov, fitmin) #=> pareil
...
figure()
bla = explore_like(leff, scl_noise_qubic, scl_noise_qubic, lmin, delta_ell, covcut, rv,
