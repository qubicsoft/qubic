...
## Now we need to have it more realistic
We need:
- a general rule for the variance or RMS as a function of number of sub-bands
- a general rule for the correlation matrix elements

```{python}
### First the RMS a function of number of sub-bands
###### From Louise simulations we have that it goes as N to some power
def rms_sub_bands_evolution(Nsub, power=0.5, return_mean=False):
    vals = np.ones(Nsub)*Nsub**power
    if return_mean:
        return np.mean(vals)
    else:
        return vals 
...
print(rms_sub_bands_evolution(3))

all_nsub = np.arange(7)+1
all_pows = np.linspace(0,1,5)

for p in all_pows:
    all_rms = np.zeros(len(all_nsub))
    for isub in range(len(all_nsub)):
        all_rms[isub] = rms_sub_bands_evolution(all_nsub[isub], power=p, return_mean=True)
    plot(all_nsub, all_rms,'o-', label='Power = {}'.format(p))
    xlabel('Number of Sub-Bands')
    ylabel('Maps RMS Evolution')
legend()
...
```{python}
### Second the shape of the correlation matrices
...
### We need a good model of the RMS as a function of sub-bands
signoise_global = 75
nsub = 4
signoise_each = signoise_global/np.sqrt(nsub)
allpix = cov > 0
