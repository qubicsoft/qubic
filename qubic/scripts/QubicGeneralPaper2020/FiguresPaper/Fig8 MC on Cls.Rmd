...
def plot_errors_lines(leff, err, dl, s, color='r', label='', lw=1):
    for i in range(len(leff)):
        if i==0:
            p=plot([leff[i]-dl/2, leff[i]+dl/2], [err[i,s], err[i,s]],color, label=label, lw=lw)
        else:
            p=plot([leff[i]-dl/2, leff[i]+dl/2], [err[i,s], err[i,s]],color, lw=lw)
        if i < (len(leff)-1):
            p=plot([leff[i]+dl/2,leff[i]+dl/2], [err[i,s], err[i+1,s]], color,lw=lw)
...
cl0_044 = qc.get_Dl_fromlib(lll, 0.044, lib=camblib, unlensed=False)[0]   
...
for s in range(4):
    ...
    if s==2:
        ...
        plot(lll, np.abs(cl0_044[:,s]),label='$\Lambda CDM$ r=0.044 ($D_\ell$)')
...
if s==2:
    ...
    plot(lll, np.abs(cl0_044[:,s]),label='r=0.044 ($D_\ell$)')
...
annotate("P+BK 2020\n 95% C.L.", xy=(0.044, 0.01), xytext=(0.044, 0.2),
...
for s in range(4):
    ...
    if s==2:
        ...
        plot(lll, np.abs(cl0_044[:,s]),color='g',label='$\Lambda CDM$ r=0.044 ($D_\ell$)')
...
plot(lll, np.abs(cl0_044[:,s]),label='r=0.044 ($D_\ell$)', color='g')
...
plot(lll, rad2arcmin * np.sqrt(np.abs(cl0_044[:,s])*2*np.pi /lll / (lll+1)),label=r'$C_\ell^{BB}(r=0.044)$', color='g')
...
lw=4
lw2 = 2
aa=hist(-np.random.rand(10), color='r', label='150 GHz')
bb=hist(-np.random.rand(10), color='b', label='220 GHz')
plot_errors_lines(leff_150, scl_150, delta_ell, s, 'r', lw=lw)
plot_errors_lines(leff_220, scl_220, delta_ell, s, 'b', lw=lw)
lmc, = plot([-10,-11],[-10,-11],'k',lw=lw, label='MC QUBIC FI {} years ($\Delta D_\ell$)'.format(durationA))
...
plot(leff_220, fact_220_bi**2 * mytherr_220, 
     'b--', lw=lw2)
lbi, = plot([-10,-11],[-10,-11],'k--', lw=lw2, label='Th. Qubic noise ($\Delta D_\ell$)')
...
# fact_150_wn = np.sqrt(np.mean(scl_150[:,s] / mywnerr_150))
# fact_220_wn = np.sqrt(np.mean(scl_220[:,s] / mywnerr_220))
plot(leff_150, fact_150_wn**2 * mywnerr_150, 'r:', lw=lw2)
plot(leff_220, fact_220_wn**2 * mywnerr_220, 'b:', lw=lw2)
lwn, = plot([-10,-11],[-10,-11],'k:', lw=lw2, label='white noise ($\Delta D_\ell$)')

p1, = plot(lll, np.abs(cl0[:,s]), color='k')
p2, = plot(lll, np.abs(cl0_01[:,s]), color='orange')
p3, = plot(lll, np.abs(cl0_044[:,s]), color='g')
...
legend1 = legend(loc='upper left', fontsize=fsl, frameon=False)
legend([p1, p2, p3], ['r=0', 'r=0.01', 'r=0.044'], 
       frameon=False, fontsize=fsl, loc='lower right', title='Theoretical $D_\ell$')
gca().add_artist(legend1)
xlabel('$\\ell$', fontsize=fs)
ylabel('$\Delta D_\\ell$({}) [$\mu$K$^2$]'.format(clname[s]), fontsize=fs)
...
plot(rv,like, lw=3, color='k', label='QUBIC FI 150 + 220 GHz')
plot([rlim68, rlim68],[0,1.1],'k--', lw=3, label='r < {0:5.3f} at 68% C.L.'.format(rlim68))
plot([rlim95, rlim95],[0,1.1],'k:', lw=3, label='r < {0:5.3f} at 95% C.L.'.format(rlim95))


annotate("P+BK 2020\n 95% C.L.", xy=(0.044, 0.01), xytext=(0.044, 0.4),
            arrowprops=dict(width=10, headwidth=20), ha='center')
#title(r'$\ell_m = {} \;-\; \Delta_\ell = {} \;-\; Cov_c = {}$'.format(lmin, delta_ell, covcut))
legend(loc='upper right', fontsize=fsl, frameon=False)
xlabel('r',fontsize=fs)
ylabel('Posterior', fontsize=fs)
xlim(0,0.09)
ylim(0,1.1)
