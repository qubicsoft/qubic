...
print(datadir)
rep_fullpipe = datadir + 'simu_nersc_dust_fullpipeline/IBCS_fullpipeline/'
...
#     list_files = glob.glob(rep_spectrumCC + 'cross_interband*same*nfrecon{}*_qubicmaps_*FI220*_v2_gal*.npy'.format(nfrecon))
...
#             'cross_interband_samereal_nfrecon{}_qubicmaps_FI220_v2_galaxycenter.npy'.format(nfrecon), allcross)
```
...
# Full pipeline
cls_cross = np.load(rep_fullpipe + 'IBCSmix_recon_{}bands_150fullpipeline.npy'.format(nfrecon))
...
sns.palplot(palet1)
palet2 = sns.cubehelix_palette(nbands, start=2, hue=1, light=0.75, dark=0.3)
sns.palplot(palet2)
...
cls_cross_mix = np.load(rep_spectrumCC + 
                    'cross_interband_mixreal_nfrecon{}_noisemaps_FI150_v2_galaxycenter.npy'.format(nfrecon))
...
# plt.savefig('/home/lmousset/QUBIC/Qubic_work/SpectroImagerie/paper_plot/IBCS_noiseBBsame.pdf')
```
...
cls_cross220 = np.load(rep_spectrumCC + 
                    'cross_interband_mixreal_nfrecon{}_noisemaps_FI220_v2_galaxycenter.npy'.format(nfrecon))
...
# plt.savefig('/home/lmousset/QUBIC/Qubic_work/SpectroImagerie/paper_plot/IBCS_CorrMatrices_AvgOverBins.pdf')
```
...
#     np.save(rep_spectrumCC + 'BBcovariance_bincross_nfrecon{}_mixreal_FI220_v2_clean.npy'.format(nfrecon), 
#             clean_BBcov)
#     np.save(rep_spectrumCC + 'BBcorrelation_bincross_nfrecon{}_mixreal_FI220_v2_clean.npy'.format(nfrecon), 
#             clean_BBcorr)
```
...
vmax = np.max(np.abs(clean_BBcov))
plt.imshow(clean_BBcov, vmin=-vmax, vmax=vmax, cmap='bwr')
...
plt.imshow(clean_BBcorr, vmin=-1, vmax=1, cmap='bwr')
...
plt.savefig('/home/lmousset/QUBIC/Qubic_work/SpectroImagerie/paper_plot/IBCS_CorrMatrix_binfreq.pdf')
```
