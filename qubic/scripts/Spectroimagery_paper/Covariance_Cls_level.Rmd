...
b = 7
s = 0
plt.scatter(cls_cross[:, 0, b, s], cls_cross[:, 1, b, s])
pval = np.polyfit(cls_cross[:, 0, b, s], cls_cross[:, 1, b, s], deg=1)
print(pval)
...
for s in range(4):
    for i in range(nfrecon):
        labs.append(clnames[s]+' f{}'.format(i))
...
#     cls_cross = np.load(rep_spectrumCC + 'cross_nfrecon{}.npy'.format(nf))
#     cls_cross_reshape = np.moveaxis(cls_cross, [1, 2, 3], [3, 1, 2])
#     covbinband, corrbinband = amc.get_covcorr_patch(cls_cross_reshape, stokesjoint=True, doplot=False)
#     np.save(rep_spectrumCC + 'covariance_binfrec_nfrecon{}.npy'.format(nf), covbinband)
#     np.save(rep_spectrumCC + 'correlation_binfrec_nfrecon{}.npy'.format(nf), corrbinband)
```
...
vmax = np.max(np.abs(bigcov[:, :, b]))/10
