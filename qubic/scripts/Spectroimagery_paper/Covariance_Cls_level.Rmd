...
f = 220
print('**********', f, 'GHz')
for nb in np.arange(1, 9):
    
    print('Nbands', nb)
    _, edges, center, _, _, _ = compute_freq(f, nb)
    print('Edges:', np.around(edges, 2))
    print('Centers:', np.around(center, 2), '\n')
np.around
```
...
# map_type = 'qubic'
# IBCS_type = 'mix'
# for nfrecon in [3, 4, 5]:
#     list_files = glob.glob(rep_spectrumCC + f'IBCS{IBCS_type}*nfrecon{nfrecon}*_{map_type}maps_*FI{freq}*_v5_*.npy')
...
#             f'IBCS{IBCS_type}_nfrecon{nfrecon}_{map_type}maps_FI{freq}_v5_qubicpatch.npy', allcross)
```
...
name = f'IBCSmix_nfrecon{nfrecon}_noisemaps_FI150_v4_galaxycenter.npy'
name = f'v5/IBCSmix_nfrecon{nfrecon}_qubicmaps_FI150_v5_qubicpatch.npy'
cls_cross = np.load(rep_spectrumCC + name)
...
cls_cross_same = np.load(rep_spectrumCC + 
                    'IBCSsame_nfrecon{}_noisemaps_FI150_v5_qubicpatch.npy'.format(nfrecon))

cls_cross_mix = np.load(rep_spectrumCC + 
                    'IBCSmix_nfrecon{}_noisemaps_FI150_v5_qubicpatch.npy'.format(nfrecon))
