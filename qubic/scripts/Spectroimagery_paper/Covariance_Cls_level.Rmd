...
cls_cross = np.load(rep_spectrumCC + f'spectrum_samereal_nfrecon{nfrecon}.npy')
...
newcombi = list(combinations_with_replacement(np.arange(nfrecon), 2))
print(newcombi)

# Reorder cross spectra, from 00 11 22 01 02 12 to 00 01 02 11 12 22
permutation = []
for i in newcombi:
...
for i in range(4):
    ...
    for cross in range(ncombi):
        ...
        ax.errorbar(ell_binned[:], 
                ...
                label='band {}'.format(newcombi[cross]))
...
for i in range(ncombi):
    labs.append(f'{newcombi[i][0]}{newcombi[i][1]}')
print(labs)
...
for s in range(4):
    for i in range(ncombi):
        labs.append(clnames[s] + f'{newcombi[i][0]}{newcombi[i][1]}')
...
for nfrecon in allnfrecon:
    cls_cross = np.load(rep_spectrumCC + 'spectrum_samereal_nfrecon{}.npy'.format(nfrecon))
    print(cls_cross.shape)
    ...
    np.save(rep_spectrumCC + 'BBcovariance_bincross_nfrecon{}_samereal.npy'.format(nfrecon), covbinband[:, :, 2])
    np.save(rep_spectrumCC + 'BBcorrelation_bincross_nfrecon{}_samereal.npy'.format(nfrecon), corrbinband[:, :, 2])
```
...
covbinbandBB = np.load('/home/lmousset/QUBIC/Qubic_work/SpectroImagerie/corr_matrices/pourClaudia/'
                       + 'BBcovariance_bincross_nfrecon{}_samereal.npy'.format(nfrecon))
# covbinbandBB = np.load(rep_spectrumCC + 'BBcovariance_bincross_nfrecon{}_mixreal.npy'.format(nfrecon))
# corrbinbandBB = np.load(rep_spectrumCC + 'BBcorrelation_bincross_nfrecon{}_mixreal.npy'.format(nfrecon))
```

```{python}
labsx,labsy = [], []
for i in range(ncombi):
    labsx.append(f'{newcombi[i][0]}{newcombi[i][1]}b0')
    for s in range(nbins):
        labsy.append(f'{newcombi[i][0]}{newcombi[i][1]}b{s}')
        
print(labsy)

labs = []
for i in range(ncombi):
    labs.append(f'Cross {newcombi[i][0]}{newcombi[i][1]}')
```
