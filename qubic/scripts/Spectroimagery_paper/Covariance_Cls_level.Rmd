...
rep_fullpipe = datadir + '/cls_spectrum_fullpipelinesimu/'
...
#     list_files = glob.glob(rep_spectrumCC + 'cross_interband*same*nfrecon{}*_qubicmaps_*FI220*v1*.npy'.format(nfrecon))
...
#             'cross_interband_samereal_nfrecon{}_qubicmaps_FI220_v1_galaxycenter.npy'.format(nfrecon), allcross)
```
...
# cls_cross = np.load(rep_spectrumCC + f'spectrum_samereal_FI-150_nfrecon{nfrecon}.npy')
cls_cross = np.load(rep_spectrumCC + 
                    'cross_interband_samereal_nfrecon{}_noisemaps_FI220_v1_galaxycenter.npy'.format(nfrecon))
...
palet = sns.palplot(sns.color_palette("hls", ncombi))
colorlist = sns.color_palette("hls", ncombi)
...
print(pval)
x = np.linspace(np.min(cls_cross[:, 1, b, s]), np.max(cls_cross[:, 2, b, s]), 10)
...
#     cls_cross = np.load(rep_spectrumCC + 
#                     'cross_interband_mixreal_nfrecon{}_noisemaps_FI220_v1_galaxycenter.npy'.format(nfrecon))
...
#     np.save(rep_spectrumCC + 'BBcovariance_bincross_nfrecon{}_mixreal_FI220_v1.npy'.format(nfrecon), covbinband[:, :, 2])
#     np.save(rep_spectrumCC + 'BBcorrelation_bincross_nfrecon{}_mixreal_FI220_v1.npy'.format(nfrecon), corrbinband[:, :, 2])
```
...
plt.imshow(corrbinbandBB, vmin=-1, vmax=1, cmap='bwr')
