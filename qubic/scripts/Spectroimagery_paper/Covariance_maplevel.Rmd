...
# im = plt.imshow(bigcorr[:, :, 8], vmin=-1, vmax=1, cmap='bwr')
...
plt.savefig('/home/lmousset/QUBIC/Qubic_work/SpectroImagerie/paper_plot/corr_matrix_IQUmaplevel_avg_overpix.pdf')
# plt.savefig('/home/lmousset/QUBIC/Qubic_work/SpectroImagerie/paper_plot/corr_matrix_IQUmaplevel_pixel8.pdf')
```
...
#### Check that we get the same using Cp, Cp_prime00 or Cp_prime

```{python}
# Using Cp directly
avg, sig2 = amc.make_weighted_av(patch[:20, ...], Cp, verbose=True)
sig2MC = np.var(avg, axis=0)
```

```{python}
# Using Cp_prime 00
avg00, sig200 = amc.make_weighted_av(patch[:20, ...], Cp_prime00, verbose=True)
...
```{python}
# Using Cp_prime
avg_prime, sig2_prime = amc.make_weighted_av(patch[:20, ...], Cp_prime, verbose=True)
...
#     coverage_norm = coverage_normlist[i]
    ...
    avg, sig2 = amc.make_weighted_av(patch[:100, ...], Cp, verbose=True)
    sig2MC = np.var(avg, axis=0)
...
#     coverage_norm = coverage_normlist[i]
#     plt.figure()
    # Noise profile to normalize the pixels
#     xx, yy, fitcov = qss.get_noise_invcov_profile(mylist_sig2MC[i], 
#                                                   coverage_norm[seenmap][condition],
#                                                   covcut=coveragecut,
#                                                   QUsep=True,  
#                                                   norm=False,
#                                                   nbins=20, 
...
#     correctionI = np.interp(1. / coverage_norm[seenmap][condition], fitcov[0, :], fitcov[1, :])
#     correctionQU = np.interp(1. / coverage_norm[seenmap][condition], fitcov[0, :], fitcov[2, :])
#     print(correctionI.shape)
    
    # Apply the correction to sigma theo and sigma MC and compute the profile over pixels 
    for istk in range(3):
        sig = np.sqrt(mylist_sig2[i]) 
        sigMC = np.sqrt(mylist_sig2MC[i])
        print('sigma',sig.shape)
#         if istk == 0:
#             factor = np.sqrt(correctionI * coverage_norm[seenmap][condition]) 
#         else:
#             factor = np.sqrt(correctionQU * coverage_norm[seenmap][condition]) 
#         npix = sig.shape[0]
#         a = np.arange(npix)
        a = ang[condition]
...
for istk in range(3):
    ...
    for i in range(6):
        plt.errorbar(x[i, :, istk], y[i, :, istk]/y[0, :, istk], xerr=dx[i, :, istk], yerr=dy[i, :, istk], 
                     color=colorlist[i], label='theo - {} bands'.format(allrecon[i]))
        plt.errorbar(xMC[i, :, istk], yMC[i, :, istk]/yMC[0, :, istk], xerr=dxMC[i, :, istk], yerr=dyMC[i, :, istk], 
                     fmt='o', color=colorlist[i], label='MC - {} bands'.format(allrecon[i]))

    plt.ylim(0.9, 2.2)
    plt.ylabel('$\sigma/ \sigma_{1band}$', fontsize=15)
