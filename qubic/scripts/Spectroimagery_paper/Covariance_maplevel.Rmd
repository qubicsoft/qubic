...
#         pickle.dump(coverage, f)
    ...
    mylist_residuals.append(residuals)
...
#     print('Residual full map shape:', residuals_fullmap.shape)
    ...
    with open(rep_save + 'residualspatch_allreals_nfsub{}_nfrecon{}.pkl'.format(nfsub, nfrec), 'wb') as f:
...
#         covI[i], covQ[i], covU[i],  fitcov, _ = qss.get_cov_nunu(residuals[i], coverage_norm)
...
# #     residual_norm, _, _ = qss.flatten_noise(residuals[0], coverage_norm, nbins=20, doplot=False)
...
#     print('covI_brut', covI_brut.shape)
...
# Save some matrices
import pickle
rep_save = '/home/lmousset/QUBIC/Qubic_work/SpectroImagerie/paper_simu/corr_matrices/pourJC/'
...
```{python}
with open(rep_save + 'coverage_nfsub15_nfrecon3.pkl', 'rb') as f:
...
#### Normalisation factor per pixel for maps or covariance matrices 
...
```{python}
isub = 0
xx, yy, fitcov = qss.get_noise_invcov_profile(residuals[0, isub, :, :], 
                                              coverage_norm, 
                                              nbins=20, 
                                              norm=False,
...
plt.figure()
for istk in range(3):
    myterms = [mylist_Cp_corrected_avg[i][diag_term, diag_term, istk] for i in range(7)]
    plt.plot(nbands, myterms, 'o', color=colorlist[istk])
...
plt.xlabel('# Bands')
plt.ylabel('$Cp_{00}$ corrected and averaged')
plt.legend()
```
