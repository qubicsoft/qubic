...
# real = 0
cells_recon = np.zeros((nreals, nfrecon, nbins, 4))
# Get spectra
for real in range(nreals):
    for isub in range(nfrecon):
        maps = maps_recon[real, isub, :, :] #+ x0[isub, :, :]
        leff, cells_recon[real, isub, :, :], w = Namaster.get_spectra(maps.T, mask_apo, w=w,
                                                                  purify_e=True, 
                                                                  purify_b=False, 
...
```{python}
for nbin in range(16):
    plt.scatter(cells_recon[:, 0, nbin, 1], cells_recon[:, 1, nbin, 1])
    pval = np.polyfit(cells_recon[:, 0, nbin, 0], cells_recon[:, 1, nbin, 0], deg=1)
    print(pval)
    x = np.linspace(np.min(cells_recon[:, 0, nbin, 1]), np.max(cells_recon[:, 0, nbin, 1]), 10)
...
for i in range(4):
    ax = axs[i]
    for isub in range(nfrecon): 
        ax.plot(leff[:], cells_recon[0, isub, :, i],'o', color=colorlist[isub], 
...
for isub in range(nfrecon):
    ...
    for c1 in range(0, nreals-1, 2): # do not mix pairs to avoid correlation 
        c2 = c1 + 1 
        c = (c1, c2)
        map1 = maps_recon[c1, isub, :, :] #+ x0[isub, :, :]
        map2 = maps_recon[c2, isub, :, :] #+ x0[isub, :, :]
