...
```{python}
reload(fsed)
covname = [global_dir+'/doc/FastSimulator/Data/DataFastSimulator_TD150G_coverage.fits',]
coverageTD = fsed.coverage(dictionaries, regions, bands, filename = covname)
```
...
#NEW
reload(fsed)
fsed._plot_covs(regions, bands, coverageTD, center, config = "TD")
```
...
reload(fsed)
noise = fsed.foreground_with_noise(dictionaries, sky_conf, coverageTD, 10, verbose = False) 
```
...
```{python}
# NEW (16 Feb 2021)
maps, cov = fsed.foregrounds_with_noise()
```
