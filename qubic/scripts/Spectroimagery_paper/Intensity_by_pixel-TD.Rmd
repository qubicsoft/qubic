...
```{python}
noisyP_ud = np.zeros((nreals, dTD['nf_recon'], 12 * nside_new ** 2))
for i in range(nreals):
    for j in range(dTD['nf_recon']):
        noisyP_ud[i, j] = np.sqrt( (31 * noiseFI_ud[i, j, :, 1] + dust_map_TD_ud[j, :, 1])** 2 + \
                                   (31 * noiseFI_ud[i, j, :, 2] + dust_map_TD_ud[j, :, 2])** 2 ) 
mapsP_ud, stdP_ud =  np.mean(noisyP_ud, axis = 0), np.std(noisyP_ud, axis = 0)
