...
```{python}
pixQ = [hp.ang2pix(d150Q['nside'], np.pi / 2 - np.deg2rad(centerQ[1] + 15), np.deg2rad(centerQ[0] - 14  ) ), ]
pixG = [hp.ang2pix(d150G['nside'], np.pi / 2 - np.deg2rad(centerG[1] + 7), np.deg2rad(centerG[0] + 12 ) ), ]
...
FuncModel = func353
istk = 0
Imvals150Q, Isvals150Q, x150 = PixSED_Xstk(nus_out150, maps150Q_ud, FuncModel, pixQ_ud, pixQ_red, 
                               istk, Cp150Q_prime, nus_edge150)
Imvals150G, Isvals150G, _ = PixSED_Xstk(nus_out150, maps150G_ud, FuncModel, pixG_ud, pixG_red, 
                               istk, Cp150G_prime, nus_edge150)

Imvals220Q, Isvals220Q, x220 = PixSED_Xstk(nus_out220, maps220Q_ud, FuncModel, pixQ_ud, pixQ_red, 
                               istk, Cp220Q_prime, nus_edge220)
Imvals220G, Isvals220G, _ = PixSED_Xstk(nus_out220, maps220G_ud, FuncModel, pixG_ud, pixG_red, 
...
for j in range(1):#enumerate(range(600,12000,500)):
    ...
    ax[0].errorbar(nus_out150, maps150G_ud[:,pixG_ud,0],yerr = std150G_ud[:,pixG_ud,0], capsize=capsize,
           fmt = 'o-', color = 'r',label = 'MC reconstructed')
    ax[0].errorbar(nus_out220, maps220G_ud[:,pixG_ud,0],yerr = std220G_ud[:,pixG_ud,0], capsize=capsize,
           fmt = 'o-', color = 'r')
    
    ax[0].plot(nus_out150, dust_map_150G_ud[:,pixG_ud,0], 'bo-', label = 'convolved')
    ax[0].plot(nus_out220, dust_map_220G_ud[:,pixG_ud,0], 'bo-')
    ax[0].plot(x150, Imvals150G, 'g-', label = 'MCMC reconstructed')
    ax[0].fill_between(x150, y1 = Imvals150G - Isvals150G, y2 = Imvals150G + Isvals150G, 
                       color = 'g', alpha = 0.3)
    ax[0].plot(x220, Imvals220G , 'g-')
    ax[0].fill_between(x220, y1 = Imvals220G - Isvals220G, y2 = Imvals220G + Isvals220G, 
                       color = 'g', alpha = 0.3)
    ...
    ax[2].plot(x150, Imvals150Q, 'g')
    ax[2].fill_between(x150, y1 = Imvals150Q - Isvals150Q, y2 = Imvals150Q + Isvals150Q, 
                       color = 'g', alpha = 0.3)
    ax[2].plot(x220, Imvals220Q, 'g')
    ax[2].fill_between(x220, y1 = Imvals220Q - Isvals220Q, y2 = Imvals220Q + Isvals220Q, 
                       color = 'g', alpha = 0.3)
...
ax=ax.ravel()
# Plotting
ax[0].errorbar(nus_out150, maps150G_ud[:,pixG_ud,stkP],yerr = std150G_ud[:,pixG_ud,stkP], capsize=capsize,
       fmt = 'o-', color = 'r',label = 'MC reconstructed')
ax[0].errorbar(nus_out220, maps220G_ud[:,pixG_ud,stkP],yerr = std220G_ud[:,pixG_ud,stkP], capsize=capsize,
       fmt = 'o-', color = 'r')
...
ax[0].plot(x150, Qmvals150G, 'g-', label = 'MCMC reconstructed')
...
ax[2].errorbar(nus_out150, maps150Q_ud[:,pixQ_ud,stkP],yerr=std150Q_ud[:,pixQ_ud,stkP], capsize=capsize,
       fmt='o-', color='r', alpha=0.8)
ax[2].errorbar(nus_out220, maps220Q_ud[:,pixQ_ud,stkP],yerr=std220Q_ud[:,pixQ_ud,stkP], capsize=capsize,
       fmt='o-', color='r',alpha=0.8)
...
ax[0].plot(x150, Umvals150G, 'g-', label = 'MCMC reconstructed')
