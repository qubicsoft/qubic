...
for j,ipx in enumerate([10000,]):#enumerate(range(600,12000,500)):
    ...
    hp.gnomview(maps_qubic150G[-1,:,0], reso=15,hold=True,notext=True,title=' ',#'{:.1f}GHz'.format(nus[0]),
                max=0.3*np.max(maps_qubic150G[0,:,0]), rot=None)
    ...
    ax[4].errorbar(nus, np.sqrt(maps_qubic150G[:,IPIXG,1]**2+maps_qubic150G[:,IPIXG,2]**2),
                   #yerr=std_qubic150G[:,IPIXG,0], 
                   capsize=capsize,
           fmt='o-', color='r',label='reconstructed', alpha=0.8)
    ax[4].errorbar(nus_220, np.sqrt(maps_qubic220G[:,IPIXG,1]**2+maps_qubic220G[:,IPIXG,2]**2),
                   #yerr=std_qubic220G[:,IPIXG,0], 
                   capsize=capsize,
           fmt='o-', color='r', alpha=0.8)
    ax[4].plot(nus, np.sqrt(maps_noiseless150G[:,IPIXG,1]**2+maps_noiseless150G[:,IPIXG,2]**2),
           'bo-',label='convolved', alpha=0.7)
    ax[4].plot(nus_220, np.sqrt(maps_noiseless220G[:,IPIXG,1]**2+maps_noiseless220G[:,IPIXG,2]**2),
           'bo-', alpha=0.7)
    ax[4].axvspan(nus[-1],nus_220[0],color='k',alpha=0.2)

    ax[6].errorbar(nus, np.sqrt(maps_qubic150Q[:,IPIXQ,1]**2+maps_qubic150Q[:,IPIXQ,2]**2),
                   #yerr=std_qubic150Q[:,IPIXQ,0], 
                   capsize=capsize,
           fmt='o-', color='r',label='reconstructed', alpha=0.8)
    ax[6].errorbar(nus_220, np.sqrt(maps_qubic220Q[:,IPIXQ,1]**2+maps_qubic220Q[:,IPIXQ,2]**2),
                   #yerr=std_qubic220Q[:,IPIXQ,0], capsize=capsize,
           fmt='o-', color='r',alpha=0.8)
    ax[6].plot(nus, np.sqrt(maps_noiseless150Q[:,IPIXQ,1]**2+maps_noiseless150Q[:,IPIXQ,2]**2),
           'bo-',label='input', alpha=0.7)
    ax[6].plot(nus_220, np.sqrt(maps_noiseless220Q[:,IPIXQ,1]**2+maps_noiseless220Q[:,IPIXQ,2]**2),
           'bo-', alpha=0.5)

    ax[6].grid(which='both')
    ax[6].axvspan(nus[-1],nus_220[0],color='k',alpha=0.2)
        
    ax[4].set_title('GC patch',fontsize=17)
    ax[4].set_ylabel(r'$I_{P} = \sqrt{Q^2+U^2}$ [$\mu$K]',fontsize=18)
    ax[4].set_xlabel(r'$\nu$[GHz]',fontsize=18)
    ax[6].set_title('QUBIC patch',fontsize=17)
    #ax[2].set_ylabel(r'$I_\nu$ [$\mu$K]',fontsize=18)
    ax[6].set_xlabel(r'$\nu$[GHz]',fontsize=18)
    ...
    ax[5].cla()
    plt.axes(ax[5])
    hp.gnomview(np.sqrt(maps_qubic220G[-1,:,1]**2+maps_qubic220G[-1,:,2]**2), 
                reso=15,hold=True,notext=True,title='220GHz band ',#'{:.1f}GHz'.format(nus[0]),
                #max=0.3*np.max(maps_qubic150G[0,:,0]), 
                rot=None)
    hp.projscatter(hp.pix2ang(d['nside'],IPIXG),marker='+',color='r',)
    ax[7].cla()
    plt.axes(ax[7])
    hp.gnomview(np.sqrt(maps_qubic220Q[-1,:,1]**2+maps_qubic220Q[-1,:,2]**2), 
                reso=15,hold=True,notext=True,title='220GHz band ',#'{:.1f}GHz'.format(nus[0]),
                #max=np.max(maps_qubic150Q[0,:,0]), 
                rot=centerQ)
    hp.projscatter(hp.pix2ang(d['nside'],IPIXQ),marker='+',color='r',)
