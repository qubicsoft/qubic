...
```{python}
nfrecon = 3
rep = f'/home/lmousset/QUBIC/Qubic_work/SpectroImagerie/paper_simu/galaxy_center_d1/nfrecon{nfrecon}/'
...
for i in range(nfrecon):
    ...
    print('FWHM:', fwhm)
    for istk in range(3):
        sky_conv[i, :, istk] = hp.sphtfunc.smoothing(sky[i, :, istk], fwhm)
...
Qubic_sky = qss.Qubic_sky(sky_config, d)
skyqubic = Qubic_sky.get_simple_sky_map()
print('Input map with shape:', skyqubic.shape)
hp.mollview(skyqubic[2,:,0], rot=center)
```
...
for i in range(4):
    ...
    plt.figure(figsize=(15, 4))
    plt.subplot(121)
    plt.plot(nus_rec, sky[:, pix, istk], 'ro', alpha=0.5, label=f'Input sky')
    ...
    theta, phi = hp.pix2ang(d['nside'], pix)
    print(theta, phi)
    
    hp.mollview(sky[0, :, 0]*seenmap_dgrad, sub=(122), title='Pixel {}'.format(pix))
    hp.visufunc.projtext(theta, phi, 'PIX', color='r', fontsize=15)
