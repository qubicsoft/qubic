...
```{python}
#build coverage map in QUBIC field
def build_cov():
...
if mctest:
    ...
    for i in range(10):
        auxmaps, maps_noiseless, auxnoise, coverage = Qubic_sky_dust.get_partial_sky_maps_withnoise(spatial_noise=False,
                                                                                              verbose=False)
        print('=== Done (dust) interation #{} ===='.format(i+1))
        auxmapscmb, maps_noiseless_cmb, auxnoisecmb, coverage_cmb = \
            Qubic_sky_cmb.get_partial_sky_maps_withnoise(spatial_noise=False)        
        print('=== Done (cmb) interation #{} ===='.format(i+1))
    ...
    maps_noise_cmb=np.mean(mcmcnoisecmb,axis=0)
else: 
...
hp.gnomview(maps_qubic[NU,:,STK], sub=(2,4,1), 
            max=0.1*np.max(maps_qubic[NU,cov,STK]), title='reconstructed', reso=RESO)
...
for ifreq in range(d['nf_recon']):
    ...
    auxmap[auxmap>0.01*max(auxmap)] = hp.UNSEEN
    ...
    hp.gnomview(auxmap, reso=15,hold=True,notext=True,title='{:.1f}GHz'.format(nus[ifreq]),
               min=-10, max=0.2*np.max(maps_qubic[ifreq,cov,0]) )
#ax=ax.ravel()
