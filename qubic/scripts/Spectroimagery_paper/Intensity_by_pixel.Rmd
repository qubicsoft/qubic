...
#Imvals = np.zeros((len(dictionaries), samples//2, 3))
#Isvals = np.zeros((len(dictionaries), samples//2, 3))
#xarr = np.zeros((len(dictionaries), samples//2, 3))

#for istk in range(3):
#    print("======== Doing {} Stokes parameter =============".format(dictionaries[0]['kind'][istk]))
#    for j in range(len(dictionaries)):
#        Imvals[j, :, istk], Isvals[j, :, istk], xarr[j, :, istk] = \
#                                                    PixSED_Xstk(nus_out[j], fgr_map_ud[j], FuncModel, 
...
ySED = np.zeros((len(dictionaries), nf_recon, 2))
popt = np.zeros((len(dictionaries), len(gmodel.param_names), 2))
pcov = np.zeros((len(dictionaries), len(gmodel.param_names), len(gmodel.param_names), 2))
#With polarization
...
for icomp in range(2):
    for j in range(len(dictionaries)):
        if istk == 0:
            ...
            popt[j, :, icomp], pcov[j, :, :, istk] = curve_fit(quad, xSED[j], ySED[j, :, icomp])
        else:
            ySED[j, :, icomp] = np.sqrt(fgr_map_ud[j][:,pixs_ud[j], 1] ** 2 + \
                                        fgr_map_ud[j][:,pixs_ud[j], 2] ** 2)
            popt[j, :, icomp], pcov[j, :, :, icomp] = curve_fit(quad, xSED[j], ySED[j, :, icomp])


ySED_fit = np.zeros((len(dictionaries), len(xarr[0]), 2 ))
Perr = np.zeros((len(dictionaries), len(Imvals[j, :, 2])) )
for icomp in range(2):
    for j in range(len(dictionaries)):
        if icomp == 0:
            ySED_fit[j,:,icomp] = quad(xarr[j, :], *popt[j, :, icomp])
        else:
...
for icomp in range(2):
    ...
    plt.plot(xnew1, quad(xnew1, *popt[0,:,icomp]),'r-',label='150Q')
    plt.plot(xnew1, quad(xnew1, *popt[2,:,icomp]),'b-',label='150G')
    plt.plot(xSED[0], ySED[0,:,icomp], 'ro')
    plt.plot(xSED[2], ySED[2,:,icomp], 'bo')
    plt.legend()
    plt.subplot(1,4,icomp*2+2)
    plt.title("{} component".format("I" if icomp ==0 else "P"))
    plt.plot(xnew2, quad(xnew2, *popt[1,:,icomp]),'r-',label='220Q')
    plt.plot(xnew2, quad(xnew2, *popt[3,:,icomp]),'b-',label='220G')
...
Average realizations and compute std of realizations
```{python}
# OLD (17 Feb 2021) Implemented above
# Modeling fit to map values
...
```{python}
# OLD (17 Feb 2021 not use it anymore)
...
```{python}
#OLD (17Feb 2021) Implemented above
ErrBar2 = lambda Q, U, Qerr, Uerr: np.sqrt( Q ** 2 * Qerr ** 2 + U ** 2 * Uerr ** 2) / \
                np.sqrt( Q ** 2 + U ** 2)
```
...
e1 = ax[0].fill_between(xarr[0], y1 = ySED150G_fit_pol - Perr150G, y2 = ySED150G_fit_pol + Perr150G, 
                   color = 'r', alpha = 0.3, label = '68% C.L. ')
e2 = ax[0].fill_between(x220, y1 = ySED220G_fit_pol - Perr220G, y2 = ySED220G_fit_pol + Perr220G, 
                   color = 'b', alpha = 0.3)
ax[0].axvspan(nus_edge150[-1], nus_edge220[0], color = 'k', alpha = greyscale)
