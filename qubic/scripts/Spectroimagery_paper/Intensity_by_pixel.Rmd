...
pixQ = [hp.ang2pix(dictionaries[0]['nside'], 
                   np.pi / 2 - np.deg2rad(centers[0][1] - 8), np.deg2rad(centers[0][0] - 10  ) ), ]
pixG = [hp.ang2pix(dictionaries[0]['nside'], 
                   np.pi / 2 - np.deg2rad(centers[2][1] - 5), np.deg2rad(centers[2][0] + 2 ) ), ]
...
if study == "dust":
    ...
    ax.plot(xrange, FuncModel(xrange, [100, 1.5, 30, - 3]), 'r-', label = r"$A_{d} =100$ $\beta_{dt} = 1.5$ $A_{s} = 30$ $\beta_{s} = -3$")
