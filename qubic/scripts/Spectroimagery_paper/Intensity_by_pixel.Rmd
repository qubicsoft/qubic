...
if makeCov:
    ...
    for jr, region in enumerate(regions):
        for jb, band in enumerate(bands):
            index = len(bands) * jr + jb
            coveragesmaps[index] = np.load(global_dir+\
                    '/doc/FastSimulator/Data/DataFastSimulator_FI{}{}_coverage.npy'.format(band, region[0]))
...
for ic, icov in enumerate(coveragesmaps):
    ...
    print(ic//2)
    ax.set_title('cov{} {}-patch'.format(bands[ic//2], regions[ic//2][0]), fontsize=16)
...
##### QubicSkySim instanciation
QubicSkyObject = []
sky_configuration = {'dust': 'd1'}
dust_maps = np.zeros((len(dictionaries), dictionaries[0]['nf_recon'], 12 * dictionaries[0]['nside'] ** 2, 3))
for ic,idict in enumerate(dictionaries):
    QubicSkyObject.append(qss.Qubic_sky(sky_configuration, idict) )
    dust_maps[ic] = QubicSkyObject[ic].get_fullsky_convolved_maps(FWHMdeg = None, verbose = False)
...
# NEW (16 Feb 2021)
reload(fsed)
maskmaps = fsed._mask_maps(maps, coveragesmaps, dictionaries[0]['nf_recon'])
```
...
pixG = [hp.ang2pix(dictionaries[0]['nside'], np.pi / 2 - np.deg2rad(centers[2][1] + 1 ), 
                   np.deg2rad(centers[2][0]) ), ]

fig,ax=plt.subplots(nrows=1,ncols=2,figsize=(14,5),)
ax=ax.ravel()
IPIXG = pixG[0]
ax[1].plot(nus_out150, maskmaps[2,:,IPIXG,0], 'o-', color='r')
ax[1].set_ylabel(r'$I_\nu$ [$\mu$K]')
ax[1].set_xlabel(r'$\nu$[GHz]')
ax[0].cla()
plt.axes(ax[0])
hp.gnomview(maskmaps[2,-1,:,0], reso = 15,hold = True, title = ' ',unit = r'$\mu$K', notext =True,
            min = 0 ,
            max = 0.23 * np.max(maskmaps[2,-1,:,0]), rot = centers[2])
hp.projscatter(hp.pix2ang(dictionaries[0]['nside'], IPIXG), marker = '*', color = 'r',s = 180)
dpar = 10
dmer = 20
#Watch out, the names are wrong (change it)
mer_coordsG = [centers[2][0] - dmer, centers[2][0], centers[2][0] + dmer]
long_coordsG = [centers[2][1] - 2*dpar, centers[2][1] - dpar, centers[2][1], 
                centers[2][1] + dpar, centers[2][1] + 2 * dpar]
#paralels
for ilong in long_coordsG:
    plt.text(np.deg2rad(mer_coordsG[0] - 13), 1.1*np.deg2rad(ilong), 
             r'{}$\degree$'.format(ilong))
...
noise_ud = np.zeros((nreals, len(nus_out150), npix_ud, 3))
...
fgr_map_ud = np.zeros((len(dictionaries), len(nus_out150), 12 * nside_new ** 2, 3))
noise_ud_i = np.zeros((len(dictionaries), nreals, len(nus_out150), npix_ud, 3))
maps_ud_i = np.zeros_like(noise_ud)

for bandreg in range(len(dictionaries)):
    for irec in range(len(nus_out150)):
        fgr_map_ud[bandreg, irec] = hp.ud_grade(fground_maps[bandreg, irec].T, nside_new).T
        for ireal in range(nreals):
            noise_ud_i[bandreg, ireal, irec] = hp.ud_grade(noise[bandreg, ireal, irec].T, nside_new).T
...
```{python}
#OLD (17 feb 2021)
cov_ud = hp.ud_grade(covm, nside_new)
...
for br in range(len(dictionaries)):
    Cp = amc.get_Cp(noise_ud[br][:, :, cov_ud[br], :], verbose = False, doplot = False)
...
samples = 5000
Imvals = np.zeros((len(dictionaries), samples//2))
Isvals = np.zeros((len(dictionaries), samples//2))
xarr = np.zeros((len(dictionaries), samples//2))

for j in range(len(dictionaries)):
    Imvals[j], Isvals[j], xarr[j] = PixSED_Xstk(nus_out[j], fgr_map_ud[j], FuncModel, 
                                                pixs_ud[j], pixs_red[j], 
                               istk, Cp_prime[j], nus_edge[j])
print('Done in {:.2f} min'.format((time.time()-t0)/60 ))
```

```{python}
# OLD (17 Feb 2021) The new one is implemented in the above cell together with I component
t1 = time.time()
istk = 1
...
gmodel = Model(quad, independent_vars=['x'])
print(len(gmodel.param_names))
ySED = np.zeros((len(dictionaries), nf_recon))
popt = np.zeros((len(dictionaries), len(gmodel.param_names)))
pcov = np.zeros((len(dictionaries), len(gmodel.param_names), len(gmodel.param_names)))
#With polarization
#ySED = np.zeros((len(dictionaries), nf_recon, 3))

for j in range(len(dictionaries)):
    ySED[j] = fgr_map_ud[j][:,pixs_ud[j],0]
    popt[j], pcov[j] = curve_fit(quad, xSED[j], ySED[j])
...
plt.subplot(121)
plt.plot(xnew1, quad(xnew1, *popt[0]),'r-',label='150Q')
plt.plot(xnew1, quad(xnew1, *popt[2]),'b-',label='150G')
plt.plot(xSED[0], ySED[0], 'ro')
plt.plot(xSED[2], ySED[2], 'bo')
plt.legend()
plt.subplot(122)
plt.plot(xnew2, quad(xnew2, *popt[1]),'r-',label='220Q')
plt.plot(xnew2, quad(xnew2, *popt[3]),'b-',label='220G')
plt.plot(xSED[1], ySED[1], 'ro')
plt.plot(xSED[3], ySED[3], 'bo')
plt.legend()
```
...
ySED_fit = np.zeros((len(dictionaries), len(xarr[0])))
for j in range(len(dictionaries)):
    ySED_fit[j] = quad(xarr[j], *popt[j])

fig,ax = plt.subplots(nrows = 1, ncols = 4,figsize = (19,4.5), gridspec_kw = {'wspace': 0.4})
ax = ax.ravel()
plt.subplots_adjust(wspace = 0.1)
# Plotting
p1, = ax[0].plot(nus_out150, fgr_map_ud[2, :, pixG_ud, 0], 'ro', label = 'Input sky')
p2, = ax[0].plot(nus_out220, fgr_map_ud[3, :, pixG_ud, 0], 'bo')
e1 = ax[0].fill_between(xarr[2], y1 = ySED_fit[2] - Isvals[2], y2 = ySED_fit[2] + Isvals[2], 
                 color = 'r', alpha = 0.3, label = '68% C.L.')
e2 = ax[0].fill_between(xarr[3], y1 = ySED_fit[3] - Isvals[3], y2 = ySED_fit[3] + Isvals[3], 
                   color = 'b', alpha = 0.3)

ax[2].plot(nus_out150, fgr_map_ud[0, :, pixQ_ud, 0], 'ro')
ax[2].plot(nus_out220, fgr_map_ud[1, :, pixQ_ud, 0], 'bo')
ax[2].fill_between(xarr[0], y1 = ySED_fit[0] - Isvals[0], y2 = ySED_fit[0] + Isvals[0], 
                   color = 'r', alpha = 0.3)
ax[2].fill_between(xarr[1], y1 = ySED_fit[1] - Isvals[1], y2 = ySED_fit[1] + Isvals[1], 
                   color = 'b', alpha = 0.3)
