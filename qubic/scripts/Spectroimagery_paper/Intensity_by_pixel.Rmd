...
reload(fsed)
nreals = 43
noise = fsed.noise_qss(dictionaries, sky_conf_all, coveragesmaps, nreals, verbose = False) 
...
maskmaps_dust, _ = fsed._mask_maps(fground_maps_dust, coveragesmaps, dictionaries[0]['nf_recon'])
maskmaps_synch, _ = fsed._mask_maps(fground_maps_synch, coveragesmaps, dictionaries[0]['nf_recon'])
maskmaps_all, _ = fsed._mask_maps(fground_maps, coveragesmaps, dictionaries[0]['nf_recon'])
...
```{python}
#noise_ud_i = np.zeros((4, 4*43, 5,12 * nside_new ** 2, 3))
#for i in range(4):
#    print(43*i,43*(i+1))
#    noise_ud_i[:, 43*i:43*(i+1), :, :, :] = FitsArray("SED_NoiseMaps_nside64_nreals43_{}.fits".format(i))
```
...
if readfiles:
    ...
    maps = FitsArray("SED_FullMaps_nside{}_nreals{}.fits".format(nside_new,nreals))
    fgr_map_ud = FitsArray("SED_ForegMaps_nside{}_nreals{}.fits".format(nside_new,nreals))
    noise_ud_i = FitsArray("SED_NoiseMaps_nside{}_nreals{}.fits".format(nside_new,nreals))
    ...
    print("shapessss: ", np.shape(noise_ud_i))
    ...
    _, _, fgr_map_dust_ud, _ = fsed.udgrade_maps(fground_maps_dust, noise, 
                                                             nside_new, nf_recon, nreals)
    _, _, fgr_map_synch_ud, _ = fsed.udgrade_maps(fground_maps_synch, noise, 
                                                              nside_new, nf_recon, nreals)
    maps_ud, _, fgr_map_ud, noise_ud_i = fsed.udgrade_maps(fground_maps, noise, 
                                                           nside_new, nf_recon, nreals)
    _, covmask = fsed._mask_maps(maps, coveragesmaps, nf_recon)
    cov_ud = hp.ud_grade(covmask, nside_new)
    Cp = []
...
writefiles = False
if writefiles:
    #FitsArray(fgr_map_ud).save("SED_ForegMaps_nside{}_nreals{}.fits".format(nside_new,nreals))
    FitsArray(noise_ud_i).save("SED_NoiseMaps_nside{}_nreals{}_3.fits".format(nside_new,nreals))
    #for j, idict in enumerate(dictionaries):
    #    FitsArray(Cp_prime[j]).save("SED_CovarMatrix_nside{}_nreals{}_{}.fits".format(nside_new,nreals,
    #                                                                              bandreg[j]))
    #FitsArray(np.array(cov_ud)).save("SED_CoverMaps_nside{}_nreals{}.fits".format(nside_new,nreals))
    #FitsArray(maps).save("SED_FullMaps_nside{}_nreals{}.fits".format(nside_new,nreals))

#noise, maskmaps, mapregion, fground_maps
```
...
plt.figure()
plt.imshow(Cp_prime[0][:, :, 0, pixQ_red])
plt.colorbar()
```
