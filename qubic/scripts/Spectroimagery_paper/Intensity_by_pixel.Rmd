...
hp.gnomview(maps_qubic[NU,:,STK], sub=(2,4,1), 
            max=0.1*np.max(maps_qubic[NU,cov,STK]), title='input', reso=RESO)
hp.gnomview(maps_noiseless[NU,:,STK], sub=(2,4,2), 
            max=0.1*np.max(maps_noiseless[NU,:,STK]), title='noiseless', reso=RESO)
...
for j,ipx in enumerate(range(0,24000,3500)):
    ...
    hp.gnomview(maps_qubic_cmb[0,:,0], reso=15,hold=True,notext=True,title='{:.1f}GHz'.format(nus[0]),
                max=0.1*np.max(maps_qubic_cmb[0,:,0]))
    hp.projscatter(hp.pix2ang(d['nside'],IPIX),marker='+',color='r',)
...
for ifreq in range(d['nf_recon']):
    ...
    auxmap[auxmap>0.003*max(auxmap)] = hp.UNSEEN
...
axbig.plot(nus, dvals,
       'ro-', lw=4,label='Dust', alpha=1)
axbig.plot(nus, cvals,
        'go-',lw=4,label='CMB' , alpha=1)
