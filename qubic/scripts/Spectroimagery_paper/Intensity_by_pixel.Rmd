...
QUBICField=True
if QUBICField:
    coverage = np.load(global_dir+'doc/FastSimulator/Data/coverage_nfsub15_nptgs10000_qubicpatch.pk')
...
if mctest:
    ...
    mcmcnoisecmb220=[]
    for i in range(10):
        auxmaps, maps_noiseless, auxnoise, coverage = \
        ...
        auxmapscmb, maps_noiseless_cmb, auxnoisecmb, coverage_cmb = \
        ...
        auxmaps220, maps_noiseless220, auxnoise220, coverage220 = \
        ...
        auxmaps220cmb, maps_noiseless_220cmb, auxnoise220cmb, coverage_220cmb = \
        ...
        mcmcnoisecmb.append(auxnoisecmb)
        
        mcmcmaps220.append(auxmaps220)
        mcmcnoise220.append(auxnoise220)
        mcmcmapscmb220.append(auxmaps220cmb)
        mcmcnoisecmb220.append(auxnoise220cmb)
    ...
    maps_noise220=np.mean(mcmcnoise220,axis=0)
    maps_noise_cmb220=np.mean(mcmcnoisecmb220,axis=0)
else: 
...
cov=np.zeros_like(coverage,dtype=bool)
covmsk=np.where(coverage > 0.01*np.max(coverage))
cov[covmsk] = 1
NU = 0
STK = 0
RESO = 15
plt.figure(figsize=(12,8))
hp.gnomview(maps_qubic[NU,:,STK], sub=(2,4,1), 
            max=0.1*np.max(maps_qubic[NU,cov,STK]), title='reconstructed', reso=RESO, rot=center)
hp.gnomview(maps_noiseless[NU,:,STK], sub=(2,4,2), 
            max=0.1*np.max(maps_noiseless[NU,:,STK]), title='noiseless', reso=RESO, rot=center)
hp.gnomview(maps_noise[NU,:,STK], sub=(2,4,3), title='noise', reso=RESO, rot=center)
hp.gnomview(cov, sub=(2,4,4), title='covmsk', reso=RESO, rot=center)

hp.gnomview(maps_qubic_cmb[NU,:,STK], sub=(2,4,5), title='input', reso=RESO, rot=center)
hp.gnomview(maps_noiseless_cmb[NU,:,STK], sub=(2,4,6), title='noiseless', reso=RESO, rot=center)
hp.gnomview(maps_noise_cmb[NU,:,STK], sub=(2,4,7), title='noise', reso=RESO, rot=center)
hp.gnomview(cov, sub=(2,4,8), title='covmsk', reso=RESO, rot=center)

```

```{python}
_,nus_edge,nus,_,_,_=qubic.compute_freq(d['filter_nu']/1e9, d['nf_recon'], d['filter_relative_bandwidth'] )
_,nus_edge_220,nus_220,_,_,_=qubic.compute_freq(d220['filter_nu']/1e9, d220['nf_recon'], 
                                                d220['filter_relative_bandwidth'] )
...
for j,ipx in enumerate(range(0,24000,3500)):
    ...
    if mctest:
        ...
        ax[0].errorbar(nus_220, maps_qubic220[:,IPIX,0],yerr=std_qubic220[:,IPIX,0], 
               fmt='o-',color='r', alpha=0.5)
        ax[0].plot(nus_220, maps_noiseless220[:,IPIX,0],#yerr=stdconv, 
               'bo-', alpha=0.5)
        #ax[0].plot(nus_in, inputmaps[:,IPIX,0],#yerr=np.array(stdinp), 
        #       'go-',label='input', alpha=1)
        ax[0].set_title('Dust - pix {}'.format(IPIX))
        ax[0].axvspan(nus[-1],nus_220[0],color='k',alpha=0.2)
        ax[2].errorbar(nus, maps_qubic_cmb[:,IPIX,0],yerr=std_qubic_cmb[:,IPIX,0], 
               fmt='o-', color='r',label='recon', alpha=0.5)
        ax[2].plot(nus, maps_noiseless_cmb[:,IPIX,0],#yerr=stdconv, 
               'bo-',label='conv', alpha=0.5)
        ax[2].errorbar(nus_220, maps_qubic_cmb220[:,IPIX,0],yerr=std_qubic_cmb220[:,IPIX,0], 
               fmt='o-',color='r', alpha=0.5)
        ax[2].plot(nus_220, maps_noiseless_220cmb[:,IPIX,0],#yerr=stdconv, 
               'bo-', alpha=0.5)
        #ax[2].plot(nus_in, inputmaps_cmb[:,IPIX,0],#yerr=np.array(stdinp), 
        #       'go-',label='input', alpha=1)
        ax[2].set_title('CMB - pix {}'.format(IPIX))
        ax[2].set_ylabel(r'$I_\nu$ [$\mu$K]',)
        ax[2].set_xlabel(r'$\nu$[GHz]',)
        #ax[2].legend(bbox_to_anchor=(0.83, 0.5, 0.5, 0.5),fontsize=12)
        ax[2].grid()
    ...
    hp.gnomview(maps_qubic[0,:,0], reso=15,hold=True,notext=True,title='{:.1f}GHz'.format(nus[0]),
                max=0.1*np.max(maps_qubic[0,:,0]), rot=center)
    hp.projscatter(hp.pix2ang(d['nside'],IPIX),marker='+',color='r',)
    
    #CMB map
    #ax[2].plot(nus, maps_qubic_cmb[:,IPIX,0],#yerr=std_qubic_[:,IPIX,0], 
    #       'ro-',label='recon', alpha=0.5)
    #ax[2].plot(nus, maps_noiseless_cmb[:,IPIX,0],#yerr=stdconv, 
    #       'bo-',label='conv', alpha=0.5)
    #ax[2].plot(nus_in, inputmaps_cmb[:,IPIX,0],#yerr=np.array(stdinp), 
    #       'go-',label='input', alpha=1)
    ...
    ax[3].cla()
    plt.axes(ax[3])
    hp.gnomview(maps_qubic_cmb[0,:,0], reso=15,hold=True,notext=True,title='{:.1f}GHz'.format(nus[0]),
                min=0.8*np.min(maps_qubic_cmb[0,:,0]),
                max=0.8*np.max(maps_qubic_cmb[0,:,0]),
               rot=center)
    hp.projscatter(hp.pix2ang(d['nside'],IPIX),marker='+',color='r',)
