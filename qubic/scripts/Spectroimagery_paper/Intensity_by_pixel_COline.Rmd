...
co = COLines(target_nside=32, output_units="uK_CMB", line="21")
#pysm_map += bandpass_weight * hp.smoothing(co.signal(), fwhm=fwhm)
```

```{python}
print(x0.shape)
print(co.signal().shape)
```
...
plt.figure(figsize=(13,8))
plt.subplots_adjust(wspace=0.9)
hp.mollview(x0[imap,:,istk], cmap='jet', sub = (1,3,1),
            title = 'x0', min=0, max=1000)
hp.mollview(COsky[imap,:,istk], cmap ='jet', sub = (1,3,2),
            title = 'x0 + CO', min=0, max=10000)
hp.mollview(COsky[imap,:,istk]-x0[imap,:,istk], cmap ='jet', sub = (1,3,3),
            title = 'Diff', min=0, max=100)
...
TOD_CO, _ = a.get_observation(COsky, noiseless=d['noiseless'])#, convolution = True)
```
...
## Build a MultiAcquisition operator for reconstruction (map-making). 
...
print('\n ======== Starting map-making without CO ==========')
...
for pix in pix_indices:
    ...
    plt.plot(nus_rec, maps_recon_CO[:, pix, istk], 'ro', label='Recon CO')
...
plt.plot(nus_rec, np.mean(maps_recon_CO[:, pix_indices, 0], axis=1)/, 'ro', label='Recon with CO')
plt.plot(nus_rec, np.mean(maps_recon[:, pix_indices, 0], axis=1), 'go', label='Recon no CO')
