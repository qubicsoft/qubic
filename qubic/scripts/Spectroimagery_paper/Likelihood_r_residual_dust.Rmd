...
import seaborn as sns # For the color palet
...
```{python}
# If you plan to save data or plots change the path here
dir_save = '/home/lmousset/QUBIC/Qubic_work/SpectroImagerie/Simu_spectroim/FastSimulator'
save_plot = '/home/lmousset/QUBIC/These_manuscrit/Figures/Plots/'
```

```{python}
# Choose the config FI150 or FI220
config = 'FI220'
```
...
```{python}
DataFastSimCoverage = pickle.load(open('../../data/FastSimulator_version01/DataFastSimulator_' + config +'_coverage.pkl',
                                       "rb"))
...
Compute the BB power spectrum for each sky containing dust residuals.

For a fraction of 0.0057, we detect r=0.036 whic corresponds to the BICEP limit today.

```{python}
# dust_fraction = [0., 0.005, 0.01, 0.015, 0.02, 0.025, 0.03, 0.035, 0.04, 0.045, 0.05]
dust_fraction = np.arange(0.0056, 0.0059, 0.0001)
print(dust_fraction)
```
...
### Plot the BB power spectra for each dust fraction
...
The likelihood on r is computed for all the IBCS at the same time. So we obtain one value of r for each case. 
...
#     ax0.plot(rvalues, allLLH[f] / np.max(allLLH[f]), 'o', color=c, label=f'Likelihood frac {frac}')
    ax0.plot(x, allLLH_interp[f]/ np.max(allLLH_interp[f]),
                 color=c, label=f'{dust_fraction[f]*100:1.2f} % dust residuals')
...
for f, frac in enumerate(dust_fraction[:]):
    ax.errorbar(dust_fraction[f], allr_dust[f],
             yerr=allsigma68[f] - allr_dust[f], 
             fmt='o', color='r')
ax.set_xlabel(r'$f_{\rm dust}$', fontsize=16)
ax.set_ylabel(r'$r_{\rm dust}$', fontsize=16)
ax.grid()

#plt.savefig(save_plot + f'Likelihood_dust_rdust_{config}_3y_{nbands}bands_fdust0to06.pdf', bbox_inches='tight')
```

## Likelihood of each IBCS

The likelihood on r is computed for each IBCS separately. So we get one value of r for each IBCS. For example, in case of 2 sub-bands, we have 3 IBCS (f1, f1), (f1, f2) and (f2, f2).
...
for f in range(len(dust_fraction)):
    ...
    ax0.errorbar(nus_eff[:], 
                 ...
                 fmt='o',
                 color=c, 
                 label=f'{dust_fraction[f]*100:1.2f} % dust residuals')   
...
- r is not 0 (B modes in the sky)
- r is 0 but with dust residuals so we detect a non zero r.

The value of r in the first cases is adjusted such that it is equal to the value of r detected with dust residuals.
The likelihood is still computed for each IBCS.
...
plt.errorbar(nus_eff, 
             ...
             fmt='o',
             color='b', 
             label=r'CMB$(r = 0)$' + f' + {dust_fraction[index]*100:1.2f} % dust residuals' + 
                     f'\nSlope significance: {significance_a[index]:1.2f} $\sigma$')   
...
plt.errorbar(np.array(nus_eff)+0.5, 
                 ...
                 fmt='o',
                 color='r', 
                 label='CMB$(r = {:1.3f})$'.format(smallr))  
plt.plot(x, np.polyval(popt_CMBwithr, x), color='r')


plt.xlabel(r'$\nu$ [GHz]', fontsize=20)
plt.ylabel(r'$r_{\rm dust}$', fontsize=20)
plt.legend(fontsize=14, loc='upper left')
plt.grid()
plt.title(config[-3:] + f' GHz - {nbands} bands', fontsize=20)
# plt.savefig('/home/lmousset/QUBIC/Qubic_work/SpectroImagerie/paper_plot/'+
