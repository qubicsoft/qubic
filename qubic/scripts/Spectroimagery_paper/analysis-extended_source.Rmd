...
plt.rc('font', size=16)
reso=12
fig,ax=plt.subplots(nrows=nrec,ncols=coln,figsize=(16,6), sharex=True,
...
plt.text(-4.,0.66,'Convolved',fontsize=16)
plt.text(-4.,0.,'Reconstructed',fontsize=16)
#plt.savefig('extsource_bar_5rec')
#plt.text(-1.5,0,'{:.0f}GHz'.format(nusrec[2]),fontsize=16)
```

```{python}
hp.gnomview(maprec[0,:,0], rot= center,reso=12,notext=True,
                max= vmax,
                min =vmin,
                cmap=cmap, title = None,
                cbar= True if i == 4 else False)
