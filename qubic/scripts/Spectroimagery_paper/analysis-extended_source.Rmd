...
plt.rc('font', size=8)
hp.gnomview(maprec[1,:,0],reso=13,rot=center,min=-1e5,max=1e5,cmap='jet')
```
...
for i in range(d['nf_recon'][0]):
    ...
    #PSF into sub-band
    for iel,jband in enumerate(inband):
        if jband: 
            #print(i,iel,jband)
            input_editted[jband, :, :]*=model_pol(rel_dist(nus[iel]), *parspol)
    #PSF neigbohur bands
    ...
    if i == 0:
        ...
        if optA:
            #print(inband_prev)
            #print(inband_post)
            #print(np.max(input_editted[inband_prev, :, :]),
            #      model_pol(rel_dist(nus[inband_prev]), *parspol ))
            #print(np.max(input_editted[inband_post, :, :]),
            #      model_pol(rel_dist(nus[inband_post]), *parspol) )
            #print(model_pol(rel_dist(nus[inband_post]), *parspol ))
...
for i in range(nrec):
    ...
    ax[coln*i+1].cla()
    plt.axes(ax[coln*i+1])
    hp.gnomview(conv_psf_new[i,:,0],  rot= center,reso=12,notext=True,hold=True,
                max=vmax,
                min=vmin,
                cmap=cmap,
                cbar= True if i == 5 else False,
    ...
    ax[coln*i+4].cla()
    plt.axes(ax[coln*i+4])
    auxmap=(conv_psf_new-maprec)[i,:,0]
    auxmap[~mask]=hp.UNSEEN
    hp.gnomview(auxmap, rot= center,reso=12,notext=True,hold=True,
                max= vmax,
                min =vmin,
