...
#plt.plot(-1, integration1/maxim, 'ko')
#plt.plot(0,  integration2/maxim, 'ko')
#plt.plot(1,  integration3/maxim, 'ko')
plt.legend()
plt.tight_layout()
if savefigs:
    plt.savefig('fpsf_nrec{}_new.pdf'.format(np.shape(Data)[0]), format='pdf' )
```
...
interval1 = np.where((xData > -1.5) & (xData < -0.5))
xintegrate1 = xData[interval1]
#print(xintegrate1 )
#print(result_pol.data[interval1])
interval2 = np.where((xData > -0.5) & (xData < 0.5))
xintegrate2 = xData[interval2]

interval3 = np.where((xData > 0.5) & (xData < 1.5))
xintegrate3 = xData[interval3]

#I1 = simps(y1, x)
integration1 = simps(result_pol.data[interval1], xintegrate1)
integration2 = simps(result_pol.data[interval2], xintegrate2)
integration3 = simps(result_pol.data[interval3], xintegrate3)
print(np.max(integration1))
maxim = np.max([np.max(integration1), np.max(integration2), np.max(integration3)]) 
plt.plot(-0.5, integration1/maxim, 'ko')
plt.plot(0,  integration2/maxim, 'ko')
plt.plot(0.5,  integration3/maxim, 'ko')
```

```{python}
avfwhm_nu = 0.05*qubic.compute_freq(150,2,0.25)[2]/qubic.compute_freq(150,2,0.25)[3]
print(avfwhm_nu[0]/2.35)
plt.plot(xData, np.exp(-0.5 * xData**2/(avfwhm_nu[0]/2.35)**2))
```
