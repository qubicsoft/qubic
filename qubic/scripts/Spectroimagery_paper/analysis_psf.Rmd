...
ax=ax.ravel()
for ifreq, NUS in enumerate(['2','4','6']):#NUS='6'
    print('NUS = {}'.format(NUS))
    ...
    for jk,irec in enumerate(allrecon):
        if len(irec[NUS])==0:
            ...
            yvals.append(irec[NUS][nreal,ifreq,pixel,0]/1e5*NormFact2)
    ...
    for i in range(len(innu)):
        ...
        if yvals[i] == None:
            ...
            ax[ifreq].plot(nus_in[innu][i],yvals[i]/(nusfulledge[NUS][ifreq+1]-nusfulledge[NUS][ifreq]),'o--',c='b',alpha=0.4,ms=10)
            #secax.plot(xvals[i],yvals[i],'o',c=c2,ms=6)
    xlim=ax[ifreq].get_xlim()
    for iedge in nusfulledge[NUS]:
        if (iedge > np.array(xlim)[0]) and (iedge < np.array(xlim)[1]):
            ax[ifreq].axvline(iedge, color='k', ls='--')
    ax[ifreq].grid()
    ax[ifreq].axvspan(nusfulledge[NUS][ifreq],nusfulledge[NUS][ifreq+1], color='k', alpha=0.20)
    print(NUS, ifreq, nusfull[NUS][ifreq])
    ax[ifreq].axvline(nusfull[NUS][ifreq],ls='-.',c='k')
