...
plt.rc('font', size=14)
result_pol.plot(fig=fig,fit_kws = {'color':'r', },
...
for j,fwhm_i in enumerate(fwhm):
    print('Doing {}/{} sample, nu = {:.1f} FWHM = {:.3f} sigma = {:.3f}'.format(j+1,nsamp, 61.34/fwhm_i,
...
for j,fwhm_i in enumerate(fwhm):
    ...
    print("Chi2 {:.2f} {:.2f} FWHM {:.2f} th {:.2f}".format(result_calb_g.redchi, nusfull['4'][j],
...
for j in range(4):
    print(j)
    nuscal = (nus_in > nusfulledge['4'][j]) * (nus_in < nusfulledge['4'][j+1])
    imap = []
...
fwhmMeasured = np.zeros((len(arrMapsRec),4)) 
for j_c, maps_recon_i in enumerate(arrMapsRec):
    for j, imap in enumerate(maps_recon_i):
        ...
        gmodel = Model(gaussian2d_b, independent_vars=['x',], )
        #Set initial guess 
        fwhm_s0init = d['synthbeam_peak150_fwhm'] * 150 / nusfull['4'][j]
        ...
        print('Done {}/{}, fwhm = {}'.format(j + 1, len(maps_recon_1), fwhm_s0))
...
for j in range(4):
    ax[0].errorbar(nusfull['4'][j], np.mean(fwhmMeasured, axis=0)[j] - model_fwhm(nusfull['4'][j], 
                                                                              *parsfwhm_f) ,
                   yerr = np.std(fwhmMeasured, axis=0)[j], color = 'r', marker = '*',
              label = 'FWHM measured' if j == 0 else None)
    ax[0].plot(nusfull['4'][j], fwhmTh(nusfull['4'][j]), 'bo', alpha = 0.5, 
...
```{python}
for j in range(4):
    deltaPorc = fwhm_arrs0[j] - model_fwhm(nusfull['4'][j], *parsfwhm_f) - fwhmTh(nusfull['4'][j])
    print('{:.3f}'.format(deltaPorc/fwhmTh(nusfull['4'][j] *100) ))
```
