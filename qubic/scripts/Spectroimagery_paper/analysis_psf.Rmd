...
dictshift = {'33130322':-11,'33130175':-9,'32832344':-7, '33130311':-5, 
             '32759969':-4,'32759907':-3, '32574156':-2, '32574178':-1,
             '32574170':0, '32759934': 1, '32574174': 2, '32759973':3, 
             '33130307':5, '32832346':7, '33130149':9,'33130316':11}
...
```{python}
fig,ax=plt.subplots(nrows=1,ncols=3,figsize=(16,4),sharey=True,gridspec_kw={'wspace': 0.02})
fig.subplots_adjust(hspace=0.1,wspace=0.)
ax=ax.ravel()
for ifreq, NUS in enumerate(['2','3','4']):#NUS='6'
    if NUS == '1':
        freqdata=range(1)
    elif NUS == '2':
        freqdata = range(2)
    ...
    for ifreqdata in freqdata:
        ...
        for jk,irec in enumerate(allrecon):
            if len(irec[NUS])==0:
                ...
                yvals.append(irec[NUS][nreal,ifreqdata,pixel,0]/1e5*NormFact2)
        ...
        #secax.set_xlabel(r'$d_\nu$[GHz](dist to the $\nu_{c}$ of the band)')
        ax[ifreq].set_ylabel(r'${I_\nu}$[$\mu K$]' if ifreq ==0 else None, fontsize=20, labelpad=15)
        #ax[ifreq].set_xlim(nus_in[innu][0]*0.98,nus_in[innu][-1]*1.02)
        ax[ifreq].set_xlim(nus_in[0],nus_in[-1])
        #ax[ifreq].set_ylim(,1.08*max(yvals)) if ifreq == 0 else None
        ...
        for i,yh in enumerate(yvals):
            if yh!=None:
                xx.append(nus_in[innu][i])
                yy.append(yh)
                yc.append(yconv[i])
        xx,yy,yc=np.array(xx), np.array(yy), np.array(yc)
        ax[ifreq].plot(xx,yy, 
                       marker=imark,ls='-',c='b',alpha=0.7,ms=6)
        #ax[ifreq].plot(xx,2*yc, 
        #               'o-',c='b',alpha=0.4,ms=6)
        xlim=ax[ifreq].get_xlim()
        for iedge in nusfulledge[NUS]:
            if (iedge > np.array(xlim)[0]) and (iedge < np.array(xlim)[1]):
                ax[ifreq].axvline(iedge, color='k', ls='--',alpha=0.9)
        #ax[ifreq].grid()
        ax[ifreq].axvspan(nusfulledge[NUS][ifreqdata],nusfulledge[NUS][ifreqdata+1], 
                          color='k' if ifreqdata%2==0 else 'w', alpha=0.20)
        #print(NUS, ifreq, nusfull[NUS][ifreq])
        ax[ifreq].axvline(nusfull[NUS][ifreqdata],ls='-.',c='k' , alpha=0.2)
        ax[ifreq].text(nusfull[NUS][ifreqdata], 0.05 , 
                       r'{:.0f}-{:.0f}GHz'.format(nusfulledge[NUS][ifreqdata],
                        nusfulledge[NUS][ifreqdata+1]),weight="bold",fontsize=12,rotation=-90, alpha=0.7)
