...
```{python}
fig,ax=plt.subplots(nrows=1,ncols=3,figsize=(16,5),sharey=True,gridspec_kw={'wspace': 0.03})
...
for ifreq, NUS in enumerate(['2','3','4']):#NUS='6'
    ...
    for ifreqdata in freqdata:
        ...
        for jk,irec in enumerate(allrecon):
            if len(irec[NUS])==0:
                ...
                if neig:
                    ...
                    searchmin.append(np.mean(irec[NUS][nreal,ifreqdata,pixel,0])/1e5*NormFact2)
        ...
        for i,yh in enumerate(yvals):
            if yh!=None:
                xx.append((nus_in[innu][i]-cenfreq)/(nusfulledge[NUS][ifreqdata+1]-nusfulledge[NUS][ifreqdata]))
                yy.append(yh)
                yc.append(yconv[i])
        ...
        #ax[ifreq].text(nusfull[NUS][ifreqdata]-cenfreq, 0.05 , 
    ...
    ax[ifreq].grid()
    #print(searchmin)
    #print(np.argmin(yvals))
    #ax[ifreq].axvline(np.min(searchmin),ymin=0,ymax=1)
...
for i in range(2):
    ax[0].plot(coso2[i],'o-',c=next(color))
    ax[0].axhline(cut, ls='--',c='k')
#
color=iter(cm.jet_r(np.linspace(0,1,5)))
for i in range(3):
    c=next(color)
    ax[1].plot(coso3[i],'o-',c=c)
...
color=iter(cm.jet_r(np.linspace(0,1,5)))
for i in range(4):
    ax[2].plot(coso4[i],'o-',c=next(color))
