...
print('{:.2f}hs'.format(t0[-1]), 2571/60)
print('{:.2f}hs'.format(t2[-1]), 2794/60)
print('{:.2f}hs'.format(t3[-1]), 2495/60)
print('{:.2f}hs'.format(t4[-1]), 2666/60)
...
ax=ax.ravel()
for ifreq, NUS in enumerate(['1','2','3','4','6']):#NUS='6'
    if NUS == '1':
        ifreqdata=0
    elif NUS == '2' or NUS == '3':
        ifreqdata = 1
    ...
    ax[ifreq].set_xlabel(r'$\nu$[GHz]' if ifreq > 1 else ' ')
    #secax.set_xlabel(r'$d_\nu$[GHz](dist to the $\nu_{c}$ of the band)')
    ax[ifreq].set_ylabel(r'${I_\nu}$' if ifreq ==0 else None, rotation=0,fontsize=20)
    ...
    ax[ifreq].plot(xx,yy, 
                   'o-',c='b',alpha=0.7,ms=6)
    #ax[ifreq].plot(xx,2*yc, 
    #               'o-',c='b',alpha=0.4,ms=6)
    xlim=ax[ifreq].get_xlim()
    for iedge in nusfulledge[NUS]:
        if (iedge > np.array(xlim)[0]) and (iedge < np.array(xlim)[1]):
            ax[ifreq].axvline(iedge, color='k', ls='--',alpha=0.6)
    #ax[ifreq].grid()
    ax[ifreq].axvspan(nusfulledge[NUS][ifreqdata],nusfulledge[NUS][ifreqdata+1], color='k', alpha=0.20)
    #print(NUS, ifreq, nusfull[NUS][ifreq])
    ax[ifreq].axvline(nusfull[NUS][ifreqdata],ls='-.',c='k', alpha=0.5)
    ax[ifreq].text(nusfull[NUS][ifreqdata], min(yy) , 
                   r'{:.0f}-{:.0f}GHz'.format(nusfulledge[NUS][ifreqdata],
                    nusfulledge[NUS][ifreqdata+1]),fontsize=12,rotation=-90, alpha=0.7)
ax[5].spines['top'].set_visible(False)
ax[5].set_axis_off()
ax[5].axis("off")
```
...
```{python}
_,nustestedge,nustest,deltanus,_,_=qubic.compute_freq(150,48,0.25)
```
