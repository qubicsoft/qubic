...
def get_Cp(patch, nfrecon, verbose=True, doplot=True):
    ...
    for istokes in range(3):
        Cp[:, :, istokes, :] = covariance[istokes * irec : (istokes + 1) * irec,
                                          istokes * irec : (istokes + 1) * irec, :]
    if verbose: 
...
for irec in d['nf_recon']:
    ...
    if irec == 1:
        ...
        sig2MC_monofreq, sigMCmean_monofreq = get_sig2MC(recon_patch, ang, ang_threshold=ang_th)
        ...
        # Using Cp
        weighted_av, sig2, sigmean = make_weighted_av(recon_patch, Cp, ang, ang_threshold=ang_th,
                                                       verbose=True, doplot=True)
        dict_sig2.update({'{}'.format(irec): sig2})
        dict_sig2.update({'{}mean'.format(irec): sigmean})
        
        sig2MC, sigMCmean = get_sig2MC(weighted_av, ang, ang_threshold=ang_th)
        ...
        sig2MC_prime, sigMCmean_prime = get_sig2MC(weighted_av_prime, ang, ang_threshold=ang_th)
