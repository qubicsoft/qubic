...
def sigma2WCp(rep_simu, date_name, irec, nfrecon, noiseless = 'False', 
        ...
        for ipix in range(npix_patch):
            for istokes in range(3):
                ...
                if np.linalg.det(Cp[:, :, istokes, ipix])!=0.:
                    pr[ireal, ipix, istokes], sig2pr[ipix, istokes] = \
    ...
    if verbose: 
        print('Weigthed mean matrix per pixel', pr.shape)
        print('Variance in MC simulation (shape) = ', sig2MC.shape)

    # Average the over pixels => useless
    for istokes in range(3):
