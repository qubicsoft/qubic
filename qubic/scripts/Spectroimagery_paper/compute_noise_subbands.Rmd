...
def average_pix_sig2(sig2, ang, ang_threshold):
    ...
    print('npix =', npix)
    for istokes in range(3):
        sig2mean[istokes] = np.sqrt(np.sum(sig2[:, istokes][ang < ang_threshold])) / npix
    return sig2mean
...
def get_sig2MC(map_monoband, ang, ang_threshold):
    ...
    sig2MCmean = average_pix_sig2(sig2MC, ang, ang_threshold)
...
```{python}
# Using Cp
weighted_av, sig2, sig2mean = make_weighted_av(maps_recon_patch, Cp, ang, ang_threshold=d['dtheta'], 
                                               verbose=True, doplot=True)
print(sig2, sig2mean)

# Using Cp_prime
weighted_av_prime, sig2_prime, sig2mean_prime = make_weighted_av(maps_recon_patch, Cp_prime, 
...
sig2MC, sig2MCmean = get_sig2MC(weighted_av_prime, ang, ang_threshold=d['dtheta'])
...
sig2MC_monofreq, sig2MCmean_monofreq = get_sig2MC(patch_recon_monofreq, ang, ang_threshold=d['dtheta'])
sig2MCmean_monofreq
```
...
for irec in d['nf_recon']:
    ...
    if irec == 1:
        ...
        sig2MC_monofreq, sig2MCmean_monofreq = get_sig2MC(recon_patch, ang, ang_threshold=d['dtheta'])
        ...
        # Using Cp
        weighted_av, sig2, sig2mean = make_weighted_av(recon_patch, Cp, ang, ang_threshold=d['dtheta'],
                                                       verbose=True, doplot=True)
        dict_sig2.update({'{}'.format(irec): sig2})
        dict_sig2.update({'{}mean'.format(irec): sig2mean})
        
        sig2MC, sig2MCmean = get_sig2MC(weighted_av, ang, ang_threshold=d['dtheta'])
        dict_sig2MC.update({'{}'.format(irec): sig2MC})
        dict_sig2MC.update({'{}mean'.format(irec): sig2MCmean})

        # Using Cp_prime
        weighted_av_prime, sig2_prime, sig2mean_prime = make_weighted_av(recon_patch, Cp_prime, 
                                                                         ang, ang_threshold=d['dtheta'],
                                                                         verbose=True, doplot=True)
        dict_sig2_prime.update({'{}'.format(irec): sig2_prime})
        dict_sig2_prime.update({'{}mean'.format(irec): sig2mean_prime})
        
        sig2MC_prime, sig2MCmean_prime = get_sig2MC(weighted_av_prime, ang, ang_threshold=d['dtheta'])
        dict_sig2MC_prime.update({'{}'.format(irec): sig2MC_prime})
        dict_sig2MC_prime.update({'{}mean'.format(irec): sig2MCmean_prime})
...
```{python}
def plot_sigma2mean(thedict, nf_recon):
    for s in range(3):
        for irec in nf_recon: 
            ...
            if irec == 1: 
                sig2_monofreq = dict_sig2MC['{}mean'.format(irec)][s]
                plt.plot(irec, sig2_monofreq, 'o', label='irec = {}'.format(irec))
                
            else:
                p = plt.plot(irec, thedict['{}mean'.format(irec)][s], 'o', label='irec ={}'.format(irec))
                
#             plt.plot(nf_recon, (sig2_monofreq*np.sqrt(nf_recon) + sig2_monofreq)/1.5)
            plt.title(Stokes[s])
            plt.xlabel('irec')
            plt.ylabel('sig2')
        #         plt.ylim(0, 5)
...
```{python}
p = plot_sigma2mean(dict_sig2MC, d['nf_recon'])
```

```{python}
p = plot_sigma2mean(dict_sig2MC_prime, d['nf_recon'])
```

```{python}
p = plot_sigma2mean(dict_sig2, d['nf_recon'])
```

```{python}
p = plot_sigma2mean(dict_sig2_prime, d['nf_recon'])
