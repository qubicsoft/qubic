...
def get_Cp(rep_simu, date_name, irec, nfrecon, noiseless = 'False', 
    ...
    if irec == 1:
        print('You have to use get_Cp_OneSB function to compute sigma for a single subband')
        return
    if irec not in nfrecon:
        print('Invalid number of freq. {0} not in {1}'.format(irec, nfrecon))
        return
    
    if verbose: print('==== Computing Cp matrix ====')
    #Prepare to save
    
    if verbose: print('nfrecon = ', nfrecon)

    index_frec = nfrecon.index(irec)
    if verbose: print('index_frec = ', index_frec)
    ...
    if verbose: print('maps_recon.shape = ', maps_recon.shape); print('npix_patch = ', npix_patch)
...
def sigma2WCp(rep_simu, date_name, irec, nfrecon, noiseless = 'False', 
    ...
    if verbose: print('Weigth mean matrix per pixel', pr.shape)
    if verbose: print('Variance in MC simulation (shape) = ', sig2MC.shape)

    #Average over realizations. Compute sigma with this method:
    for istokes in range(3):
        sigma2mean[istokes] = np.sqrt(np.sum(sig2MC[:, istokes])) / npix_patch
