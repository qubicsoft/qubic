...
#         Cp_prime = amc.Cp2Cp_prime(Cp, verbose=True)
        ...
        # Using Cp
        weighted_av, sig2, sig2mean = amc.make_weighted_av(recon_patch, Cp, ang, ang_threshold=ang_th,
                                                       verbose=True)
        ...
        # Using Cp_prime
        weighted_av_prime, sig2_prime, sig2mean_prime = amc.make_weighted_av(recon_patch, Cp_prime, 
        ...
        sig2MCmean_prime = amc.average_pix_sig2(sig2MC_prime, ang, ang_threshold=ang_th)
    ...
    for s in range(3):
        for irec in nf_recon: 
            ...
            if irec == 1: 
                ...
                plt.plot(nf_recon, nf_recon, 'k--', label=r'$y = irec$')
                plt.plot(nf_recon, np.ones(len(nf_recon)), 'b--', label=r'$y = 1$')
            else:
            ...
            plt.ylabel(r'$\sigma^2 / \sigma^2_{1band}$', fontsize=20)
#             plt.ylim(0, 0.002)
...
```{python}
p = plot_sig_mean(dict_sig2, d['nf_recon'], color='g', signame='sig2MC')
```

```{python}
p = plot_sig_mean(dict_sig2MC, d['nf_recon'], color='b', signame='sig2MC_prime')
```

```{python}
p = plot_sig_mean(dict_sig2_prime, d['nf_recon'], color='r', signame='sig2')
```
