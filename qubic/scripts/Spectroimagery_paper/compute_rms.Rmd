...
def plot2(simulations, nzones, residuals_way, noiseless):
    ...
    for jsimu, simu in enumerate(simulations):
        #     print(jsimu)
        rms_i, rms_q, rms_u, par = amc.rms_method(name=simu, residuals_way=residuals_way, zones=nzones, noiseless=noiseless)
        rms_I.append(rms_i)
        rms_Q.append(rms_q)
        rms_U.append(rms_u)
        param.append(par)
        ...
        for ist, istokes in enumerate('IQU'):
            ...
            # mean over subbands
            if nzones == 1:
                rms_av = [np.mean(i, axis=0) for i in values] 
            else:
                rms_av = [np.mean(i, axis=1) for i in values]
            ...
            for izone in range(nzones):
                ...
                if nzones==1:
                    plt.plot(nfreq, rms_av,
                             label='nep,ptg: {:1.0e},{}'.format(param[jsimu]['nep'], param[jsimu]['npoint']))
                else:
                    plt.plot(nfreq, rms_av[:, izone],
                             label='nep,ptg: {:1.0e},{}'.format(param[jsimu]['nep'], param[jsimu]['npoint']))
...
```{python}
kfreq = '4' # number of channels for reconstruction
            ...
            for izones in nzones:
                if izones == 1: 
                    ...
                    for iizo in range(izones):
                        ...
                        plt.plot(xdim, rms[jsimu]['{}'.format(kfreq)][iizo], 
                                 color=color[jsimu], marker='*', linestyle='', mew=3, 
                                 ms=par[jsimu]['npoint']/1e3*2, alpha=0.4)
...
```{python}
plot2(simulations9000, nzones=3, residuals_way='noiseless', noiseless=False)
```
