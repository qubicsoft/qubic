    ...
    for i in range(Nbsubbands):
      for istokes in xrange(nstokes):
        if kind == 'QU': #This condition keeps the order IQU in the healpix map
          x0[i, :, istokes+1] = (cmb.T[istokes+1] + dust.T[istokes+1] *
                                 scaling_dust(150, sub_nus[i], 1.59))
        else:
          x0[i, :, istokes] = (cmb.T[istokes] + dust.T[istokes] *
                               scaling_dust(150, sub_nus[i], 1.59))
    return x0
...
def create_input_sky(d, skypars):
    ...
    if d['seed']:
        ...
        cmb = np.array(hp.synfast(
            sp, d['nside'], new=True, pixwin=True, verbose=False)).T
        ...
        spectra_dust = [np.zeros(len(ell)), 
                        ...
                        np.zeros(len(ell))]
        dust = np.array(hp.synfast(
            spectra_dust, d['nside'], new=True, pixwin=True, verbose=False)).T
