...
residuals = amc.get_residuals(date_name, rep_simu, 'noiseless', nf_sub_rec)
...
# def plot_sigma2profile(x, dict_sig2, dict_sig2MC, nbins, nf_recon, xlabel):
...
#             bin_centersMC, mean_binMC, fitMC = make_fit(x, dict_sig2MC['{}'.format(nband)][:, s], nbins)
# #             ax.plot(bin_centersMC, mean_binMC, 's', color=colors[i],
# #                            label='sig2MC nband = {}'.format(nband))
# #             ax.plot(x, fitMC(x), '--', color=colors[i])

#             if nband != 1:
#                 # Interpolation to get a profile
#                 bin_centers, mean_bin, fit = make_fit(x, dict_sig2['{}'.format(nband)][:, s], nbins)
#                 ax.plot(bin_centers, mean_bin/mean_binMC, 'o', label='nband = {}'.format(nband), color=colors[i])
# #                 ax.plot(x, fit(x)/fitMC(x), '--', color=colors[i])
                
            
#             ax.set_title(Stokes[s], fontsize=20)
#             ax.set_xlabel(xlabel, fontsize=15)
#             ax.set_ylabel('$\sigma^2 / \sigma_{MC}^2$', fontsize=15)
# #             ax.set_yscale('log')
# #             ax.set_ylim(0, 10)
# #             ax.setxlim(0, 18)
...
```{python}
plt.plot(np.sqrt(coverage_norm), dict_sig2MC['3'][:, 0], '.')
...
xc, yc, dx, dy, _ = ft.profile(x, dict_sig2['3'][:, 0], 
                               nbins=30, 
                               plot=False,
                               clip=True)


xcMC, ycMC, dxMC, dyMC, _ = ft.profile(x, dict_sig2MC['3'][:, 0], 
                                       nbins=30, 
                                       plot=False,
                                       clip=True)
...
#                 ax.plot(irec, sig2/sig2MC, 'ro', label='sig2')
            
#             ax.set_title(Stokes[s])
#             ax.set_xlabel('# of bands', fontsize=20)
#             ax.set_ylabel(r'$\sigma^2 / \sigma^2_{MC}$', fontsize=20)
...
```{python}
plt.imshow(np.mean(corr, axis=2))#, vmin=-200, vmax=200)
plt.colorbar()
```
