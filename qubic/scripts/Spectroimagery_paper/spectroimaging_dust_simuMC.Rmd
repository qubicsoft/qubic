...
# rep = datadir + 'simu_TD/'
# rep = datadir + 'simu_FI220/'
# rep = datadir + 'simu_FI150/'

nfrecon = 3
rep = datadir + '/simu_nersc_dust_fullpipeline' + f'/nfrecon{nfrecon}/'
...
_, _, nus, _, _, _ = compute_freq(150, nfrecon)
print('nus:', nus)
...
for band in range(nfrecon):
    ...
    plt.text(-0.9,0., '{:3.0f} GHz'.format(nus[band]), fontsize=30)
    ...
    hp.gnomview(maps_residu[real, band, :, istk], cmap='jet', sub=(nfrecon, 3, 3*band+3), 
                rot=(0, 0), reso=rr,
                title = 'Residuals - {0}'.format(d['kind'][istk]), unit='$\mu K$', min=-40, max=40)
plt.tight_layout()
# plt.savefig(f'/home/lmousset/QUBIC/Qubic_work/SpectroImagerie/paper_plot/mapsI_FI220_{nfrecon}bands.pdf')
```
...
#     seenmap = rmc.get_seenmap(fits_noiseless[0])
#     print('seenmap shape:', seenmap.shape)
#     np.save(rep_save + f'seenmap_{nfrecon}bands_FI150.npy', seenmap)
...
#     coverage_patch = coverage[seenmap]
#     coverage_norm = coverage_patch / np.max(coverage_patch)
#     np.save(rep_save + f'coverage_{nfrecon}bands_FI150.npy', coverage_norm)

   
#     # Compute residuals in a given way
#     residuals = amc.get_residuals(fits_noise, fits_noiseless[0], 'noiseless')
#     np.save(rep_save + f'residuals_{nfrecon}bands_FI150.npy', residuals[0])
#     print(residuals.shape)
...
```{python}
# Save IBCS
np.save(rep_save + f'IBCSmix_recon_{nfrecon}bands_150fullpipeline.npy', IBCSmix_recon)
np.save(rep_save + f'IBCSmix_residu_{nfrecon}bands_150fullpipeline.npy', IBCSmix_residu)
```
...
ax.set_ylabel('$D_\\ell$')
ax.legend(fontsize=12, loc='upper center')
ax.set_title(clnames[2])
ax.grid()

plt.tight_layout()
plt.savefig('/home/lmousset/QUBIC/Qubic_work/SpectroImagerie/paper_plot/'+
            'IBCSmixBB_{}bands_recon_galaxydustFullpipeline.pdf'.format(nfrecon),
            bbox_inches='tight')
```
