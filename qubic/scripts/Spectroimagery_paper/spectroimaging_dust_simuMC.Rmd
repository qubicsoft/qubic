...
Nfreq_edges, nus_edge, nus, deltas, Delta, Nbbands = compute_freq(220, nfrecon)
print(nus)
print(nus_edge)
```
...
#     hp.gnomview(maps_conv[real, band, :, istk], cmap='jet', sub=(nfrecon, 3, 3*band+1), 
#                 rot=center, reso=rr, notext=True,
#                 title = 'Input - {}'.format(d['kind'][istk]), min=min, max=max)
#     plt.text(-0.85,0, '{:3.0f} GHz'.format(nus[band]), fontsize=30)
#     hp.gnomview(maps_recon[real, band, :, istk], cmap='jet', sub=(nfrecon, 3, 3*band+2), 
#                 rot=center, reso=rr, notext=True,
#                 title = 'Output - {0}'.format(d['kind'][istk]), min=min, max=max)
#     hp.gnomview(maps_residuals[real, band, :, istk], cmap='jet', sub=(nfrecon, 3, 3*band+3), 
#                 rot=center, reso=rr, notext=True,
#                 title = 'Residuals - {0}'.format(d['kind'][istk]), min=-70, max=70)
# # plt.tight_layout()
# plt.savefig(f'/home/lmousset/QUBIC/Qubic_work/SpectroImagerie/paper_plot/maps_FI220_{nfrecon}bands.pdf')
```
