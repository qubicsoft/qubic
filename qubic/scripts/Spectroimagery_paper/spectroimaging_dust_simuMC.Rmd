...
for band in range(nfrecon):
    ...
    hp.gnomview(maps_residuals[real, band, :, istk], cmap='jet', sub=(nfrecon, 3, 3*band+3), 
                rot=center, reso=rr, notext=True,
                title = '{0} - Residuals with noiseless'.format(d['kind'][istk]), min=-20, max=20)
plt.tight_layout()
...
toto[:, seenmap, :] = maps_recon[0][:, seenmap, :] - noiseless[:, seenmap, :]
toto = maps_recon[0] - noiseless
hp.gnomview(maps_recon[0, band, :, istk], cmap='jet', sub=(131), 
...
```{python}
# rep_save = '/home/lmousset/QUBIC/Qubic_work/SpectroImagerie/Data_for_FastSimulator/'
# rep = datadir + 'simu_TD/'
# # rep = datadir + 'simu_FI220/'
...
#     seenmap = rmc.get_seenmap(fits_noiseless[0])
#     print('seenmap shape:', seenmap.shape)
#     np.save(rep_save + f'seenmap_{nfrecon}bands_TD150.npy', seenmap)
...
#     coverage_patch = coverage[seenmap]
#     coverage_norm = coverage_patch / np.max(coverage_patch)
#     np.save(rep_save + f'coverage_{nfrecon}bands_TD150.npy', coverage_norm)

   
#     # Compute residuals in a given way
#     residuals = amc.get_residuals(fits_noise, fits_noiseless[0], 'noiseless')
#     np.save(rep_save + f'residuals_{nfrecon}bands_TD150.npy', residuals[0])
#     print(residuals.shape)

```

```{python}
sm = np.load(rep_save + f'seenmap_{nfrecon}bands_FI220.npy')
sm.shape

cov = np.load(rep_save + f'coverage_{nfrecon}bands_FI220.npy')
cov

res = np.load(rep_save + f'residuals_{nfrecon}bands_FI220.npy')
res.shape
```
