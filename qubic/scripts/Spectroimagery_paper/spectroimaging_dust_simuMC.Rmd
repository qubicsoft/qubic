...
for band in range(nfrecon):
    hp.gnomview(maps_conv[real, band, :, istk], cmap='jet', sub=(nfrecon, 3, 3*band+1), 
                rot=center, reso=rr,
                title = 'Input - {}'.format(d['kind'][istk]), unit='$\mu K$', min=min, max=max)
    plt.text(-1, 0., '{:3.0f} GHz'.format(nus[band]), fontsize=30)
    hp.gnomview(maps_recon[real, band, :, istk], cmap='jet', sub=(nfrecon, 3, 3*band+2), 
                rot=center, reso=rr,
                title = 'Output - {0}'.format(d['kind'][istk]), unit='$\mu K$', min=min, max=max)
    hp.gnomview(maps_residu[real, band, :, istk], cmap='jet', sub=(nfrecon, 3, 3*band+3), 
                rot=center, reso=rr,
                title = 'Residuals - {0}'.format(d['kind'][istk]), unit='$\mu K$', min=-90, max=90)
# plt.tight_layout()
# plt.savefig(f'/home/lmousset/QUBIC/Qubic_work/SpectroImagerie/paper_plot/maps{stokes[istk]}_FI220_{nfrecon}bands_GoodPhotonNoise.pdf',
#            bbox_inches='tight')
```
...
# Plot for spectroim paper
for band in range(1):
    hp.gnomview(maps_conv[real, band, :, istk], cmap='jet', sub=(1, 3, band+1), 
                rot=(0, 0), reso=rr,
                title = 'Input - {}'.format(d['kind'][istk]), unit='$\mu K$', min=min, max=max)
    plt.text(-1,0., '{:3.0f} GHz'.format(nus[band]), fontsize=30)
    hp.gnomview(maps_recon[real, band, :, istk], cmap='jet', sub=(1, 3, band+2), 
                rot=(0, 0), reso=rr,
                title = 'Output - {0}'.format(d['kind'][istk]), unit='$\mu K$', min=min, max=max)
    hp.gnomview(maps_residu[real, band, :, istk], cmap='jet', sub=(1, 3, band+3), 
                rot=(0, 0), reso=rr,
                title = 'Residuals - {0}'.format(d['kind'][istk]), unit='$\mu K$', min=-50, max=50)
plt.tight_layout()
# plt.savefig(f'/home/lmousset/QUBIC/Qubic_work/SpectroImagerie/paper_plot/mapsQ_FI220_{nfrecon}bands_firstband.pdf')
```
...
Of course this makes sens only if you have many realisations.

```{python}
# band = 0
# istk = 0
# diff = np.mean(maps_recon, axis=0) - noiseless
# hp.gnomview(np.mean(maps_recon, axis=0)[band, :, istk], cmap='jet', sub=(131), 
#                 rot=center, reso=rr, notext=True, min=0, max=10000,
#                 title = 'Recon mean')
# hp.gnomview(noiseless[band, :, istk], cmap='jet', sub=(132), 
#                 rot=center, reso=rr, notext=True, min=0, max=10000,
#                 title = 'Noiseless')
# hp.gnomview(diff[band, :, istk], cmap='jet', sub=(133), 
#                 rot=center, reso=rr, notext=True, min=-70, max=70,
#                 title = 'Recon mean - Noiseless')
```
...
Here we save the files that will be used to create data for FastSimulator

```{python}
# rep_save = datadir + 'Data_for_FastSimulator/'

# rep = datadir + '/NERSC_simuEnd2End/Good_photon_noise2020/simu_FI220/'
...
#     seenmap = rmc.get_seenmap(fits_noiseless[0])
#     print('seenmap shape:', seenmap.shape)
#     np.save(rep_save + f'seenmap_{nfrecon}bands_FI220.npy', seenmap)
...
#     coverage_patch = coverage[seenmap]
#     coverage_norm = coverage_patch / np.max(coverage_patch)
#     np.save(rep_save + f'coverage_{nfrecon}bands_FI220.npy', coverage_norm)

   
#     # Compute residuals in a given way
#     residuals = amc.get_residuals(fits_noise, fits_noiseless[0], 'noiseless')
#     np.save(rep_save + f'residuals_{nfrecon}bands_FI220.npy', residuals[0])
#     print(residuals.shape)
