...
_, _, nus_rec, _, _, _ = qubic.compute_freq(d['filter_nu'] / 1e9, nf_sub_rec,
                                                d['filter_relative_bandwidth'])
print(nf_sub_rec, nus_rec)

```

```{python}
"""
To nus_rec = 2 --> idx0(140GHz) =  
"""
```
...
for ifreq in d['nf_recon'][1:]:
    ...
    nusrec.update({'{}'.format(ifreq) : nus})
```
...
rc('font', size=12)
for iN in [2,4,8]:
    plt.figure(figsize=(iN*3,iN*3))
    plt.subplots_adjust(wspace=0.1, hspace=0.1)
    for i in range(iN):
    
        hp.gnomview(mapsrec['{}'.format(iN)][i,:,istk]/1e10, cmap ='jet', sub = (3,iN,i+1), 
                    rot=center, reso=rr, 
            title = '{1:4.1f}GHz {0} Stk, tol: {2}, rec'.format(d['kind'][istk], 
                                                                nusrec['{}'.format(iN)][i], d['tol']))
        hp.gnomview(mapsconv['{}'.format(iN)][i,:,istk]/1e10, cmap ='jet', sub = (3,iN,i+(iN+1)), 
                    rot=center, reso=rr, 
            title = '{0} Stk, conv'.format(d['kind'][istk],))
        hp.gnomview((mapsconv['{}'.format(iN)]/1e10-mapsrec['{}'.format(iN)]/1e10)[i,:,istk], 
                    cmap ='jet', sub = (3,iN,i+(2*iN+1)), 
            rot=center, reso=rr,title = '{0} Stk, diff'.format(d['kind'][istk]))
    #plt.savefig('20200319-recconvmap{}sub'.format(iN))
    #plt.clf()
```
...
hp.gnomview(np.sum(maps_recon,axis=0)[:,0]/1e10, cmap ='jet', sub = (1,2,1), 
            rot=center, reso=rr,
            title='sum Rec (nf={})'.format(iN, norm))
...
for j, iN in enumerate([2,4,8]):
    IDX = '{}'.format(iN)
    C = ['r','b','g']
    F = ['*','^','o']
    AmpMax4R = []
    for imap in mapsrec[IDX]:
        AmpMax4R.append(np.max(imap[:,0])/1e10) 
    AmpMax4C = []
    for imap in mapsconv[IDX]:
        AmpMax4C.append(np.max(imap[:,0])/1e10) 

    plt.plot(nusrec[IDX], AmpMax4R, '{}{}'.format(C[j],F[1]), ms=13,label='recons {}sub'.format(iN))
    plt.plot(nusrec[IDX], AmpMax4C, '{}{}'.format(C[j],F[2]), ms=13,label='conv')
plt.plot(nus_in[idx0], 1, 'k+', ms= 10, mew=4, label='input')
plt.plot(nus_in[idx1], 1, 'k+', ms=10, mew=4)
plt.legend(loc='best')
plt.grid()
```
