...
for i in range(10):
    ...
    np.random.seed(seed) 
    TOD, _ = si.create_TOD(d, p, x0ones, verbose=True)
    allTOD.append(TOD)

#     maps_recon[i], cov[i], nus, nus_edge, maps_conv[i] = si.reconstruct_maps(TOD, d, p, nf_sub_rec, x0cmb)
    
#     simu_name = 'skycmb_tol=1e-5_kmax=8_sbfrac={}'.format(sbfrac)
...
for i in range(10):
    sbfrac = (i + 1) / 10
    std_rapport = np.max(allTOD[i, :, :]) / np.max(allTOD[9, :, :])
    plt.plot(sbfrac, std_rapport, 'o')
    plt.xlabel('SynthBeam fraction')
    plt.ylabel('TOD Mean for a given f / TOD Mean for f=1 ')
    plt.plot([0, 1], [0, 1], 'k')
```
...
for i in range(9):
    ...
    plt.plot(ang, std_profile[:, 0, 0], '.', label=simu_name[-10:])
# plt.xlim(0, 7)
# plt.ylim(0.3, 0.8)
...
```{python}
plt.figure()
plt.plot(TOD1[0], 'ro', label='kmax=1')
plt.plot(TOD2[0], 'bo', label='kmax=2')
plt.plot(TOD3[0], 'go', label='kmax=8')
plt.legend()
plt.show()
```
...
nf_sub_rec = 1
# d['synthbeam_kmax'] = 1
# maps_recon1, cov1, nus, nus_edge, maps_conv1 = si.reconstruct_maps(TOD1, d, p, nf_sub_rec, x0)

# d['synthbeam_kmax'] = 2
# maps_recon2, cov2, _, _, maps_conv2 = si.reconstruct_maps(TOD2, d, p, nf_sub_rec, x0)

d['synthbeam_kmax'] = 8
maps_recon3, cov3, _, _, maps_conv3 = si.reconstruct_maps(TOD3, d, p, nf_sub_rec, x0)
print(maps_recon.shape)
```

```{python}

# rmc.save_simu_fits(maps_recon1, cov1, nus, nus_edge, maps_conv1,
#                   save_dir=save_dir, simu_name=simu_name + '_kmax1')

# rmc.save_simu_fits(maps_recon2, cov2, nus, nus_edge, maps_conv2,
#                   save_dir=save_dir, simu_name=simu_name + '_kmax2')


rmc.save_simu_fits(maps_recon3, cov3, nus, nus_edge, maps_conv3,
                  save_dir=save_dir, simu_name=simu_name + '_kmax8')
