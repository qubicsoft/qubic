...
d_TOD['noiseless'] = True#noiseless
d_TOD['config'] = 'FI'
...
if Nf_recon == 1 and fact_sub == 1:
    TOD_QUBIC = H_TOD(m_sub[0]).ravel()# + n_qubic.ravel()
else:
...
for i in range(Nf_recon):
    ...
    target = np.mean(qubic_acquisition.allfwhm[i*fact_sub:(i+1)*fact_sub])                   # Target FWHM
    print(target)
    C = HealpixConvolutionGaussianOperator(fwhm=target)                                     # Operator
    m_planck_for_tod[i] = C(m_sub[i])#planck_obs[0]# + (n_planck.copy() * noise_planck_level)         # Convolution
...
for i in range(Nf_recon*fact_sub):
    ...
    print(target)
    C = HealpixConvolutionGaussianOperator(fwhm=target)                                     # Operator
    m_planck_all[i] = C(m_sub[i])#planck_obs[0]# + (n_planck.copy() * noise_planck_level)         # Convolution
...
if user == 'Pierre' or user == 'Mathias':
    ...
    solution_qubic_planck = mypcg.pcg(A, b, x0=m_planck_for_tod[0, seenpix], M=M, tol=tol, disp=True, maxiter=50)
elif user == 'full':
    solution_qubic_planck = mypcg.pcg(A, b, x0=None, M=M, tol=tol, disp=True, maxiter=200)
elif user == 'old':
...
#seenpix = generate_circular_mask(256, 10, center[0], center[1])
#seenpix = covnorm > threshold
mysolution[:, ~seenpix] = m_planck_for_tod[0, ~seenpix].copy()
inputs[:, ~seenpix] = m_planck_for_tod[0, ~seenpix].copy()
```
...
for i in range(3):
    ...
    hp.gnomview(inputs[0, :, i], cmap='jet', rot=center, reso=res, min=-v, max=v, sub=(2, 3, 2), title='Input')

    hp.gnomview(mysolution[0, :, i]-inputs[0, :, i], cmap='jet', rot=center, reso=res, title='Residual - Input',
...
```{python}
nb_samples = 4
allfwhm = np.linspace(0.004, 0.008, nb_samples)
allnus = np.linspace(190, 205, nb_samples)

dust = np.zeros((nb_samples, 12*nside**2, 3))
dust_conv = np.zeros((nb_samples, 12*nside**2, 3))

for i in range(nb_samples):
    print(i)
    C = HealpixConvolutionGaussianOperator(fwhm=allfwhm[i])
    dust[i] = qubic_acquisition.get_PySM_maps(sky_config, nus=np.array([allnus[i]]))[0]
    dust_conv[i] = C(qubic_acquisition.get_PySM_maps(sky_config, nus=np.array([allnus[i]]))[0])
    
mean_dust = np.mean(dust, axis=0)
mean_dust_conv = np.mean(dust_conv, axis=0)
```

```{python}
mydust = qubic_acquisition.get_PySM_maps(sky_config, nus=np.array([197.5]))[0]
#C = HealpixConvolutionGaussianOperator(fwhm=0.0060)
#mydust = mydust

plt.figure(figsize=(15, 5))

hp.mollview(mean_dust[:, 1]-mydust[:, 1], cmap='jet', min=-1, max=1, sub=(1, 3, 1))
hp.mollview(mean_dust_conv[:, 1]-mydust[:, 1], cmap='jet', min=-0.1, max=0.1, sub=(1, 3, 2))
#hp.mollview(mydust[:, 1], cmap='jet', min=-8, max=8, sub=(1, 3, 3))
#hp.mollview(mydust[:, 1], cmap='jet', min=-8, max=8, sub=(1, 3, 3))
