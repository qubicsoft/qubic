...
Nbfreq, nus_edge, nus, deltas, Delta, Nbbands = qubic.compute_freq(220, Nfreq=Nf)
print(nus)
print(nus_edge)
...
```{python}
H_qubic = qubic_acquisition.get_operator(convolution=True)
```
...
for i in range(2):
    ...
    display_maps(np.array([C(mapin[i])]), np.array([solution_qubic['x'][i]]), istk=0, display=True, rot=center, res=25, min=-300, max=300, minr=-30, maxr=30)
    plt.show()
```
...
```{python}
H = qubicplanck_acquisition.get_operator(convolution=True)
```
