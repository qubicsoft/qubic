...
qmono = qubic.QubicInstrument(d)

# QUBIC Acquisition
qubic_acquisition = Acq.QubicMultibandAcquisition(q, p, s, d, nus_edge)
qubic_acquisition_mono = Acq.QubicAcquisition(qmono, p, s, d)
```
...
```{python}
tod = qubic_acquisition.get_observation(m=mapin, convolution=False, noiseless=True)
```
...
display_maps(mapin, solution_qubic['x'], istk=0, display=True, rot=center, res=25, min=-300, max=300, minr=-300, maxr=300)
```
...
```{python}
cov = qubic_acquisition_mono.get_coverage()
```
...
#mapin_conv[:, ~mask, :] = 0
planck_acquisition = Acq.PlanckAcquisition(143, s, true_sky=mapin_conv[1], mask=None)
```
...
```{python}
display_maps(mapin_conv, solution_qubic_planck['x'], istk=1, display=True, rot=center, res=15, min=-10, max=10, minr=-10, maxr=10)
```

```{python}
plt.figure(figsize=(15, 5))
hp.gnomview(mapin_conv[0, :, 1], rot=center, reso=15, sub=(1, 3, 1), cmap='jet', min=-7, max=7)
hp.gnomview(np.mean(solution_qubic_planck['x'], axis=0)[:, 1], rot=center, reso=15, sub=(1, 3, 2), cmap='jet', min=-7, max=7)
hp.gnomview(mapin_conv[0, :, 1]-np.mean(solution_qubic_planck['x'], axis=0)[:, 1], rot=center, reso=15, sub=(1, 3, 3), cmap='jet', min=-7, max=7)
plt.show()
```
