...
Pipeline = Acq.PipelineReconstruction(o2, [], comp=[c.CMB(), c.Dust(nu0=150, temp=20)], type='TwoBands', convolution=False)
...
Then, we apply the PCG by defining the operator `H`, the `tod` used, the inverse noise covariance matrix `invN`. We need to define also a starting point for spectral index $\beta_d$, we fix the `kmax` to 1 in this notebook to see the convergence of the algorithm, a vlue of `kmax` different to 1 allow to estimate the spectral index every n-iteration.

```{python}
sol, beta = Pipeline.call_pcg(H, tod, invN, 1.54, tolerance=1e-10, x0=comp_for_pcg, kmax=1, maxiter=500)
```
