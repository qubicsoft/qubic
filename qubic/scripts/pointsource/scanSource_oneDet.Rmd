...
elcenter = el[len(el)/2]#+(offele-50.)#peak95[1,4]#
azcenter = az[len(az)/2]#-(offaz-0)peak95[0,4]#
...
#      interpolation=None)
plot(az2d,el2d,'yo')
plot(az2d[0,0],el2d[0,0],'yo', label='meshgrid')
plot(peaks[tesid,0,:], peaks[tesid,1,:],'wo', label = 'fitted')
legend(loc='best', numpoints=1, bbox_to_anchor=(0.5,1.0))
#show()
grid()
#savefig('gridproblem')
```
...
for j,each in enumerate(newarr):
    # rotation from center of the field in az,el to 
    posF00 = np.dot(rotmatZ(-(np.pi-phis9[j]) )  , each  )
    posF0i = np.dot(rotmatX(+thetas9[j] ) , np.asarray(posF00)  )
