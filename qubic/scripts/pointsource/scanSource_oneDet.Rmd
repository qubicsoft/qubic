...
TESid=93
d['nside']=512
dirflat = '/home/mgamboa/qubic/QUBIC/qubiccalibration/Data/150GHz-2019-04-06/Flat/'
...
tes93healpy =hp.ud_grade(tes93healpy, 512)
xr=0.1*np.max(sb)

figure(figsize=(12,10))
#min=-3.61e3, max=1.39e5,
sat = np.where(sb>xr)
sb2=np.copy(sb)
sb2[sat] = xr

hp.gnomview(tes93healpy/np.max(tes93healpy),title='Measured @150GHz 2019-04-06 \n Healpix TES={} \n Normalized to the maximum'.format(TESid), 
            sub=(1,3,1), reso = 10, xsize = 200, min=0,max=1,cmap='viridis')

hp.gnomview(sb/np.max(sb), rot=[0,90], xsize=200, reso=10, title='Theory - No aberrations  \n Normalized to the maximum', 
            sub=(1,3,2), min=0, max=1,hold=False,cmap='viridis')

hp.gnomview(sb2/np.max(sb2), rot=[0,90], xsize=200, reso=10, title='Theory - No aberrations  \n Saturated and normalized to the maximum', 
            sub=(1,3,3), min=0, max=1,hold=False,cmap='viridis')
```

```{python}
pp= qubic.get_pointing(d)
#delta_el = pp.elevation[-1]-pp.elevation[0]
#print('Delta_elevation = {}'.format(delta_el))
```

```{python active="", eval=FALSE}
print(len(set(pp.elevation)))

plt.figure(figsize=(8,12))
plt.subplot(3,1,1)
plt.plot(pp.time,pp.azimuth)
plt.ylabel('Azimuth',fontsize=22)
plt.subplot(3,1,2)
plt.plot(pp.time,pp.elevation)
plt.ylabel('Elevation',fontsize=22)
plt.subplot(3,1,3)
plt.plot(pp.time,pp.angle_hwp)
...
```{python active="", eval=FALSE}
a = qubic.QubicAcquisition(q, pp, s, d)
TOD,maps_conv = a.get_observation(x0, convolution = True, noiseless=True)
```

```{python active="", eval=FALSE}
#print(TOD.shape)
plt.plot(TOD[0,:])
#plt.xlim(1.2e6,1.5e6)
plt.show()
```
...
if alaImager==True:
    ...
    q = qubic.QubicInstrument(d)
    if sel_det:
        q=select_det(q,did_det['93'])
    
    pp= qubic.get_pointing(d)
    s = qubic.QubicScene(d)
    arec = qubic.QubicAcquisition(q, pp, s,d)
else:
    arec = qubic.QubicAcquisition(q, pp, s,d)

d['tol'] = 5e-4

maps_recon = arec.tod2map(TOD, d, cov=None)

cov = arec.get_coverage()
cov[id]=np.nan
hp.mollview(cov, rot=center, cmap='viridis')
plt.show()
im_old=hp.gnomview(cov, rot=center, reso=5,title='Cov ', return_projected_map=True,cmap='viridis',hold=True, xsize=500)
plt.show()
...
if d['kind']=='I':
    xr=0.1*np.max(maps_recon[:])
    im_old=hp.gnomview(maps_recon[:], rot=center, reso=5, title='Output ', min=-xr, max=xr,cmap='viridis',
                       return_projected_map=True,hold=True, xsize=500)
    plt.show()
else:
...
```{python active="", eval=FALSE}
# Read the output dir and store the info

n = '__ScanMap_Speed_VE4_El_' #sys.argv[1]
freq_mod = float(9) #float(sys.argv[2])
doit = int(0) #int(sys.argv[3])
replace = 0#int(sys.argv[4])
method = 'rms'#sys.argv[5]
days = ['150GHz-2019-04-06']#sys.argv[6:]
AsicNum = 1
print(n)
print days
dirs = []
elevations=[]
for da in days:
    dd = glob.glob('/home/mgamboa/qubic/QUBIC/qubiccalibration/Data/'+da+'/*'+n+'*')
...
```{python}
figure(figsize=(8,6))
plt.imshow(datamap, cmap='viridis')
plt.show()
...
```{python}
# figure(figsize=(12,12))
#plot(myTOD, 'b-', label = 'real TOD')
#plot(1e28*simTOD[0], 'r-', alpha = 0.6, label = 'simu TOD')
#legend()
#cc=myq.horn.center
#cc.shape
#np.sqrt(np.sum((cc[0,:]-cc[1,:])**2))

"""choose TES from peaks"""
#print(TESid,tesid)
peak95 = peaks[tesid]
#print(peak95[1,:])
...
locpeak_tod = []
print('====')
for i in xrange(0,len(peak95[0])):

    azloc = np.where(abs(az2dravel-peak95[0,i]) < 0.12 )
...
#print(xaxisred)
#print(xaxisgreen)
angulo = np.arccos(np.dot(xaxisred,xaxisgreen))
import math as m
angulo2 = m.atan2(np.radians(0.81),np.radians(0.12)*np.cos(np.radians(50)) )
...
if rott:
    for j in xrange(0,len(el0)):
        posFx= np.dot(rotmatX(-np.radians(angulo2) )  , np.asarray(pos0[j])  )
        posFz = np.dot(rotmatZ(-angulo )  , np.asarray(posFx)  )
        posFxx = np.dot(rotmatX(np.radians(angulo2) )  , np.asarray(posFz)  )
        posF0 = np.dot(rotmatZ(-(np.radians(azcenter)*np.cos(np.radians(elcenter)) )), np.asarray(posFxx))
...
np.savetxt('leb.txt',[th0.T, ph0.T])
...
newnew = mytod[0][::-1]
imshow(np.reshape(mytod[0],(el.shape[0],linen)), extent=[az.min(),#*np.cos(np.radians(50)),
...
map1=hp.gnomview(maps_recon_real[:,0],sub=(1,3,1),title='Recons (real TOD, modify myinst)',#min=-0.1,max=0.05,
