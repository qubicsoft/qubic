...
tod = tod1[tesnum-1]
slowpart_tod = savgol_filter(tod, int(len(tod)/10), 4)
flat_tod = tod-slowpart_tod
...
risetime_flat = np.zeros(N)
riseerr_flat = np.zeros(N)
falltime_flat = np.zeros(N)
fallerr_flat = np.zeros(N)
...
for i in range(N):
    ...
    risetime[i] = risefit_folded
    riseerr[i] = riseerr_folded
    falltime[i] = fallfit_folded
    fallerr[i] = fallerr_folded
    ...
    risetime_flat[i] = risefit_folded
    riseerr_flat[i] = riseerr_folded
    falltime_flat[i] = fallfit_folded
    fallerr_flat[i] = fallerr_folded
    
```

```{python}
figure()
errorbar(np.arange(N),risetime,yerr=riseerr,fmt='.',label='raw tod')
...
figure()
errorbar(np.arange(N),falltime,yerr=fallerr,fmt='.',label='raw tod')
errorbar(np.arange(N),falltime_flat,yerr=fallerr_flat,fmt='.',label='flattened tod')
