...
thedirs = np.sort(glob.glob(data_dir+'*12.55.04*'))
...
for i,thedatadir_i in enumerate(thedirs):
    try:
        alldic['{}'.format(i+1)] = np.load('d__{}.npy'.format(str.split(thedatadir_i,'/')[-1]),allow_pickle=True).item()
    except:
        print('No directory')
```
...
for i in np.arange(len(thedirs)):
    ...
    hist(1e3*alldic['{}'.format(i+1)]['alltod_folded_nonorm']['risetime'][ok],30,alpha=0.5,label='Vbias {:.1f}'.format(Vbias[i]))
