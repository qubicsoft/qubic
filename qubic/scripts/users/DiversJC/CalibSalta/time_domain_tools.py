...
def decorel_azimuth(mytod, azt, scantype, degree=2, doplot=True):
    ...
    for i in range(len(oks)):
        ...
        z = polyfit(xc, yc, degree, w=1.0 / dy)
        p = np.poly1d(z)
        polys.append(p)
...
def identify_scans(inthk, az, el, tt=None, median_size=101, thr_speedmin=0.1, doplot=False, plotrange=[0, 1000]):
    ...
    if doplot:
        ...
        plot(
            az[cpos], medaz_dt[cpos], ".", label="Scan + : mean={:4.2f}deg/s max={:4.2f}deg/s".format(np.mean(medaz_dt[cpos]), np.max(medaz_dt[cpos]))
        )
        plot(
            az[cneg], medaz_dt[cneg], ".", label="Scan - : mean={:4.2f}deg/s max={:4.2f}deg/s".format(np.mean(medaz_dt[cneg]), np.min(medaz_dt[cneg]))
        )
        xlabel("Azimuth [deg]")
...
def jumps_finder(dd, size_haar=51, threshold=5, size_fact=3.0, doplot=False, verbose=False):
    ### Haar filter
    ...
    if verbose:
        print("Detected {} cluster".format(len(xc)))
    if verbose:
        print(xc)
